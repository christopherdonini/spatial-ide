...
**DIRECTIVE 2026-09-18 — the record cap (the human, mid-turn after round 15; recorded verbatim at `state/directives/2026-09-18-record-cap.md:5` (the directive line; its sha256 e44168d98d83bff75fc1155dd8b1f94b4ac077e113c8deabacaa93fd2622ef58 at the commit that adds it)):** five numbered points — the record of a piece is the smallest text a gate can verify, a closing amendment references and hashes only; record-fidelity failures spawn no new clauses, the fourteen adopted this week are the mechanism, a new class goes to the ledger and to the human at most once a week; two record-correction rounds per piece, then the architect reduces the record to references and the piece lands; the health strip carries a record-round count per piece, target zero; after the erratum rows land, P3b, the LOD and the checker are landed, the checker's own amendment reduced to references rather than attempted a fifth time. Applied: the directive's points (1) and (3) enter `docs/PREREGISTRATION-TEMPLATE.md` §10, both gate checklists, the worker brief and `AI_DEVELOPMENT.md` by reference to the directive file; round 15's item 2 ("Hold both") is read as superseded by point (5) — the erratum rows for P3b and the LOD are written by the architect as references and hashes, resolved per row, executed by a fresh worker with the resolution step, and the three pieces then land through PRs the human clicks; the RESIDENCY note (two failed reads) is reduced to references by the architect under point (3) and lands with the reduced record; point (4) becomes PLAN node `governance-record-round-count` (a `record` field on gate records, counted per node on the strip); the checker's one build-fix round (round 15, item 3) stands with a references-only closing amendment.

**RULED 2026-09-19 — entry 115, item (c), the shell's npm vulnerabilities (the human, mid-turn; verbatim):** **"Entry 115 vulnerabilities: fix now, as one dependency piece under the full gate. First determine reachability: for each of the five, is the vulnerable package dev-only/build-only or does it ship in the packaged frontend or the bundle viewer? Shipped ones (any severity) are fixed first; dev-only ones follow in the same piece. Patch-level bumps land under P-033; anything minor/major or introducing a new package comes back to me with the lockfile diff and licence check. npm audit's severity is an input, not the ruling — reachability decides urgency. Regenerate the notice set in the same PR"** Applied: one dependency piece, `cut/shell-dependency-audit`, under §21a full gating (reviewer + architect) with its own preregistration committed before any change; phase 1 is read-only — a fresh worker classifies each of the five advisories by reachability (dev-only/build-only versus shipped in the packaged frontend or the bundle viewer, by the notice generator's own definition of the shipped set), with the dependency paths, the fix npm proposes, the semver class of every bump, any new package, and the licence of the installed and target versions; phase 2 lands the shipped ones first and the dev-only ones after in the same piece, patch-level bumps under P-033 (lockfile diff in the record), and regenerates the notice set in the same PR; anything minor or major, or any new package, is not applied — it returns to the human with the lockfile diff and the licence check as the piece's decision item. npm audit's severity is recorded as an input; the reachability class orders the work.
...
**RULED 2026-09-26 — question round 25 (AskUserQuestion, one set, mirrored to Telegram first as `state/questions/round-25.md`; the weekly window, raised a day late; each verbatim; item 4 a red line):**

- *Item 1 — the six small defects and flakes:* **"(1), with (d) promoted: reproduction first, before the other five. Determine whether any Drop reached from Catalog::open's replaced dataset can block or reacquire the catalog lock (the #116 pattern). If it can, it's S1 and becomes a cut with #116's fix shape (drop after the guard is released). If it can't, it's S2: a latency note, filed like the rest. For (a), the fix follows the slice.rs precedent: assert the property and its ordering, report the timing, and never assert a budget docs/08 doesn't declare"** Applied, in PLAN.yaml:
  - (d) is `catalog-open-drop-reproduction`, ready now: a tester-high determines whether any Drop reached from the replaced dataset can block or reacquire the catalog lock. Its outcome is S1 (a cut, #116's fix shape) or S2 (a latency note).
    - Outcome, 2026-09-26: S2 (`state/consults/2026-09-26-catalog-open-drop-reproduction.md`). No Drop reached from the replaced dataset can block on or reacquire the catalog lock; filed as `catalog-open-replace-drop-latency-note`, proposed.
  - (a), (b), (c), (e) and (f) are proposed nodes, each depending on the reproduction: `timing-tests-assert-property-not-budget`, `verify-mutation-test-temp-dirs`, `extent-degenerate-zoom-doc`, `verify-quotes-show-cites-narrowed` and `adr-023-s2-widenings-adr-021-consult`. (a)'s summary carries the slice.rs rule; (f)'s ADR text stays the human's.
- *Item 2 — the five process proposals:* **"(1), delivered as one governance docs piece. (b)'s gate reports each name the commit they reviewed, so their advisory cites still resolve against something fixed. This batch is this week's allowance under the record cap"** Applied: PLAN node `governance-weekly-proposals-2026-09-26`, ready, one governance docs piece for (a) to (e) under the usual gates. Its (b) carries the reviewed-commit rule. The next weekly window falls due 2026-10-02.
- *Item 3 — B1's build:* **"After the watcher merges"** Applied: `b1-engine-kernel-half` already depends on `engine-source-change-watcher`. Its summary now records that the build starts after the watcher merges, on skp/0.5 directly.
- *Item 4 — the published v0.1.0 bundles (red line):* **"(1): the KNOWN-LIMITATIONS line plus the next release's notes, no GitHub advisory; the defect gives a crafted bundle's author nothing beyond what they already control. The line states the general truth as well as the specific one: a bundle carries its author's viewer code, so opening a bundle means running that author's code in your browser, sandboxed as any website is; open bundles only from sources you trust. Then the specific limit: bundles published before the fix keep a viewer that can stall on crafted offsets. Final wording is sighted at the PR"** Applied: no GitHub advisory. The viewer piece's KNOWN-LIMITATIONS line is reworded to state the general truth, then the specific limit, and recorded as an amendment to its preregistration. The final wording goes to the human at the PR. The next release's notes carry the same line.

**RULED 2026-09-26 — question round 24 (AskUserQuestion, one set, mirrored to Telegram first as `state/questions/round-24.md`; wave 1's after-wave batch; items 1 and 3 typed, items 2 and 4 by option label; each verbatim):**

- *Item 1 — which S1 candidates become cuts (typed):* **"(1): all three are cuts. (b) runs now, in parallel with the watcher (disjoint files, full gate: it restores ADR-020's accepted posture). (c) follows (b), fixed with a time bound plus a declared count ceiling, mirroring the kernel's registry; not dropping at the terminal, which would break answering a late cancel. (a) is independent and carries a KNOWN-LIMITATIONS line for bundles already published"** Applied, as three PLAN nodes:
  - `data-plane-origin-non-ascii` (A1-1): ready now, in parallel with the watcher, under full gating (AUTONOMY §21a). Its own preregistration comes before any code.
  - `data-plane-stream-registry-bound` (A5-1): blocked on the first node. The fix is a time bound plus a declared count ceiling, mirroring the kernel's `StreamRegistry`. It does not drop an entry at its terminal, so a late cancel is still answered.
  - `bundle-viewer-partition-offset-bounds` (A3 3(b)): ready and independent. It carries a KNOWN-LIMITATIONS line for bundles already published, and A3 3(a)'s S2 rides along.
- *Item 2 — A2-1's severity on record:* **"S1, fold-in stands"** Applied: A2-1 is S1 on record. Its fix stays folded into the watcher (Amendment 4), gated with it; no separate cut.
- *Item 3 — B's licences and PR #124 (typed):* **"(1). Hold #124. The follow-up tracks the corpus's generator scripts, exact commands and full hashes from the local corpus of record, with no data, whatever the licence outcome. That's the single point of failure B found. Licences are researched at primary sources and the table comes to me before merge; any file without an identifiable open licence stays local-only, listed by name, and never enters the reproducible set"** Applied:
  - #124 is held unmerged.
  - PLAN node `corpus-reproducibility-record` tracks the corpus's generator scripts, exact commands and full hashes from the local corpus of record, with no data, whatever the licence outcome.
  - Its licence table comes to the human before any merge. A file without an identifiable open licence stays local-only, listed by name, outside the reproducible set.
- *Item 4 — PR #123's sequencing:* **"Hold until the watcher"** Applied: #123 is held until the watcher merges. The custodian then merges main into `cloud/wave1-C`, updates its fixtures to skp/0.5, re-runs its harness, records the counts, and brings it back for the human's click. The watcher node's summary carries this.

**RULED 2026-09-25 — question round 23 (AskUserQuestion, one set, mirrored to Telegram first as `state/questions/round-23.md`; items 1 and 3 by option label, items 2 and 4 typed; each verbatim; item 2 a red line):**

- *Item 1 — wave 1's next launches:* **"Pairs as planned (Recommended)"** Applied: batch 1 runs as two pairs, A1+A2 and then A4+A5, with the balance read before and after each pair; then C+D; then B alone. Each launches from claude.ai/code, its prompt hash-checked before sending.
- *Item 2 — S1, the unheld session reference and skp/0.3's rule (red line):* **"Yes. A kernel-minted reference that no client holds does not engage skp/0.3's no-generation-value rule, under round 21's riders: it carries no ticket attribution, and it is never persisted or published. Added rider: the second dated note to SKP-V0 §8 states that this reference is containment for the close race, not a protocol feature. Once kernel-generation-close-races lands, the unheld path must become unreachable, and that piece adds a test proving it. Recorded in the watcher piece"** Applied:
  - The watcher piece appends a second dated note to SKP-V0 §8. It records that the unheld reference does not engage the rule under round 21's riders, and that it is containment for the close race, not a protocol feature.
  - The watcher's preregistration gains an amendment recording the ruling.
  - `kernel-generation-close-races` must make the unheld path unreachable and adds a test proving it; its PLAN summary carries this.
- *Item 3 — S2–S4:* **"All three as rec. (Recommended)"** Applied: S2 (the unheld reference uses `open_dataset`'s mint source and is never reused), S3 (the dropped-event log line carries no reference) and S4 (SKP-V0 §3's minting rule worded to cover a reference no client holds) are recorded on the watcher's preregistration.
- *Item 4 — the test-claims follow-ups form's O1–O9 (typed; verbatim, line by line):*
  > (1) with two amendments.
  > O1: no in-place edit of a preregistration, not even two bytes. My round-22 condition said "correct or reduce", and it did not license editing append-only records; round 15 (f) bounds in-place restores to unmerged branches. Instead, distinguish use from mention: the scanner treats a marker as a withdrawal attempt only in its row position, so a heading or clause that merely mentions the token is not an attempt. If that can't be done cleanly, exempt exactly these two historical lines by hash pin, append-never, the baseline-ratchet pattern.
  > O3: pin-condition failures on accepted rows are named too, in the same finding shape. A withdrawal whose pin does not hold is not valid, which is within rider (a)'s intent. I'm widening the ruling explicitly, so the piece isn't exceeding it.
  > O2 and O4–O9 as recommended

  Applied:
  - No landed preregistration is edited.
  - The architect revises the form to the ruling:
    - O1 by use versus mention: the marker is a withdrawal attempt only in its row position. The fallback is exactly the two historical lines exempted by hash pin, append-never (the baseline-ratchet pattern).
    - O3 names pin-condition failures on accepted rows in the same finding shape, the human's explicit widening.
    - O2 and O4–O9 as recommended.
  - The revised form is committed before any code.

**RULED 2026-09-25 — question round 22 (AskUserQuestion, one set, mirrored to Telegram first as `state/questions/round-22.md`; items 1 and 3 typed, item 2 by option label; each verbatim; item 1 a red line):**

- *Item 1 — the watcher's stop item X1, the event payload for a generation with no session reference (red line):* **"The architect's recommendation, as an invariant: every generation carries a kernel-minted SessionRef, and a generation not created by open_dataset gets one no client holds. Its end still emits, and the shell drops an event for an unknown session with a logged line, never silently. Recorded as an appended, dated note to ADR-035's Decision 4 after #119 merges. The close race is not deferred indefinitely: kernel-generation-close-races is placed immediately after the watcher, and it takes in whatever wave-1 A2 reports about it. A generation minted for a closing dataset is the defect; the unheld reference only makes it harmless meanwhile."** Applied:
  - The watcher's preregistration (`engine/SOURCE-WATCHER-PREREGISTRATION.md`, on `cut/source-change-watcher`) gains Amendment 1: every generation carries a kernel-minted `SessionRef`; one not created by `open_dataset` gets a reference no client holds; its end emits; the shell's listener drops an event for an unknown session with a logged line that carries no reference (round 21 item 1, rider (b)).
  - After #119 merges: one fully gated docs piece appends the dated note to ADR-035's Decision 4 (PLAN node `adr-035-decision-4-note`).
  - PLAN node `kernel-generation-close-races` is placed immediately after the watcher. It carries the ADR-035 drafter's two kernel notes and takes in whatever wave-1 A2 reports about the close race.
- *Item 2 — B1's preregistration, OPEN markers O1–O8:* **"All eight as rec. (Recommended)"** Applied: B1's preregistration (PR #121) gains Amendment 1, resolving O1–O8 to the recommendations in `state/consults/2026-09-25-b1-prereg-revision.md` §3. O7's node, `b1-shell-half`, is added as proposed.
- *Item 3 — the withdrawn marker's refused-line residual:* **"(1). Rider (a) governs every line that carries the withdrawn marker, not only accepted ones: a malformed or unresolvable withdrawal attempt fails by name with the grammar's reason. Condition: the follow-ups PR also corrects or reduces every such line already on main, so verify never reddens main when it merges."** Applied: `governance-test-claims-superseded-followups` carries the ruling and its condition, and is placed ready at order 1 in the governance lane, before `test-claims-landedness-bound` (order 2).

**RULED 2026-09-25 — the cloud-hook finding, with the human's baseline correction (the human's messages mid-session, verbatim in `state/directives/2026-09-25-cloud-hooks.md` §2 and §5; cited as "RULED 2026-09-25, the cloud hooks" with its item numbers; the ruling lines and the correction byte-copied here by script):**

- *Item (1):* **"(1) Hooks become custodian-only by construction: every hook in .claude/settings.json exits 0 immediately, with no output, when it detects a cloud session. Use the environment marker the cloud-environments documentation names; quote it, don't assume it. Land it on main before any launch as one small reviewer-gated PR, with a dry-run test per hook: marker set → exit 0 and no output; marker unset → behaviour unchanged. That includes the Notification/Telegram hook (no token in the cloud → a silent no-op, never an error)."**
- *Item (2):* **"(2) Amend every wave-1 prompt: replace "git checkout <SHA>" with git worktree add /tmp/wave1-baseline 59406134a447d6187fae4a6a79fb9f390c8efefb. All reading, building, reproducers and branches happen inside that worktree, and the session's own checkout stays on main so the fixed hooks stay in force. Record the amendment in state/cloud/wave1-prompts.md as a dated deviation from the original text, with this reason."**
- *Item (3):* **"(3) The baseline stays 59406134: the hook PR touches .claude/ and scripts/hooks/ only, which no audit examines. Launch the A3 calibration only after (1) has merged and its dry run has passed."**
- *The correction, beside the ruling (§5):* **"(1) The baseline is bb98f71f43a2891d317b10a124387df9d5ee0ebf, your tracked re-pin, correctly made under the handoff rule. My hook ruling's 59406134 is superseded: every amended prompt uses git worktree add /tmp/wave1-baseline bb98f71f43a2891d317b10a124387df9d5ee0ebf. Record the correction beside the ruling."**
- *The wave-1 block (§4), confirmed (§5):* **"(2) The pasted wave-1 block is my instruction, relayed from Fable. Apply whatever in it isn't already done: skip anything already applied, and say which."**

Applied:
- (1) One single-gate piece (`AUTONOMY.md` §21b; `PRECEDENTS.md` P-005), branch `governance/hooks-cloud-inert`: the four scripts `.claude/settings.json` runs as hooks exit 0 at entry, with no output, when `CLAUDE_CODE_REMOTE` is `true` — the marker the cloud-environments documentation names for scoping hooks (quoted in the piece's five-line form, committed before code). One dry-run test per hook, marker set and unset. Reviewer-gated; merged by the human's click.
- (2) and (3), as corrected: the baseline stays bb98f71f43a2891d317b10a124387df9d5ee0ebf (the handoff's tracked re-pin); the ruling's 59406134 is superseded. Every amended prompt uses `git worktree add /tmp/wave1-baseline bb98f71f43a2891d317b10a124387df9d5ee0ebf`, recorded in `state/cloud/wave1-prompts.md` as a dated deviation with the ruling's reason. A3 launches only after (1) has merged and its dry run has passed.
- The wave-1 block (§4): its triage rules govern wave 1's reports. Already applied before it arrived, so skipped: round 21's two rulings (#118 merged at 0a03c0e; ADR-035's acceptance is PR #119, ready for the click). Applied after the hook PR, in order: B1's sighted preregistration committed; the two design references copied to `state/drafts/design/` under a design-reference header; the watcher's preregistration and implementation, fully gated. The skp/0.4 fixtures note is the custodian's integration task if the watcher moves the literal first.
...
- *Item 2 — entry 122, the watcher's S1 and S2 (typed):* **"Option (1), with riders: (a) dataset_session_ended is advisory delivery, never the authority. The generation's end is decided server-side, and every later call still refuses by name, so a lost or late event cannot leave a client acting on an ended generation. (b) The event carries only the session reference and the typed reason (observed change vs coverage lost), is non-authorising, and never carries feature data or a handle. Its ADR is filed Proposed for my acceptance. S2: one grandparent watch as recommended; ancestors above it get a KNOWN-LIMITATIONS line."** Applied: S1 becomes one SKP control-plane event, `dataset_session_ended`, on the watcher's literal, and riders (a) and (b) bind its preregistration. Its ADR is filed Proposed, carrying the riders, before the preregistration is committed; its acceptance is the human's. S2 is one grandparent watch, and ancestors above it get a KNOWN-LIMITATIONS line. The preregistration is committed on a branch cut from `cut/crs-unit-fact-and-bounds`, per RULED 2026-09-24 (night) item (2). Node `engine-source-change-watcher` depends on `crs-unit-fact-and-bounds`.
- *Item 3 — entry 123, B1's stop items 1 and 2, a red line (typed; the answer field carried this paragraph twice, identically, and it is recorded once):* **"Item 1: ADR-017 §4 stays byte-identical. Publishing enforces its type list as a named bundle-format restriction at the publish site, refusing Float32 and dictionary columns at preflight with byte-identical text, until B3's bundle_version-2 ADR decides. This is not a second admission policy: admission ("is this type meaningful") stays one function; the bundle check ("can v1 carry it") is format-owned. ADR-023's still-Proposed Decision text gains one clarifying sentence saying so. Item 2: Float32 becomes filterable, recorded by a dated note appended to ADR-021: its namespace widens by reference to ADR-023, and that widening is stated, not implicit. Dictionary-encoded columns are excluded from the filter namespace by name, using the existing filter_column_not_filterable{column, reason} code. Their ordered comparison follows declared position (H4), which would make < and > mean something no user expects from a string."** Applied:
  - ADR-017 is not edited.
  - The ADR-021 dated note and ADR-023's clarifying sentence go into one docs PR for the click, drafted by the architect from this ruling and gated. ADR-021's status line and ADR-023's status line do not change.
  - B1's preregistration states the publish-site restriction, the Float32 filterability and the dictionary exclusion as ruled here.
- *Item 4 — entry 123, B1's stop items 3-8 and the sight:* **"Sighted, 3-8 as rec. (Recommended)"** Applied: the draft is revised to the item 3 ruling and committed as `b1-engine-kernel-half`'s preregistration, and B1's literal is the one after the watcher's.
- *Item 5 — entry 124, crs-unit's Q1-Q4:* **"All four as rec. (Recommended)"** Applied:
  - Q1: today's one-unit values stay for `other` and `unestablished`, and `other`'s own values are declared when such a dataset enters a slice corpus.
  - Q2: a follow-on node, `crs-zoom-constants-per-unit`.
  - Q3: its own node, `interactive-zoom-ceiling`, which declares a ceiling under ADR-010 rule 6.
  - Q4: the degree drift cap is 2^17 degree, as declared.
- *Item 6 — entry 121, the fixture's post-write watchdog:* **"Separate entry point (Recommended)"** Applied: node `fixture-regeneration-entry-point`, a bounded light-lane piece: a regeneration entry point outside the measurement harness, with byte-identical output. `kernel/SCALE-PASS-PREREGISTRATION.md` is not amended. `drill-fix-fixture-watchdog` is done at PR #105, which carried its first half.
- *Item 7 — entry 126, MultiPolygon's eight decisions (typed):* **"All eight as recommended, and decision 4's wording sighted as proposed. Rider on decision 3: a column with an empty geometry_types admits under the MultiPolygon encoding, but any row of a type the engine does not read is refused by name or counted as unread and shown, never silently dropped. Decision 1's promotion of Polygon rows is declared as an encoding in describe (decision 6's declared_types), never presented as the file's own type."** Applied: `geometry-types-beyond-polygons` becomes MP-1, placed after crs-unit, the watcher and B1, and carries the two riders. ADR-034, geometry admission and encoding selection, is filed Proposed before MP-1's preregistration, carrying the riders. Publishing rides B3's bundle v2, and LOD stays out.
- *Item 8 — entry 125, test-claims-landedness-bound:* **"After the scanner (Recommended)"** Applied: the node depends on a new proposed node, `governance-test-claims-superseded-scanner` (`governance/test-claims-superseded` @ 3100766, still held, scheduled at the weekly window). Design (c) follows once the scanner has retired P3b's three mentions.
- *Item 9 — entry 127, PR #108 and PR #110:* **"#108 only; drop #110"** Applied: `governance-verify-mutation-multiline-attrs` continues under a fresh count with the narrowed fix. An unclosed attribute falls back to single-line handling after a bounded look-ahead, and the unclosed case is printed as a finding. PR #110 is closed unmerged with this ruling cited, and its branch is kept. `governance-verify-mutation-header-token` becomes unscheduled, so the whole-file window stays.

**RULED 2026-09-24 (night) — the program's order governs; protocol literals follow merge order (the human's message opening the night session, verbatim in `state/directives/2026-09-24-program-order-and-literals.md`; cited as "RULED 2026-09-24 (night)" with its item numbers; the two ruling sentences byte-copied here):**

- *(1):* **"(1) while a recorded program exists, the program's order governs; the queue's lane-priority display and the Stop hook's "next" are advisory; no generator change."**
- *(2):* **"(2) Protocol literals follow merge order (the C2 precedent): the watcher takes the literal after crs-unit-fact-and-bounds, B1 the one after, and B1's draft states this rather than leaving it open; the watcher branch stacks on the unit piece's branch because both touch describe."**

Applied:
- (1) The night program (DIRECTIVE 2026-09-24) runs in its recorded order; the queue's "Next" and the Stop hook's `next:` are read as advisory while it exists. The morning question on a generator feature for a cross-lane program order is answered by "no generator change" and leaves the round.
- (2) The wire order is crs-unit-fact-and-bounds, then the watcher, then B1. `b1-engine-prereg-draft`'s summary and its draft state B1's literal as the one after the watcher's. The watcher's branch is cut from the unit piece's branch, not from main. The morning question on B1's literal leaves the round.

**RULED 2026-09-24 — the watcher sight, and its place tonight (the human's two messages, verbatim in `state/directives/2026-09-24-watcher-sight.md`; cited as "RULED 2026-09-24, the watcher sight"):** The brief `state/briefs/source-change-watcher-brief.md` is accepted as the record of the settled direction, and the seven additions are binding on its preregistration. Addition 1 corrects entry 119 item (3)'s value list: describe carries two independent facts, coverage and checks, and ADR-016 A1 item 2's owed display is delivered by checks. Applied:
- `engine-source-change-watcher` is sighted and ready as night program item 4b, after item 4: the preregistration, the architect consult and the implementation on a branch under both gates, no merge. A preregistration choice beyond the seven points stops the piece and queues it.
- The brief gains one sighting line at its end.
- The ADR-016 acceptance note in PR #103 is aligned to the checks fact.
- Wire order: the watcher's describe fields take the SKP literal after crs-unit-fact-and-bounds. Where B1's literal falls relative to the watcher's is not yet ruled; `b1-engine-prereg-draft` states it as open for the morning round.

**DIRECTIVE 2026-09-24 — the night program and the night rules (the human, before the handoff; verbatim in `state/directives/2026-09-24-night-program.md`):** Applied:
- Entry 120 was already applied: 8704535 and PR #103.
- The eight program items are recorded in PLAN in the program's order, as "Night program 2026-09-24, item N" in each node's summary, and in the continuity block's intended sequencing. Item 1 is PR #103, prepared with CI green and waiting on the click.
- Work-order dependencies became merge-order notes: PR #97 merges after PR #103; the drill fixes after PR #97; crs-unit-fact-and-bounds after PR #103 and before B1.
- The five tool fixes are placed and ready.
- New nodes: `known-limitations-owed-rows`, `b1-engine-prereg-draft` and `multipolygon-assessment-consult`.
- The queue's ready listing sorts by the lane priorities ruled 2026-09-13, so it cannot show the program's cross-lane order. The program order governs the night.

**RULED 2026-09-23 (later) — entry 120, ADR-013's item 6 and item 5's disclosure (the human's message, verbatim in `state/directives/2026-09-23-entry-120.md`; cited as "RULED 2026-09-23 (later)" with its item numbers):**

- *(1):* ADR-013 Amendment 1 is accepted in the appending PR, with a dated acceptance note naming item 6 as owed. The item is scheduled, not parked. Applied:
  - (a) a KNOWN-LIMITATIONS line added now, in a section for main's build after v0.1.0;
  - (b) the node `crs-unit-fact-and-bounds` in the kernel-protocol lane, placed first after the acceptance PR and ahead of `b1-engine-kernel-half`, which now depends on it. It covers a typed CRS unit fact on describe (a wire change under full gating, taking the next SKP literal, B1 following), the declared per-unit values for the anchor span, and the architect-declared degrees value for the drift bound, both pinned by tests;
  - (c) its P0 opens with the one arithmetic bound, and the KNOWN-LIMITATIONS line is then revised to state it.
- *(2):* recorded as a known limitation, in the same section, with no piece now. Its migration requirement is added to `shell-redesign-map-studio`.
- Then one appending PR carries all five acceptances.

**RULED 2026-09-23 (late) — entry 119, the five acceptances; and the shell direction, "Map studio" (the human's message, typed, red line, verbatim in `state/directives/2026-09-23-entry-119-and-map-studio.md`; cited as "RULED 2026-09-23 (late)" with its item numbers):**

- *Entry 119:* ADR-015 Amendment 1 (1) and ADR-032 (2) are accepted as they stand. ADR-016 Amendment 1 (3) is accepted with a dated acceptance note naming item 2's degradation display as owed; the watcher piece's detection-state field on describe delivers it. ADR-013 Amendment 1 (5): the short-form piece comes first, then N1 is re-sighted, and it is accepted once item 5 is true of the build. ADR-028 Amendment 4 (4): the short N8 rerun on the merged build, in the same sitting as N1's re-sight. Then one PR appends all five for the click. Applied: new node `adr-013-display-statement`, whose five-line form is committed before code. `briefa-p6-part-n-acceptances` (the sitting: N1 re-sight and N8 rerun) depends on that node, and the five accept nodes depend on the sitting and land as one PR. `engine-source-change-watcher` carries the detection-state field as ADR-016's owed display.
- *Shell direction, items (a)–(e):* (a) the proposed node `shell-redesign-map-studio` is added in the shell lane, citing this block; no structural shell work is authorised. (b) B1's shell half, B2's screens and the watcher's status display are held for the new structure, as notes on their nodes; B1's engine/kernel half proceeds as sequenced. (c) and (d) bind every piece: a new display goes into an existing component and is listed in the ledger as a migration-inventory item. (e) PR #97 and the drill repairs proceed as ruled.

**RULED 2026-09-23 — the N8 residual, the enforced sequence, effort variants (the human's message, verbatim in `state/directives/2026-09-23-n8-residual-and-sequence.md`; the three ruling sentences byte-copied here):**

> My decision on the residual: fix the old filter Apply contamination and the three named baseline callbacks before my final N8 retest. Use a bounded correction under the existing gates; no unrelated redesign.
>
> Brief A close → PR #97 → the three clean-clone repairs → B1 → MultiPolygon assessment/cut → B2 → B3 → Part O.
>
> I approve the minimum project-local worker/tester variants needed if the installed version cannot select effort per dispatch. Avoid duplicated instruction corpora, silent model overrides or broader tools. Do not apply omitClaudeMd to implementation, testing, review or design.

- *The residual:* Applied: node `decision-n8-residual-late-writes` done with this ruling as evidence; the fix is owed on `briefa-p6-n8-reopen-reset` (PR #100) as a bounded correction under full gating, before the final N8 retest; the full sequence (old generation ended, successful reopen, then old success, error and filter completion) exercised through the real App wiring, with positive controls.
- *PR #100's branch:* Applied: current main merged into `fix/n8-reopen-reset` with a merge commit (no force push); branch-run and merge-candidate CI reported separately.
- *The sequence:* Applied in PLAN.yaml: `shell-dependency-audit-piece` depends on the five acceptance nodes; `b1-engine-kernel-half` depends on the three clean-clone repair nodes; the rest of the chain already held. `release-v0-1-1` stays off B1's path.
- *PR #97:* Applied: its title and body distinguish the decisions ruled on 2026-09-20 from the implementation still owed; it stays draft; its historical evidence is kept.
- *Effort routing:* Applied at a clean checkpoint (no agent running): `worker` pinned medium; `tester` already medium; new `worker-high` and `tester-high` carry effort high with the same model pin and tools, their bodies a pointer to the base definition rather than a copy; no omitClaudeMd on them.

**RULED 2026-09-22 — Part N run 1, the N8 correction piece, four rulings, the efficiency pilot and the sequencing direction (the human's three messages, verbatim in `state/directives/2026-09-22-part-n-n8-and-sequencing.md`; the four rulings verbatim here):**
...
- *Part N run 1:* Applied: recorded in `frontends/shell/MANUAL-WALKTHROUGH.md`, Part N run — N1–N7 PASS as reported with the human's wording verdicts, N8 deviated at reopen (its remainder unverified), N9 as given; P6 open, no ADR accepted.
- *The N8 piece:* Applied: node `briefa-p6-n8-reopen-reset`, its own branch and worktree, the five-line form committed before code, full gating (reviewer and architect — the owner-side session-ended state is a property under test, AUTONOMY.md section 21a; the round-16 item-1 shape keeps the five-line form); P6 depends on it; a focused N8 retest follows machine verification.
- *The watcher:* Applied: `engine-source-change-watcher` placed, blocked on Brief A's close and the N8 piece (it reuses the corrected reopen lifecycle; no automatic reload, no stronger snapshot claim).
- *B2's Prepare selection:* Applied: on `briefb-b2-save-reopen` for its preregistration; the Brief B draft's old B2 wording that equated Save with preparation reconciled in place with the 2026-09-18 two-verb direction.
- *Generated files:* Applied: the current regeneration procedure (AUTONOMY.md section 23) stays for this cut; a bounded governance node `governance-ci-built-site` (the CI-built site, its design also addressing tracked-queue conflicts) proposed; `site-design-v1` waits behind it.
- *The dashboard:* Applied: the ruling nodes for entries 116 and the generated outputs, the watcher and Prepare-selection rulings, and the LOD lifecycle node flipped to done with the ruling as evidence; the owed work sits on its own nodes (`shell-dependency-audit-piece`, the ADR-031 text with `lod-tier-selection`), so the waiting-on-human list holds only decisions still the human's.
- *The sequencing direction (message 3):* Applied as one bounded reconciliation of PLAN.yaml: geometry after Brief A's actual close and after B1 (the human's order), MultiPolygon first by an architect assessment, points and lines as their own proposed cuts; B3 after B2, Part O after B3; the scan-progress route returns at B2's preregistration; release-v0-1-1 names its prerequisites; the three clean-clone repairs as three bounded nodes after PR #97. The efficiency pilot (message 2) at the next clean checkpoint, not a prerequisite for N8.

**RULED 2026-09-20 — the sitting day: entries 116, 117, 118 and the LOD tier-cache lifecycle (entry 99's precondition) (the human's message, typed, verbatim; also `state/directives/2026-09-20-sitting-day.md`):**
...
- *Entry 116:* Applied: PR #97 lands as ONE dependency piece under the full gate (reviewer + architect), deferred by the same ruling until the sitting and the acceptances are done — the three major bumps approved (`vitest` ^5.0.1, `vite-node` ^6.0.0, `esbuild` ^0.28.2); the piece's steps: the two typed helper parameters at `frontends/shell/src/residency/candidateArmSession.test.ts` (attempt-1 lines 2270 and 3334) take the callback's own type; suite and build green; `src/generated/NOTICE.txt` regenerated and diffed against main; the audit script gains two dated package-scoped `PACKAGE_DECISIONS` entries — `lightningcss` 1.33.0 and `lightningcss-win32-x64-msvc` 1.33.0, tree `frontends/shell`, MPL-2.0, reason: build-time-only, not in any shipped set; a fresh decision is required if either ever enters a shipped set — and (entry 117) `caniuse-lite` 1.0.30001809 CC-BY-4.0 on the same terms; `DEPENDENCY-LICENSES.md` regenerated; the lockfile diff read in the PR. `PRECEDENTS.md` P-033 gains the boundary rider: reachability, not npm's severity, decides urgency. No worker dispatched today.
- *Entry 117:* Applied: (i) the hybrid report stays as it is, documented in its own header and preserved by the script; (ii) `caniuse-lite` approved — the dated decision rides PR #97's piece (above), so the report's review row clears when that lands.
- *Entry 118:* Applied: (1) the 200k launch-scoped flag is the human's at the next launch, a one-week pilot, the saved 800k untouched; (2) the `startup`/`clear` matchers are on main since bd2fc06; (3) the human pastes `/context` at checkpoints and the custodian records the figure with its timestamp in the block's `next` line; the status-line wiring proposal stands unapplied.
- *The LOD tier-cache lifecycle (entry 99, round 8's owed decision):* Applied: recorded on PLAN node `lod-tier-cache-lifecycle` as the human's pre-commitment for the tier-selection preregistration's sight — (a) a total cache ceiling relative to free disk at its own site; (b) delete-on-supersede, LRU only as fallback; (c) the runner is the prepare machinery: the operation lifecycle as scheduler, background priority, the prepare token as cancellation owner; (d) the operator view is tier count, size and state in describe and in the status stack. The ADR-031 amendment text carrying it is a gated piece, deferred with the rest; `lod-tier-selection` stays blocked on it.
- *The day's order:* Applied: prep done (below, in the ledger); hold with no worker dispatch during the sitting; rows recorded verbatim into the Result log as given, a deviation stopping its row; after the sitting the five acceptances as ONE question set with their evidencing rows; then #97's gate and the PLAN flip; the drill fixes and the scanner's weekly-window question deferred; a partial sitting recorded as partial.

**RULED 2026-09-20 — entry 118 and the state reconciliation (the human's second message of 2026-09-20, typed, verbatim; the first message's priorities are recorded verbatim in `state/directives/2026-09-20-reconciliation-and-continuity.md`):**

> 2) Approve the one-week launch-scoped 200k compaction pilot, leaving the global 800k setting unchanged. This is a fallback, not a mandatory clear threshold.
> Approve adding startup and clear to the existing SessionStart hook, with focused verification that the continuity block arrives before work begins. Preserve the existing lease rules; explicitly complete the outgoing/incoming handover rather than assuming context injection transfers ownership.
> Before clearing, make one bounded correction to the current-state block:
> Remove completed work from “intended sequencing.”
> Remove remaining references to #99 awaiting a click or cleanup.
> Reconcile “everything waits on the human” with the queue’s two ready items: explain any authority or sequencing hold without starting them.
> Make the revision/freshness fields satisfy the existing hook’s actual checks.
> Edit the current summary in place under its existing rules—no amendment campaign or new governance.
> For context measurement, verify the documented status-line fields on the installed version. Prefer a project-scoped, timestamped machine-readable reading that the custodian can inspect; missing data means unknown, not zero. Propose the minimal wiring before changing any user-global configuration.
> Keep the checkpoint controller deferred. For the pilot, label agent-notification token counts by their actual meaning; include custodian overhead where available. Treat comparison with four different historical pieces as directional evidence, not proof of a savings percentage.
> After the corrected handoff and lease handover, stop for my fresh-session action. Do not dispatch another piece during this transition.
> The untracked .codex-remote-attachments directory contains my Codex attachment; leave it untouched

- *The 200k pilot:* Applied: approved as a launch-scoped fallback for one week from the next launch — the session is started with `--autocompact 200000` (or `CLAUDE_CODE_AUTO_COMPACT_WINDOW=200000` in the launch command); the saved global `autoCompactWindow` (800000) untouched; the custodian records the launch flag in the ledger's first flush of that session; not a clear threshold.
- *The SessionStart hook:* Applied: `.claude/settings.json` gains the `startup` and `clear` matchers on the existing `session-resume.mjs` entry (four matchers now); verified before commit by piping startup- and clear-shaped SessionStart input to the script — it prints §0's reading order and the whole SESSION-CONTINUITY block (21 lines, the `next` line included) with rc 0; `scripts/hooks/hooks.test.mjs` green; `AUTONOMY.md` §7's one descriptive sentence corrected in place (same line, line count 470 unchanged, the checker's `:357` pin unchanged). The lease rules are unchanged: the block informs the incoming session; ownership transfers only by the relinquish-then-verify handover of `AI_DEVELOPMENT.md`, completed by the outgoing session at the end of this turn.
- *The bounded correction to the block:* Applied in place — completed work removed from `intended sequencing`; no reference to #99 awaiting anything remains; the queue's two ready items explained with their holds; `flushed_at`/`tip` written by a final ledger-only flush commit whose `tip` names its parent, within the hook's 10-minute window, pushed, porcelain clean of modified tracked files.
- *Context measurement:* Applied as a proposal, no user-global change: the installed 2.1.272's status-line input documents `context_window.context_window_size`, `total_input_tokens`, `total_output_tokens`, `used_percentage`, `remaining_percentage` and `current_usage` (the last three may be null early in a session and after a compaction until the next API call — null is recorded as unknown, never zero); `statusLine` may live in the project `.claude/settings.json` (documented scope: any file; precedence local > project > user); it runs at session start and after each assistant message, debounced. Minimal wiring, for your word: a project script `scripts/hooks/context-reading.mjs` as the project-scoped `statusLine` command, writing `.claude/state/context-<session_id>.json` (`read_at`, the six fields, nulls kept) and printing a one-line status; the custodian reads that file at each flush and records the figure with its timestamp. The `statusLine` key is a settings change beyond the hooks approved here, so it is not applied.
- *The controller, the pilot's labels:* Applied: the checkpoint controller stays deferred (weekly window). Pilot labels: the `subagent_tokens` figure in a task notification is the harness's per-agent total and its composition (fresh input, cache reads, output) is not documented — recorded as such; custodian overhead is the measured context figure when one exists, else unknown; the 2026-09-19 pieces are directional comparators only.
- *The stop:* Applied: after this reconciliation commit and the final flush, the outgoing session relinquishes the lease and stops; nothing dispatched. `.codex-remote-attachments/` untouched and unstaged.

**RULED 2026-09-18 — question round 16 (AskUserQuestion, mirrored to Telegram first as `state/questions/round-16.md`; items 1–4 answered by option label in the first call, item 5 by a typed restatement of all five in the second; both verbatim):**

- *The typed word, verbatim (the second call's answer field):* **"1 = (1) render before Part N. 2 = (2) the architect reduces the README to the verified facts; the spike lands with the reduced record. 3 = (4) defer — ADR-029 stays Proposed, G1's facts recorded, the route returns with Brief B's B2. 4 = (2) hold the scanner; P3b's three findings stay advisory-planned; the scanner's scheduling goes to the weekly window. 5 = (2) HOVER-REPICK:53 tool-false-trigger with its reason corrected; the A9′ pair under the POLISH-113 precedent, record closed."**
- *Item 1 — entry 112, the unrendered P6 sight:* **"Render before Part N (Recommended)"** Applied: a five-line shell piece (`cut/briefa-p6-describe-fields`, `AUTONOMY.md` §21d; the form committed before any code) renders `identity.session_statement` (only when non-null), `crs.provenance` and `crs.axis_provenance` verbatim in the describe summary, appended below the existing lines so no closed record's line cite into those files moves; rendering is new user-visible behaviour, which crosses §21c's bound, so the architect reads alongside the reviewer with the five-line form kept (§21b's mid-piece clause, the test-names shape); Part N's N3 then gains one row on `docs/part-n-walkthrough` and the sitting runs as written.
- *Item 2 — entry 112, ADR-029 G1 under Rule 7:* **"Architect reduces"** Applied: the architect reduces `spikes/adr-029-g1-scan-progress/README.md` on `spike/adr-029-g1` @ d9428c2 to the facts read 2 verified clean (gate-log 80, 86; the read-2 report), removing or replacing the three refuted sentences and every should-fix item as the reads found them, adding no claim; a fresh worker applies the reduction byte-exact (sha256 of the architect's text against the file) and the spike lands by a PR for the click with the reduced README as the record — no third read; the node is done on merge.
- *Item 3 — entry 112, ADR-029's route:* **"Defer"** Applied: no route dispatched; ADR-029 stays Proposed; G1's facts stand recorded at entry 111 and in the reduced README; PLAN node `decision-adr-029-scan-progress-route` (blocked on G1's landing and `briefb-b2-save-reopen`, a ruling of the human's) carries the three routes G1 established back to the human with B2's preregistration.
- *Item 4 — entry 112, the scanner under Rule 7:* **"Hold the scanner"** Applied: `governance/test-claims-superseded` @ 3100766 held, no PR, no node on main; `briefa-p3b-owner-side-invalidation` stays in-progress with its summary saying why (after PR #90 lands, three historical mentions of an old name in its record stay reported by verify-test-claims as planned — advisory under the not-done rule, never binding); the scanner's scheduling is on the weekly-window list (2026-09-25 at the earliest) with the four other filed proposals.
- *Item 5 — entry 112, the baseline dispositions:* **"5 = (2) HOVER-REPICK:53 tool-false-trigger with its reason corrected; the A9′ pair under the POLISH-113 precedent, record closed."** Applied: on `governance/verify-quotes` (PR #91), a custodian commit, no gate round (the checker's last round has run, round 15 item 3): the `HOVER-REPICK-PREREGISTRATION.md:53` entry becomes `tool-false-trigger` with its reason rewritten to name the now-tracked source `state/drafts/post-tag/architect-consult-lod-47-66b-ordering.md:65-71` (tracked under round 14, item 4) and the one difference — item 14's trailing citation dropped without a mark, not a negation, condition or qualifier; the `OWNER-INVALIDATION-PREREGISTRATION.md:799` and `:968` entries become `unfindable-by-construction` under the POLISH-113 precedent (round 12: a misquote no piece can ever correct — here because the record is closed under the record cap), their `corrected_by` removed, ruling "round 16, item 5"; one class-3 pointer row appended to the checker's record as Amendment 10 naming the edited spans by commit and hash.

**RULED 2026-09-18 — question round 15 (AskUserQuestion, mirrored to Telegram first as `state/questions/round-15.md`; the human's typed answers verbatim):**

- *Item 1 — entry 107, the consult mechanism and six clauses:* **"Adopt all seven (Recommended)"** Applied: the seven clauses (the resolution step; per-row resolution before delivery; the tool-commit clause; the reference form in source comments; the durable-reference clause; the restore bound; class 1 for a withdrawal row) added byte-copied from `state/questions/round-15.md` at 83cad32c93eb (lines 4, 5, 6, 7, 8, 9, 10; each line's sha256 beside it) to `docs/PREREGISTRATION-TEMPLATE.md` §10, `.claude/agents/reviewer.md`, `.claude/agents/architect.md`, `.claude/agents/worker.md` and `AI_DEVELOPMENT.md`; every future consult carries the resolution step as its last numbered step and is resolved per row before delivery.
- *Item 2 — entry 107, the P3b and LOD closes:* **"Hold both"** Applied: nothing dispatched on either record; `cut/briefa-p3b` @ 959bd08 and `engine/lod-tier-builder` @ bd8f1f9 (PR #84) stay held at their heads until the human's word; the LOD reviewer's attempt-6 verdict (gate-log 69: eight stale cross-file cites in Amendments 8 and 11 certified as resolving — a widened claim past the survey behind it) is recorded beside the architect's.
- *Item 3 — entry 107, the checker under Rule 7:* **"Option 1: hold until P3b and #84 merge; then one round under a fresh count — the bounded build fixes as listed (marker reproduction on its own line, a route for hash findings in immutable text, tracked fixture lines with positive assertions, the site.mjs revert, the // continuation, the header gaps), the baseline re-derived against final main, one fresh worker, both gates. Under the standing directive: this round's closing amendment is references only — no prose restating what the build does; it is the checker's last round. If the record fails again, the architect reduces it to references and the build lands with the reduced record; the build has passed both gates twice and its findings on main are correct."** Applied: `governance/verify-quotes` @ fbf3d55 holds until P3b and #84 merge; then one round under a fresh count — the bounded build fixes as listed, the baseline re-derived against the final main, one fresh worker, both gates — whose closing amendment is references only, the checker's last round; a further record failure has the architect reduce the record to references and the build lands with it.
- *Item 4 — entry 107, the acknowledgements and the hygiene ruling:* **"Acknowledged (a)–(d); (b) delete record/residency-125-note on my word. (e): the untracked-Authority rule is dated non-retroactive; the disclosure in the record suffices — with one distinction kept: those e2e reports are evidence (a run's output, cited for what it reported), not Authority (binding text a gate resolves against), and the round-14 rule was written for the latter. The one place it would matter is the mutation-red run T10 rests on; that proof is already carried in the session log and gate reports P3b's amendments cite, so nothing rests on the untracked file alone."** Applied: `record/residency-125-note` deleted on the human's word (branch and worktree); the non-retroactive dating and the evidence-versus-Authority distinction added verbatim beside the untracked-Authority rule in the template, both checklists, the worker brief and `AI_DEVELOPMENT.md`; the `:220` departure, the LOD's four hash-reference findings and the six markers acknowledged as recorded — the ledger note correcting round 14's Applied text on `:220` is owed when the checker lands.

**RULED 2026-09-17 — question round 14 (AskUserQuestion, mirrored to Telegram first as `state/questions/round-14.md`; the human's typed answers verbatim):**

- *Item 1 — entry 106, executing both re-scopes:* **"Option 1. A fresh default-model worker executes each consult's numbered steps exactly, adding nothing; both gates under a fresh count; a FAIL returns to me as a finding about the mechanism. The LOD re-scope's three decisions accepted as stated: the pointer stays with its shift recorded; its home is the closing amendment's head and foot; the two test-text spans corrected under line-count neutrality in the same commit"** Applied: two fresh workers on the configured default dispatched, one per record, each executing its consult's numbered steps exactly (the P3b consult at `state/consults/2026-09-17-p3b-re-scope.md`; the LOD consult from the custodian's scratchpad by absolute path, to be committed under `state/consults/` when #84 lands), the only addition a second `Co-Authored-By` trailer naming the worker's observed model; then reviewer + architect per record under a fresh count; a FAIL there returns to the human as a finding about the mechanism, never to a worker.
- *Item 2 — entry 106, the three clauses, the restore and the test-text exception:* **"Adopt all three as byte-copied, the engine :1371 restore by row, and class 3 with the named exception for test text. Addition to the root-cause rule: a pinned path:line @ commit sha256 reference is a historical pin — when the current tree no longer matches it, the gate resolves it by naming which is authoritative, the pin or the tree; a pin is never silently read as current."** Applied: class 7 appended to the template's class list, and clause (a′) and the root-cause rule added beside round 12's five in `docs/PREREGISTRATION-TEMPLATE.md`, `.claude/agents/reviewer.md`, `.claude/agents/architect.md`, `.claude/agents/worker.md` and `AI_DEVELOPMENT.md` — all three byte-copied by script from `state/consults/2026-09-17-p3b-re-scope.md` (lines 204, 206, 208 at 1a0251461aa2, each with its line's sha256) — with the human's addition on pins verbatim beside the rule; the engine `:1371` restore stands as the consults' step; class 3's text gains the named exception for test text (the custodian's wording, marked as such).
- *Item 3 — entry 106, the two baseline dispositions:* **"Accept :220; :125 owed-correction (Recommended)"** Applied: `engine/ADMISSION-PREREGISTRATION.md:220` carries `tool-false-trigger` with its reason naming the tracked source and the checker's own strip (already in the running fix round's brief); `frontends/shell/RESIDENCY-PREREGISTRATION.md:125` becomes `owed-correction` with `corrected_by` naming an appended note in the RESIDENCY preregistration that restates the passage by reference — the running fix round told by message; the note itself a fresh-worker docs commit queued after the current rounds.
- *Item 4 — entry 106, the untracked Authority:* **"Option 1, track it, on three conditions: (a) the custodian's own sweep against the audit classes (entry 49's) before the commit, expecting the same result as mine: nothing private beyond commit-identity emails and C:\Users\Public; (b) each file gains a two-line header naming its status — draft / consult / decision-note / superseded-by — so a stranger reads it as record, not policy; (c) the directory moves to state/drafts/ beside the other tracked record, the twenty citing files updated in the same commit — bare references, not pins, for the ones that are references — so this is the last time anything in the tree cites an untracked file. Standing rule from today: nothing tracked may cite an untracked file as Authority."** Applied: before any commit the custodian sweeps `state/drafts/` against entry 49's audit classes and records the result in the ledger; each file gains a two-line status header (draft / consult / decision-note / superseded-by); the directory moves to `state/drafts/` and every citing file is updated in the same commit as a bare reference — the accepted ADR and the append-only preregistrations that name it included, by this ruling's express authorization — and the standing rule enters the template, both checklists, the worker brief and `AI_DEVELOPMENT.md` verbatim.

**RULED 2026-09-17 — question round 13 (AskUserQuestion, mirrored to Telegram first as `state/questions/round-13.md`; the human's typed answer verbatim):**

- *Item 1 — entry 105, the LOD fix record stopped under Rule 7:* **"Option 1: one final fresh-worker correction under the round-12 mechanism — the mutation's quoted line brought current or referenced by path:line + hash, the panic site labelled with the lines it was observed at, Amendment 12 recording the cite shift by reference within the ceiling, the five should-fixes, a superseded index — then re-read 3. A further record FAIL holds #84 for the architect's re-scope of the record, as for P3b. Not option 3: a proof test's mutation record that quotes a replaced line is an unverifiable proof, not an owed nit."** Applied: a fresh worker on the configured default dispatched into the LOD worktree with a six-line verified handoff for one correction under round 12's mechanism — the proof test's recorded-mutation comment made current or turned into a `path:line` + hash reference, the recorded panic site labelled with the lines it was observed at, `engine/LOD-PREREGISTRATION.md` §10 Amendment 12 recording the shift of Amendment 10's cites by reference within the ceiling, the five should-fixes closed, a superseded index — then the scoped re-read (attempt 5); a further record FAIL holds #84 for the architect's re-scope of the record, no worker before it; option 3 closed for the reason given.

**RULED 2026-09-17 — question round 12 (AskUserQuestion, mirrored to Telegram first as `state/questions/round-12.md`; the human's typed answers verbatim):**

- *Item 1 — entry 104, the record-fidelity mechanism:* **"Adopt all five. Rider on (b): reproduction is the exception — the default is a path:line reference with span hash and no reproduced text; a passage is reproduced only when the sentence cannot be understood without it, by script, marked. Rider on (d): "three sentences" is a ceiling, not a form — a correction that needs fewer uses fewer."** Applied: the five clauses byte-copied by script from `state/questions/round-12.md:3` (sha256 of that line 8c70572216c53b355bd2e89465a7a58f2dc914006c9517f177fd6de6cd5aad83) into `docs/PREREGISTRATION-TEMPLATE.md` §10, `.claude/agents/reviewer.md`, `.claude/agents/architect.md`, `.claude/agents/worker.md` and `AI_DEVELOPMENT.md`, each with both riders verbatim and the hash form stated (sha256 over the cited whole lines as committed, LF bytes; the commit named when the file is not on main); the gates fail by name a line cite into the ledger, an unmarked or unhashed reproduction, a reproduction the sentence did not need, a correction over the ceiling or one restating an earlier amendment's claim, and a correction round without its superseded index; (c) recorded as the custodian's own rule; the checker's resolution of round + item cites owed as a follow-on.
- *Item 2 — entry 104, P3b's second Rule 7 stop:* **"One final fresh-worker append under the mechanism, fresh count: shell Amendment 14 / engine Amendment 11 reproduce no text; every ledger cite to round + item; the two cuts become references; the four should-fixes and two carried items closed in ≤3 sentences each; then gates. If this round fails on the record again, P3b holds and the record, not the code, gets re-scoped by the architect — a shorter amendment set, not another correction. Option 2 is not taken: waiving affirmative PASS on the record that ADR-016's acceptance rests on would make the acceptance rest on nothing."** Applied: a fresh worker on the configured default dispatched into the P3b worktree with a six-line verified handoff for shell Amendment 14 / engine Amendment 11 only — no reproduced text, every ledger cite converted to round + item, the two cuts replaced by references with hashes, the four should-fixes and the two carried items each within the ceiling, the superseded index and the sight-list line — then gates under a fresh count; a further record FAIL holds P3b for the architect's re-scope of the record (a shorter amendment set, no further correction round); option 2 closed for the reason given.
- *Item 3 — entry 103, the quote checker's stop:* **"One fresh-worker round under the mechanism, fresh count: Amendment 7 — the four findings, the 17→19 reconciliation, condition (b)'s two halves stated, the should-fixes — plus the hash check as the round's one named code addition, then gates. The amendment must attribute every sentence: mine as mine, the custodian's as the custodian's, the brief's as the brief's."** Applied: a fresh worker on the configured default dispatched into the checker's worktree with a verified handoff for Amendment 7 (the four findings, the 17→19 reconciliation, condition (b)'s reported and enforced halves, the should-fixes; every sentence attributed to its author — the human's, the custodian's, the brief's) and the hash check as the round's one code addition (a `byte-copied from path:line` marker with `sha256:<hex>` is recomputed over the cited lines as committed; a mismatch fails; tests; the CI step unchanged); then gates under a fresh count.
- *Item 4 — entry 103, the two unruled baseline entries:* **"Accept both (Recommended)"** Applied: `engine/ADMISSION-PREREGISTRATION.md:220` and `frontends/shell/POLISH-87-88-89-PREREGISTRATION.md:113` join the baseline by this ruling under round 11's conditions (`ruling: round 12`), each with the disposition the worker states — owed-correction naming its correcting piece where the source is real and the record immutable — listed in the PR body for the click; the baseline count becomes 19.

**RULED 2026-09-17 — question round 11 (AskUserQuestion, mirrored to Telegram first as `state/questions/round-11.md`; the human's typed answers verbatim):**

- *Item 1 — entry 102 (1), the `verify-quotes` baseline:* **"Accept the baseline as a ratchet on those terms, the 17 entries in the PR body. Conditions: (a) every baselined entry carries a disposition — unfindable-by-construction, tool-false-trigger, owed-correction — and the owed ones (the two re-typed glyphs in ruling text, the accepted-ADR note) carry the piece that corrects them by name; (b) the baseline is append-never: an entry leaves when corrected, none is added except by a ruling — a new mismatch always fails; (c) the baseline count is a line on the health strip, so a number that never shrinks is visible."** Applied: the tooling piece's next round gives every entry a `disposition` (`unfindable-by-construction` / `tool-false-trigger` / `owed-correction`, the owed ones naming the correcting piece; an entry whose bytes are immutable and whose correction is an appended statement carries `owed-correction` with that statement named — the custodian's reading), a `ruling` field required on every entry (the initial set: round 11; the check fails on an entry without one, so nothing is added except by a ruling; entries leave when corrected), and the baseline count on the site's health strip; the 17 entries go into the PR body; the tooling PR lands after P3b so its baseline is built against main as it will be.
- *Item 2 — entry 102 (2), the rule vs the tool:* **"1. Adopt the clause as written (Recommended)"** Applied: the clause added verbatim to `.claude/agents/reviewer.md`, `.claude/agents/architect.md` and `AI_DEVELOPMENT.md`: "`verify:quotes` green does not discharge this rule — the gate resolves each verbatim passage against its named source; the check is a floor, not the proof."
- *Item 3 — entry 102 (3), elisions:* **"Adopt, with: an elision may never remove a negation, a condition, or a qualifier from the retained meaning — the gate reads across each … for that, the same way it reads a paraphrase."** Applied: the clause "an elision marked `…` is honest quotation; each retained span must match byte-for-byte." plus the rider, verbatim, in both checklists and `AI_DEVELOPMENT.md`.
- *Item 4 — the 2026-09-17 directive's persistent edits:* **"1. Approve both (Recommended)"** Applied: `.claude/agents/worker.md` and `.claude/agents/tester.md` report formats gain the model observed, any override with its reason, any context handoff, and the four pre-gate failure classes as required report items; one `AI_DEVELOPMENT.md` bullet points at `state/directives/2026-09-17-dispatch-efficiency.md` with the operating rule in five lines.

**RULED 2026-09-17 — question round 10 (AskUserQuestion, mirrored to Telegram first as `state/questions/round-10.md`; the human's typed answer verbatim):**

- *Item 1 — entry 101, P3b stopped under Rule 7 on record fidelity:* **"Re-authorize the step, fresh count: a fresh Sonnet worker with a verified handoff, one appended correction round — engine Amendment 8 (the two quotes corrected verbatim, G-A2's three outstanding clauses stated including the "status text verbatim" P6 wording decision that is mine, the NEXT-CUT :107 fix) and the shell's mirror amendment (four comment cites, three ranges, the small-pan/no-query limitation added to §9's P6 sight list) — using verify-quotes and the cite-content listing as the pre-gate self-check, then gates attempt 3. Separately, verify-quotes lands on main as its own tooling PR, reviewer-gated, wired into CI beside verify:cites, and joins the gate checklists: a quote marked verbatim that does not match its source byte-for-byte is a gate failure by name. Mechanic: record-correction rounds always go to a fresh worker — fidelity work never runs from a long context."** Applied: a fresh worker on the configured default (Sonnet) dispatched into the P3b worktree with a six-line verified handoff for exactly the listed appends, self-checked with `verify-quotes` (from `governance/verify-quotes`) and a cite-content listing before the gates; gates attempt 3 under a fresh count. The verbatim-quote rule added to `.claude/agents/reviewer.md`, `.claude/agents/architect.md` and `AI_DEVELOPMENT.md`; the fresh-worker mechanic added to `.claude/agents/worker.md`'s dispatch rules and `AI_DEVELOPMENT.md`. `verify-quotes` (Sonnet pilot, `governance/verify-quotes`, 385 lines — over §21c's bound, so §21b's mid-piece clause adds the architect gate beside the reviewer the ruling names; PLAN node `governance-verify-quotes`) gets one round to close its normalization gaps (38% false positives on main's existing quotes) and to wire `verify:quotes` into `package.json` and CI beside `verify:cites`, then its gates and its own PR.

**RULED 2026-09-17 — question round 9 (AskUserQuestion, mirrored to Telegram first as `state/questions/round-9.md`; the human's typed answer verbatim):**

- *Item 1 — entry 100, P3b's end-to-end driver T10:* **"Fourth attempt with the full A9′ mechanism — the zoom-notch loop around the bisection, as regression.mjs runs it — then the three-run sequence (green / mutation red by name / green), gates after. Pre-declared fallback, not a fifth attempt: if the precondition fails again, T10 takes option 2 — the hover refusal asserted against the interior-verified pixel from the read-back — with the weakening named in the record ("formerly occupied, not formerly hovered") and the owner-side assertions still evaluated. Option 3 is not available: P3b claiming nothing about what an operator sees would undo the reason the piece exists."** Applied: the P3b worker wraps T10's S2 bisection in A9′'s zoom-notch loop exactly as `regression.mjs` runs it (shared from `e2e/lib.mjs`, byte-identical); if the precondition fails once more, the driver takes the pre-declared fallback in the same run series — S2 takes the interior-verified pixel from the read-back as the formerly-occupied point without requiring a hover id, the weakening named "formerly occupied, not formerly hovered" in the driver header, the report and the amendment, and S3–S5 still evaluated; then green / mutation red by name / green; §10 Amendment 8 quotes the ruling; the gates run after; a failure in the owner-side assertions themselves is reported verbatim, not patched.

**RULED 2026-09-17 — question round 8 (AskUserQuestion, mirrored to Telegram first as `state/questions/round-8.md`; the human's typed answers verbatim):**

- *Item 1 — entry 99 (1), the `windows-sys` dependency edge:* **"Accepted: windows-sys 0.61.2 as a direct cfg(windows) engine dependency for GetDiskFreeSpaceExW, as recorded. Its Unix counterpart (statvfs) is owed with docs/07's macOS/Linux gates, dated beside the Windows-only tier note."** Applied: the edge stands as recorded in `engine/LOD-DEP-RESOLUTION.md`; the LOD worker's Amendment 8 dates the `statvfs` counterpart as owed with docs/07's macOS/Linux gates, in the same sentence as the Windows-only tier note.
- *Item 2 — entry 99 (3), the caller rule on `build_tiers`:* **"The architect's reading: the round-3 acceptance licenses build_tiers to land ahead of the selection piece; the PR body and PLAN.yaml name the selection piece as its caller; no product path reaches build_tiers until that piece lands. Recorded as the caller rule's second exemption clause, beside the instrument-accessor one: a producer pre-committed by the human with its consumer named and gated may land ahead of that consumer; it exempts nothing whose consumer is undesigned."** Applied: the second exemption clause added verbatim beside the instrument-accessor one in `.claude/agents/worker.md`, `.claude/agents/reviewer.md`, `.claude/agents/architect.md` and `AI_DEVELOPMENT.md`; `PLAN.yaml` names the selection piece as `build_tiers`'s caller (node `lod-tier-selection`, blocked on the builder and on the lifecycle decision); the PR body will say the same; Amendment 8 records the ruling.
- *Item 3 — entry 99 (2), the tier-cache lifecycle:* **"Recorded as a named, owed decision for ADR-031 and a hard precondition of the selection piece: a total cache ceiling declared relative to free disk at its site (round 6's principle), delete-on-supersede as the default candidate with LRU over directories as the alternative, a runner with a scheduler, a priority policy and a cancellation owner — never a background job without all three — and an operator view. No product path calls build_tiers before the lifecycle is declared. Not widened into this piece."** Applied: recorded as an owed ADR-031 decision (`PLAN.yaml` node `lod-tier-cache-lifecycle`, a ruling for the human, on which `lod-tier-selection` depends); Amendment 8 records the ruling verbatim as the precondition; nothing widened into the builder.

**RULED 2026-09-16 — question round 7 (AskUserQuestion, mirrored to Telegram first as `state/questions/round-7.md`; the human's typed answer verbatim):**

- *Item 1 — entry 98, P3a stopped under Rule 7 a second time:* **"Fresh worker, exactly the listed five items, gates attempt 3 — the last attempt for P3a: if either gate fails again, P3a holds and the architect re-scopes the piece before any further worker touches it. Two additions: (1) the engine's SourceChanged Display text states the engine's fact only — "the source file changed while it was open (<component>)" — and never a consequence: the engine cannot know what the shell discarded, so the consequence sentence belongs to the owner that performs it, added by P3b when it becomes true. Engine messages state engine facts; owners state consequences. (2) Fence for the over-claim class, permanent in the gate checklists: every "discharged" or "done" clause in an amendment names the test or line that proves it, and the gate resolves each — a discharge claim with no resolvable proof is a gate failure by name, the same way an imagined interface and a stale cite are."** Applied: a fresh worker (new context) dispatched on the five items only, in the branch's existing worktree, append-only commits, `origin/main` merged in (never rebased); the engine's `SourceChanged` Display set to the ruled sentence — the `refused:` prefix the enum's refusal-class arms carry (the engine's own act) and boundary 4's limitation sentence, both engine facts, kept; the consequence sentence and the guidance removed; the three byte-identical pins re-aimed; the new sentence joins the P6 sight list; P3b adds the owner's consequence sentence when it becomes true — recorded as §10 Amendment 5 with every discharge clause naming its proof; the fence added verbatim to `.claude/agents/worker.md`, `.claude/agents/reviewer.md`, `.claude/agents/architect.md`, `AI_DEVELOPMENT.md` ("Gates and rule 7") and `docs/PREREGISTRATION-TEMPLATE.md` §10, with "Engine messages state engine facts; owners state consequences." beside it in the same five files; gates attempt 3 run with both additions; a further FAIL on either gate holds P3a and `briefa-p3-p6` waits on the architect's re-scope, no worker before it.

**RULED 2026-09-16 — question round 6 (AskUserQuestion, mirrored to Telegram first as `state/questions/round-6.md`; the human's typed answers verbatim):**

- *Item 1 — entry 97, the LOD tier builder's O8 stop (the tier-set disk ceiling):* **"The 2.0× set ceiling is withdrawn: it was derived from route A's output sizes, and route A is out; a declared value whose premise is dead is revised by ruling, reason recorded (class-5 amendment). Replaced by two things: (a) the bound that exists by construction — tier count × the per-tier 1.0× ceiling = 3.0× the source, declared as the set's hard bound; (b) a free-disk preflight: before any tier is written, the builder requires free space ≥ the 3.0× bound for that source, refusing typed engine.lod_insufficient_disk otherwise; and the built set's actual size is disclosed with the tiers (tiers.json + the prepare report), per the settled "prep time and disk cost disclosed" boundary. The five ignored tests un-ignored; then the gates. Option 3 not taken — a ladder that fits at 99.4% is one compression change from red, and the bbox column earns its bytes."** Applied: `engine/LOD-PREREGISTRATION.md` §10 gains a class-5 amendment quoting these words — `LOD_TIER_SET_MAX_BYTES` (2.0×) withdrawn with its reason; the set's hard bound becomes `LOD_TIER_COUNT × LOD_TIER_MAX_RELATIVE_BYTES` = 3.0× the source, declared at its site; a free-disk preflight before the first tier write with the typed refusal `engine.lod_insufficient_disk`; the built set's actual byte size disclosed in `tiers.json` and the prepare report; T1/T2/T3/T7/T11 un-ignored; the bbox column stays; then the full gates and the tester.
- *Item 2 — entry 97, the simplifier's epsilon:* **"The ladder's values are AREAS — minimum triangle area, in the CRS's squared linear unit: 0.1 / 1.0 / 5.0 m² on EPSG:2056 — the conversion factor squared for any non-metre linear CRS; values kept as measured; class-5 amendment to §2c and §7; T9's unit refusal unchanged. Two riders: every user-facing description of a tier (tiers.json, any future panel) says "minimum triangle area" in squared units, never "tolerance in metres"; and since §4 exercises only a factor-1.0 CRS, add one unit test with a synthetic non-metre factor that pins the squaring — the bug this ruling prevents is invisible on every fixture we own."** Applied: a class-5 amendment to §2c and §7 — the ladder is a minimum triangle AREA in the CRS's squared linear unit (0.1 / 1.0 / 5.0 m² on EPSG:2056), the ADR-026 factor squared for a non-metre linear CRS, values unchanged; `tiers.json` and every description say "minimum triangle area" in squared units; one unit test with a synthetic non-metre factor pins the squaring; T9 unchanged.

**RULED 2026-09-16 — question round 5 (AskUserQuestion, mirrored to Telegram first as `state/questions/round-5.md`; the human's typed answers verbatim):**

- *Item 1 — the P3b preregistration (`frontends/shell/OWNER-INVALIDATION-PREREGISTRATION.md`):* **"Accept as pre-committed with one change to (d): a false status string does not sit on main between P3a and P3b. Replace P3a's "Everything read so far has been discarded" now, in one docs-class commit, with a sentence true at that commit — "The source file changed while it was open; reopen the dataset to continue." — and P3b restores the stronger sentence when it becomes true, wording at P6. P3b starts after P3a lands; its two new strings join the P6 sight list."** Applied: the P3b preregistration stands as pre-committed with §10 Amendment 1 (class 5) recording this change to (d); P3a's `refusalGuidance("engine.source_changed")` text is replaced on the P3a branch, before it merges, by the human's sentence verbatim (a string-only commit, tests re-pinned); P3b's §2a(v) restores the stronger sentence, wording at P6; the two new strings (pick refusal, session-ended status) join the P6 sight list.
- *Item 2 — §21 housekeeping (the counting rule, the short-form budget class, the mid-piece route):* **"Insertions plus deletions over non-generated code and tests — deletions count, removing a guard is a larger change than adding a comment — excluding the piece's own preregistration and the governing-doc sentences it is obliged to update; that exempt set enumerated narrowly. Plus the mid-piece clause as drafted, and the template gains the short-form "budget deviation, Scope not edited" class."** Applied: `AUTONOMY.md` §21c gains the one-line counting note (these words) and §21b the mid-piece clause as the architect drafted it; `docs/PREREGISTRATION-TEMPLATE.md` gains a sixth class for the short form, "budget deviation, Scope not edited"; #82's Amendment line is left as recorded (class 2 was the nearest fit at the time; the new class governs from here).
- *Item 3 — the cancel-window guarantee (P3a, §13 C rule (ii)):* **"Keep rule (ii); the post-check stays inside the acknowledged term and P5 measures it, no figure claimed before. Two obligations: the post-check's cost is reported per cancellation (session log or the terminal's timing fields, never silent, its ≤ 8 MiB bound named); and the pre-existing slice.rs liveness test is re-aimed to assert the budgeted interval, requested → observed, reporting observed → terminal beside it with the disclaimer — a test asserting a budget nobody declared is the class that fails once under load and teaches nothing."** Applied: both obligations carried into P3a before its gates (the post-check's cost reported per cancellation through the engine's existing trace marks / stats with the ≤ 8 MiB bound named, never a claimed figure; `engine/tests/slice.rs`'s liveness test re-aimed to assert `cancel_requested → cancel_observed` and to report `observed → terminal` beside it with the disclaimer); P5 measures.
- *Item 4 — the caller rule's exemption for instrument accessors:* **"Adopt the architect's sentence as written, with one addition: the accessor's doc names the test that calls it, so the caller-grep can verify the exemption instead of trusting the words "test-only." An exemption that can't be grepped is a hole in the rule it exempts from."** Applied: the sentence, with the addition, appended to the seam rule in `.claude/agents/worker.md`, `.claude/agents/reviewer.md`, `.claude/agents/architect.md` and `AI_DEVELOPMENT.md`'s gate section: "The caller rule exempts instrument accessors: a `pub` read-only accessor over state the shipped build already maintains, whose doc declares that its only caller is the test suite, names the test that calls it, and says why the property must be proven about the shipped build. It exempts nothing that acts." P3a's four declared accessors name their tests before its gates.

**RULED 2026-09-16 — question round 4 (AskUserQuestion, mirrored to Telegram first as `state/questions/round-4.md`; the human's typed answer verbatim):**

- *Item 1 — entry 96, Brief A P3 stopped under Rule 7:* **"Split P3, on conditions. The landing half (P3a) contains only code with a real product caller and an end-to-end test from the real producer/consumer shape — both gates run the grep: no callback, code path or pub item lands without a caller; the unwired dead-ticket refusal and its fabricated source-change are removed from P3a, not carried; the publish-consumer regression is fixed in P3a (the shell parses the refusal code — no operator-visible regression on main); the Drop-backstop comment corrected. P3a claims nothing about boundary 4: the ADR-016 amendment's acceptance and any statement that detection "clears residency and refuses picks" wait for P3b, and the ledger says so. P3b gets its own preregistration and gates: the owner-side invalidation (residency cleared, picks refused — the consequence P3's own row promised), the kernel-authoritative dead-ticket refusal wired with a real caller and correct unknown-handle behaviour, and the §12e amendment recording the split. No release includes P3a without P3b.**

**Class fix, permanent, in the worker brief and both gate checklists: any cross-module seam is written against the interface the other side actually has — read it first — and is proven by one end-to-end test from the real shape; a test that encodes an imagined interface is a gate failure by name."** Applied: (a) P3a = the narrowed `cut/briefa-p3` branch — the worker removes the unwired dead-ticket refusal (`ticket_only_with_generations`, `SkpHost::generations()`, `SkpHost::invalidator()`, the redemption guard and its false diagnosis), removes the owner-side `onSessionEnded` callback and every statement that P3a clears residency or refuses picks, fixes the shell's publish-refusal consumer to parse the `"<code>: <display>"` shape (end-to-end test from the real kernel shape; no prefixed string reaches the operator), corrects the `Drop`-backstop comment, and appends §12e Amendment 3 recording the split and the deferrals (post-outcome, this ruling quoted) — then fresh reviewer + architect gates (attempt 1 of P3a), each running the caller grep and the seam rule; (b) node `briefa-p3b-owner-side-invalidation` added (engine lane, blocked on P3a; its own preregistration owed before code; contents per the ruling); the ADR-016 amendment's acceptance and every "clears residency and refuses picks" statement move to P3b; (c) "No release includes P3a without P3b" recorded in the ledger and as a plan dependency of the next release piece on P3b; (d) the class fix written into `.claude/agents/worker.md`, `.claude/agents/reviewer.md`, `.claude/agents/architect.md` and `AI_DEVELOPMENT.md`'s gate section, quoting these words.

**RULED 2026-09-16 — question round 3 (AskUserQuestion, mirrored to Telegram first as `state/questions/round-3.md` on 2026-09-15; the human's typed answers verbatim):**

- *Item 1 — the ADR-031 LOD preregistration draft (`engine/LOD-PREREGISTRATION.md`, aa5dcdb; C3 step 2; entry 93):* **"Accept the draft as pre-committed — SimplifyVwPreserve with its invalid-polygon falsifier, the 3-tier declared ladder, metre-typed tolerance through ADR-026's linear unit, content-hash sidecar tiers with row order untouched, never-serve-stale with lazy cancellable rebuild, tier vertices charged against the unchanged budget, the 100 ms cooperative-cancel ceiling with its residual measured, no Arrow 59 — with two additions recorded as class-5 amendments: (1) tier location: the sidecar directory lives in the app-local cache (%LOCALAPPDATA%\spatial-ide\tiers\<content-hash>\), never beside the source file without the user's word — a source may sit on read-only or shared storage, and writing next to it is a side effect the user didn't ask for; (2) geographic CRS: engine.lod_crs_not_linear is accepted as this cut's refusal, and the preregistration's non-goals record geographic-CRS tiers as a named follow-on, citing the corpus fact that most public GeoParquet is geographic — a declared gap with a date, never a permanent stance."** Applied: the draft stands as the preregistration; two class-5 amendments appended to its §10 quoting these words (the sidecar location; the geographic-CRS follow-on with its date); node `adr-031-lod-preregistration` closes on this ruling; the tier-builder piece is its own node under this preregistration's gate.
- *Item 2 — the LOD dependency decision (C3: "the product dependency decision rides the preregistration's ruling"; a red line):* **"Approved: geo 0.33.1, wkb 0.9.2, geo-traits 0.3.0 may be added and parquet 58 promoted to non-optional — subject to the gate steps in the draft's §8: transitive licence closures recorded compatible before any cargo add; the piece stops if a second Arrow major appears; the notice set regenerated and the lockfile diff read for new entries in the same PR. wkb's writer, never exercised by the spike, gets its own test on the first tier written."** Applied: the crate set and the `parquet` promotion are approved conditionally — §8 items 1–4 are gate steps, not satisfied by this ruling; a `wkb::writer` round-trip test is added to the tier-builder piece's §4 as a class-5 amendment; nothing is added to any manifest until the piece's gate steps record the transitive licences and the Arrow resolution.
- *Item 3 — P3, the format-default CRS class's `describe`/publish spelling (the P1 reviewer's Finding 1):* **"Crs.source gains the third value "format-rule" beside "file" and "caller_asserted", with crs.provenance carrying the specific class (crs:format-default). Existing assertions re-aimed with the ruling named; the manifest carries the same value; recorded as a value-domain widening under the SKP 0.3 bump."** Applied: carried into P3's fix round (gate attempt 2) — `CrsSource` gains `format-rule`; `crs.provenance` = `crs:format-default` beside it; the publish manifest carries the same value; existing assertions re-pinned citing this ruling; `SKP-V0.md`'s 0.3 entry records the value-domain widening.

**RULED 2026-09-15 — question round 2 (AskUserQuestion, mirrored to Telegram first as `state/questions/round-2.md`; both items from the LOD divergence spike, PR #80, whose citations the custodian re-verified — 7 of 14 fetched sources re-fetched, all SHA-256 match, every quoted line range byte-exact; the human's answers verbatim):**

- *Item 1 — C3 step 1, route A's admissibility (entry 93 / ADR-031):* **"1. Route A is out on principle (Recommended)"** — the option's own text: "'Can be statically bundled' is not met by the pinned crate; a custom static build is a new build-system and licence surface, not a bundling. The architect drafts the ADR-031 preregistration for route B alone, five open questions pre-committed for your sight, simplifier variant (VW-preserve vs RDP + degenerate-ring removal) as a pre-committed choice." The facts ruled on (PR #80, `spikes/lod-feasibility/README.md` "Route A static bundling"): the vendored `libduckdb-sys 1.10505.0` has no `spatial` feature (`extensions-full = ["json", "parquet"]`); both bundled-build backends hard-code a closed extension set and `bundled-cmake` panics on `DUCKDB_EXTENSION_CONFIGS`; the vendored DuckDB source archive holds no spatial source (0 of 3,596 members); a static route A would need a custom build of out-of-tree duckdb-spatial linking GDAL, PROJ, GEOS (`LGPL-2.1-only`), EXPAT, SQLite3, zlib (+ OpenSSL/libcurl) plus a `libduckdb-sys` linking path that does not exist; the venv's spatial extension is `install_mode = 'REPOSITORY'`. C3 step 3 is discharged by the same PR: the divergence is ring-level post-processing on both sides (GEOS drops a collapsed ring; `geo` refuses the cull at `POLYGON_INITIAL_MIN = 4` and emits a self-crossing ring), not an RDP difference; `SimplifyVwPreserve(5.0)` removes it on all five. Applied: route A is OUT on principle before any measurement; C3 step 2 proceeds — the architect drafts the ADR-031 preregistration for route B alone with the five open questions (spike README "Open questions") and the simplifier variant as pre-committed choices for the human's sight; no dependency approved (the route-B crate set stays a proposal in the spike README until the preregistration's ruling); node `lod-route-b-divergence` closes on #80's merge.
- *Item 2 — ADR-021 adjacency, DuckDB extension autoload ON by compiled default (new; from the spike's route-A facts):* **"Option 1: harden by configuration — autoinstall_known_extensions=false and autoload_known_extensions=false at every connection open, asserted fail-closed on every lease class; full two-agent gate despite size (security posture); ADR-021 amendment appended recording the configuration as part of the no-runtime-fetch property."** The fact ruled on (verified in the local cargo registry): `libduckdb-sys 1.10505.0` `build_bundled_cc.rs:96-97` compiles DuckDB with `DUCKDB_EXTENSION_AUTOINSTALL_DEFAULT` and `DUCKDB_EXTENSION_AUTOLOAD_DEFAULT` set to `"1"`; `engine/src` sets neither `autoinstall_known_extensions` nor `autoload_known_extensions` (`pool.rs` `CONFIGURE_SQL` is `SET enable_geoparquet_conversion=false` alone); ADR-021's property holds today by content (`json` built in), not by configuration. Applied: a new engine-lane node `engine-extension-autoload-off` — a short preregistration (cause → fix → test) before code; the two settings at every connection open (the pool's `CONFIGURE_SQL` site); a test asserting fail-closed on EVERY lease class (a non-bundled known extension function → catalog error, no install attempt, no filesystem write to the extension directory); reviewer AND architect gates regardless of size; the ADR-021 amendment appended (never rewritten) in the same PR, for the human's click.

**RULED 2026-09-15 — question round 1 (AskUserQuestion, mirrored to Telegram first as `state/questions/round-1.md`; the human's selections verbatim):**

- *Item 1 — entry 95, the pan's "slightly sideways" (Row 6 (c)):* **"2. Re-feel first (Recommended)"** — #78's recenter fix is in; the human feels the pan at tonight's sitting with that bug gone, then decides on the press-point anchor (candidate 2). Applied: nothing built on candidate 2; entry 95 stays open for the re-feel; the felt re-verdict is tonight's sitting.
- *Item 2 — sitting-4-rows:* **"1. Yes, close it (Recommended)"** — the node is marked done, evidence = the sitting record (the MANUAL-WALKTHROUGH sitting section, commit 69e14f6). Applied: `PLAN.yaml` sitting-4-rows → done; queue and site regenerated.
...
- *Entry 90 (static-CRT licence posture; RED LINE — the human's typed ruling):* **"69a stands for v0.1.1: dynamic CRT, declared in KNOWN-LIMITATIONS and QUICKSTART. The static-CRT build does not land: point 1's System Library reading is contested and the libcmt/libvcruntime grant is missing, so ADR-009's Caveat governs — counsel before any stronger statement, and no counsel is engaged for a question a second release does not yet need answered. Points 2–5 stay open behind point 1; 69c (bundling the redistributable) is not taken — elevation and an ADR-030 reopening for a convenience. The licence note is the recorded state of knowledge."** Applied: entry 90 and entry 69 close — v0.1.1 ships the dynamic CRT declared (69a), as v0.1.0 does; the static-CRT build setting does NOT land; 69c is not taken; points 2–5 stay parked behind point 1 (held pending counsel that is not engaged now); the licence note stands as the recorded state of knowledge. M13's exit code re-test is deferred to whenever a v0.1.1 release is built (a proposed milestone, the human's to schedule); the SignPath draft (D3) likewise waits for a CI-built v0.1.1.

**RULED 2026-09-14 — question set D of 4 (AskUserQuestion; the human's answers verbatim):**

- *D1, the walkthrough sitting date:* **"I can do it tonight, around 10 pm give or take"** — 2026-09-14, about 22:00 local. Applied: the sitting's rows queued and a prep pack drafted for that time (Part K 66(b), L7/L8, entries 84/85, Part G 86, the polish rows, the set-B rows: the 9 px threshold hovered at roughly 5, 9 and 15 px features and the pan re-pick feel; the "confirming…" marker's wording only if that piece has landed by then).
- *D2, ADR-023's Decision text (red line; typed):* **"ADR-023's Decision accepted as drafted with the entry-79 rulings folded, and two amendments to §2: (a) Float32 is ADMITTED as its own Arrow type, carried unchanged — every f32 is exactly representable in f64, so no conversion exists to object to; (b) dictionary-encoded columns are DECODED to their value type and admitted under that type's rule, the index never exposed — an ENUM of strings is text, not a synthesized identity. Timestamps/dates stay refused for v0 with dates named as the declared next admission. The four ADR-004 review conflicts: (1) the shell style module's in-code assertion that the live stream carries no attributes becomes false with B1 and is corrected in the same piece — never left as a lie; (2) ADR-023's Context sentence gets a dated corrigendum at acceptance; (3) per-column chunk retention at projection width is the architect's, with the memory consequence DECLARED as a ceiling (ADR-010 rule 6), never discovered; (4) the filter_composition prefix tests are re-pinned projection-aware without weakening the verbatim WHERE assertion — architect's, reviewer-checked. Status stays Proposed until B1's close."** Applied: the drafted Decision ported into `docs/adr/ADR-023`'s Decision with the two §2 amendments and the four conflict rulings recorded as B1 conditions, Status Proposed, in a PR for the human's click (ADR files are never docs-only-eligible).
- *D3, entry 77 post-tag:* **"Draft the application after v0.1.1 (Recommended)"** — the option's own text: "Once a release is CI-built, the custodian drafts the SignPath OSS application and the disclosure text for your sight; nothing is submitted." Applied: a release-lane node after the v0.1.1 build piece.
- *D4, the geometry node's placement:* **"Engine lane, after Brief A P3–P6 (Recommended)"** — the option's own text: "Proposed now; ordered directly after briefa-p3-p6 in the engine lane, phase alpha." Applied: node `geometry-types-beyond-polygons`, engine lane, `order` directly after `briefa-p3-p6`, phase alpha, depends on it, its own preregistration owed before code; the corpus numbers as its problem statement.

**RULED 2026-09-14 — question set C of 4 (AskUserQuestion; red-line items, the human's typed words verbatim):**

- *C1, entries 69 + 90 — the static CRT's licence posture:* **"Hold for the sourced note, with the posture I expect stated now so the note confirms or refutes it rather than starting blank. Expected posture: (i) the statically conveyed CRT is a third-party work compiled into the executable and so joins the NOTICE set under ADR-030 — one entry naming Microsoft's C runtime and pointing at its distributable-code terms; (ii) for the Corresponding Source statement it is a System Library under AGPL-3.0 §1 (a Major Component of the compiler), stated explicitly as such and excluded on that basis; (iii) yes, the note lands before the build setting: read-only fetches of Microsoft's distributable-code terms with URL, retrieval date and hash, entry-51 discipline, ADR-009's Caveat governing any stronger statement. If the sourced terms restrict static distribution or the System Library reading does not hold, the piece stops and returns to me."**
- *C2, entry 92 / ADR-029 — the operation lifecycle:* **"Q1: yes — { scanning | finished | unknown } as typed states mirroring cancel's shape; progress is a state, not a refusal, and unknown is the honest first-class answer when no reading exists, never a fake number. Q2: its own wire literal; numbering follows merge order per §13 E (0.5 if it lands after Brief B's 0.4, 0.4 if before) — one wire change per version in the order it merges, the number itself immaterial. Q3: the liveness string via the string-sight route; it is a new state, so I sight it. Standing conditions carried: the operation id is non-authorising and never derived from a handle; no data-plane change; G1's feasibility gate returns the quantity question to me if no monotone reading exists rather than shipping a non-monotone one."**
- *C3, entry 93 / the LOD preregistration (ADR-031):* **"(3) first: route B's invalid-output divergence is explained before either route is preregistered — including whether geo's topology-preserving simplification (SimplifyVwPreserve) removes it, since plain Douglas–Peucker producing invalid polygons is the expected cause and not a verdict on the route. (1) Route A is admissible only if DuckDB's spatial extension can be statically bundled with no runtime fetch — the ADR-021 security property; if it cannot, A is out on principle before any measurement. If both survive, both are measured under one preregistration with confounds removed, and the product path is decided by its outcome, with my stated prior recorded: B preferred (pure Rust, statically linked, small licence surface) unless its output validity cannot be established. (2) The architect drafts the preregistration now with the five open questions answered as pre-committed choices for my sight — I rule on drafted choices with their reasoning, not on five blank questions. No dependency is approved by this ruling; spike-only dependencies stay spike-only; the product dependency decision rides the preregistration's ruling."**
- *C4, the patch-bump precedent's scope:* **"Patch-level means: a semver patch of a direct dependency, or a lockfile-only bump of a transitive one — a bump that introduces any NEW crate or package into the lockfile is not patch-level and comes to me. CI green is necessary, not sufficient: the licence audit must show no licence change, the notice generator's output must be regenerated and checked, and the lockfile diff must be read for new entries. Dependabot security patches count if they meet the above and merge without waiting; they are announced in the same window's report and flagged; a security fix that is minor or major comes to me immediately via the alert channel. Non-security bumps are batched weekly in one PR. Any crate or package named in a preregistration's pins — measurement instruments, the transport bake-off crate, pinned tooling — is never bumped under this precedent: a dependency bump cannot silently change a measured artifact."**

Applied (custodian, from this record): C1 — a sourced licence note (read-only fetches; URL, retrieval date, hash; entry-51 discipline) drafted for the human's sight before the static-CRT build setting; entries 69/90 stay open on that note. C2 — ADR-029's decision text follows the three answers (typed states; its own wire literal numbered by merge order; the liveness string sighted as a new state; the standing conditions carried); the ADR's Status change remains the human's word after its gate. C3 — the LOD work is sequenced: the route-B divergence explanation first; route A's static-bundling admissibility checked against ADR-021; then the architect drafts the preregistration with the five questions pre-committed for the human's sight; no dependency approved. C4 — the patch-bump precedent's scope recorded in PRECEDENTS.md as confirmed with these limits (an added `Confirmed` paragraph; `Applied to: None yet` unchanged).

**RULED 2026-09-14 — question set B of 4 (AskUserQuestion; red-line items, the human's typed words verbatim):**
...
- *B4, entry 73 — the polygon-only geometry gate vs the compatibility corpus:* **"Hold the gate; do not re-scope the corpus. The corpus run records the non-polygon files as "refused, unchanged" — that is the result, not a defect in the run: the table exists to show what the world's files do, and if 8 of 12 refuse on geometry type, the geometry wall is bigger than the CRS wall and the record must say so. Brief A's admission rules are verified on the polygon corpus files plus the purpose-written polygon fixtures; prediction 1 stands as registered. Widening geometry support is filed as its own plan node — "geometry types beyond polygons (points, lines)" — with the corpus numbers as its problem statement and its own preregistration; it is not a Brief A rider."**

Applied (custodian, from this record): entry 91 (b)/(c) and entry 75 close as ruled — the labelled "confirming…" state (a new shell piece under its own preregistration amendment: the marker's tests as ruled; wording sighted at the sitting), the 9 px constant's comment carries the rationale and the revisability rule, the sitting gains a row hovering features of roughly 5, 9 and 15 px, HOVER_REPICK_ON_PAN stays pan+zoom with the sitting's flip rule, the disarm stays; entry 73 closes — the gate held, the corpus unchanged, a new proposed plan node "geometry types beyond polygons (points, lines)" citing the corpus numbers as its problem statement, its own preregistration owed; PLAN.yaml changes via PR (A1).

**RULED 2026-09-14 — question set A of 4 (AskUserQuestion; the human's answers verbatim):**

- *A1, plan-data commits (confirmation):* **"No — via PR"** — the option's own text: "Every PLAN.yaml change goes through a PR; docs-only auto-merge (§9) where it qualifies, otherwise your click." Applied from this record on: no PLAN.yaml change (nor its regenerated queue/site) is committed directly on main; each goes on a branch and a PR — the custodian merges it itself only when `docsOnly.mjs` and CI are green (§9), otherwise the click is the human's. The ledger (`state/`) and the verbatim ruling records keep their direct commits (the flush mechanism depends on it; this ruling's scope is plan data).
- *A2, M13's exit code (sight):* **"Not observed"** — the option's own text: "Leave it bracketed; re-tested at v0.1.1's release build." Applied: RELEASE-0.1.md's M13 bracket reads so; the plan node follows the v0.1.1 build piece.
- *A3, v0.1.0's evidence archive (click):* **"Prepare it; I upload (Recommended)"** — the option's own text: "Custodian builds and verifies the archive; you run the upload command." Applied: the archive built and verified by the custodian; the one-line `gh release upload` command handed to the human.
- *A4, ADR-033 (red line; typed):* **"ADR-033 accepted, on the completed gate (reviewer PASS; architect affirmed the design at gates 2 and 3), with one condition carried from my 2026-09-13 ruling: FIND′ recorded green — 63 of 63 tiles minted — on #59's head is the acceptance evidence. If FIND′ is green there, the Status line changes in #59 quoting these words and I merge; if it is not, the acceptance waits on the commit that makes it green. The three lease classes and their capacities stand as ADR-010 rule 6 declared ceilings; the ADR's statement that it does not pre-empt reserved ADR-014 is part of what I accept."** Applied: FIND′ is run on #59's head (b2c9aac) under the harness rules; its verbatim result is recorded in the preregistration (a further amendment) and here; green → the Status line changes in #59 in a commit quoting these words.
...
- **What holds (both gates).** Correctness PASS: skp 24+16 tests, the kernel crate green, verify 1055/1055, verify-mutation 12/12, CI 9/9. The architecture affirmed at attempt 1 stands, and the round changed no behaviour.
- **The record residue (record-only).** Five rustfmt blocks this piece wrote are called pre-existing. The discharge map's (b), (d) and (h) do not resolve. The superseded index is incomplete. §1 and the Aim still claim item 6 without scope. Item 17's comment mis-cites (1)(a) and dates a PR state. Its visible text says the drift value "happens to" equal the metre value, against "by derivation".
- **Merge order.** PR #109 and PR #112 both edit item 17. #109 lands first, and #112 then merges main and resolves item 17 by stating the bound.
- **Recommendation: continue, one text-only round under a fresh count.** Item 17 keeps a degrees sentence: the Q2 constants are still declared for metres, and the P0 render bound still holds; the anchor-span clause is replaced by the declared per-unit value. #109's item 17 text is carried through the merge. The five blocks are formatted. The record residue is corrected, then both gates run. A record-only failure after that goes to the architect's reduction under the record cap.
- **Alternative:** retire (1)(c)'s statement at landing. That is your word on entry 120, not this piece's.
- **Or hold.** The watcher (entry 122) stacks on this branch, and B1 takes the literal after both, so both wait with it.

Touches: PLAN node `crs-unit-fact-and-bounds` (blocked on this ruling); PR #112 stays draft.

127. **[RULED 2026-09-24 — question round 17; see the RULED block at the top; recorded as filed:]** **[FOR YOUR WORD, the morning round — `governance-verify-mutation-multiline-attrs` (PR #108) STOPPED under Rule 7 after two Correctness FAILs; PR #110 (stacked on it) waits with it.]**
...
**Recommendation:**
- (1): accept ADR-013 Amendment 1 with a dated acceptance note naming item 6 as owed, as for ADR-016's item 2. Add a proposed node for the unit fact and the two bounds, placed by you. The alternative is to hold ADR-013 out of the appending PR until that piece lands.
- (2): record it as a known limitation. The Map studio Inspector's Layer tab is its natural home, so no piece now.

What applying this touches: ADR-013's acceptance note in the appending PR, one proposed PLAN node, and one row owed to KNOWN-LIMITATIONS.]** Filed 2026-09-23.

119. **[RULED 2026-09-23 (late) — see the RULED block at the top; recorded as filed:]** **[FOR YOUR WORD — the five ADR acceptances, presented together at P6 after your N8 retest; two of the proposed texts are not yet true of the build.]** Context: Part N run 1 (N1–N7 PASS, N9 as given) and your N8 retest of 2026-09-23 are recorded in `frontends/shell/MANUAL-WALKTHROUGH.md`'s Part N run section. The retest passed as reported, on 7301dc6, which has PR #100 and not PR #101. Each proposed text is on main; it is appended to its ADR only on your word, with its PROPOSED file deleted in the same commit, as one PR for your click. Checked against the build, two proposed texts claim what the build does not do. ADR-013 Amendment 1 item 5 (`docs/adr/PROPOSED-amendment-to-ADR-013-geographic-degrees-instance.md` line 44 at 433413a) requires the shell's status at open to carry the equirectangular statement, but no component renders it. The statement exists only on describe's wire (`engine/src/crs.rs:54`, `kernel/src/skp.rs:1058`, `frontends/shell/src/skp/types.ts:100`); entry 113 (3) raised this and it was never put to you. ADR-016 Amendment 1 item 2 (`docs/adr/PROPOSED-amendment-to-ADR-016-identity-tier-model.md` line 49 at 433413a) says the descriptor's degradation is shown, but nothing shows it (`engine/ADMISSION-PREREGISTRATION.md` Amendment 4 (ii)); no corpus file reaches a degradation. **The five:**
- **(1) ADR-015 Amendment 1.** Evidence: Part N rows N1, N2 and N4, and the corpus run (`engine/ADMISSION-RESULTS.md`). Limits: the sanity check convicts and never confirms, and a projected file inside ±180/±90 is not caught (N9 item (4), owed to KNOWN-LIMITATIONS). The §5 normalize-later OPEN block stays open.
- **(2) ADR-032.** Evidence: N1, N2 and N4. Limit: the declared axis order is retained but rendered nowhere (N2's row).
- **(3) ADR-016 Amendment 1.** Evidence: N3 and N6–N8. Limits: item 2's display is unmet (above). The partitioned refusal is not exercised by Part N, for want of a fixture. The verified tier is unbuilt until Brief B. The change detector has its limits (N9 items (1) and (5)).
- **(4) ADR-028 Amendment 4.** Evidence: N6, N7 and N8. Limits: the late-result drops across a reopen are proven by tests and recorded mutations (PRs #100 and #101), and your retest ran without PR #101's guards. Their real width in WebView2 is unmeasured.
- **(5) ADR-013 Amendment 1, with the entry-81 clarification.** Evidence: N1 and N5. Limit: item 5's status-at-open surface is unmet (above).

**Recommendation:**
- Accept (1) and (2) as they stand.
- (5): a short-form piece that renders the already-wired statement in the describe summary and at open, then N1 is re-sighted.
- (3): accept with a dated acceptance note naming item 2's display as owed. Meeting it needs a new describe wire field, a wire change under full gating, for a condition no corpus file reaches. Alternatively, hold (3) for that piece.
- (4): a short N8 rerun on the merged build, in the same sitting as N1's re-sight.
- Then one PR appends all five for your click.

What applying this touches: the five ADR files, the four PROPOSED files (deleted), `PLAN.yaml`'s five accept nodes, and, for (5), one new short-form piece before the sitting. The enforced order keeps PR #97 behind the acceptances.]** Filed 2026-09-23.

118. **[RULED 2026-09-20 — see the RULED block at the top; recorded as filed:]** **[FOR YOUR WORD — three configuration decisions from the continuity/context/worktree proposal you asked for on 2026-09-20 (the proposal itself is the custodian's reply of that date; the practices it found already in force are applied, nothing else changed).]** (1) **A launch-scoped auto-compaction fallback:** run the custodian session with `--autocompact 200000` (or `CLAUDE_CODE_AUTO_COMPACT_WINDOW=200000` in the launch command only), leaving the saved global `autoCompactWindow` of 800000 untouched; the existing PreCompact flush hook then gets its one blocking chance near 200k loaded tokens instead of near the model's limit — a one-week pilot, reversible by dropping the flag. (2) **The fresh-start path:** `.claude/settings.json`'s SessionStart hook runs `scripts/hooks/session-resume.mjs` only on the `compact` and `resume` matchers; a new session or a `/clear` gets no reading order and no SESSION-CONTINUITY block injected. The smallest change is two more matcher entries (`startup`, `clear`) pointing at the same script — a project hooks change, yours; until then a fresh custodian reads `AUTONOMY.md` §0 by hand. (3) **A measured context figure the custodian can read:** the only measurement today is your `/context` output (2026-09-19: 257.7k loaded of 800k, 212.9k of it messages); the custodian cannot run it. Either you paste the figure at checkpoints, or a status line shows the context percentage (a user-level setting, the `statusline-setup` agent; whether the installed version's status-line input carries a context field is to be verified against its docs first, not assumed). Until one exists the block records `context: unmeasured`. Applied under existing authority, no ruling needed: the SESSION-CONTINUITY block gains three lines by convention (`workers`, `tested`, `next`) and records the last measured context; a completed piece or a changed assignment is the checkpoint, a fresh worker inherits the piece's worktree, and worktrees that run cargo on the workspace-excluded crates live outside the checkout. A checkpoint marker for the Stop hook and a pilot measure of four figures per piece are proposed, not built.]** Filed 2026-09-20.

117. **[RULED 2026-09-20 — the hybrid stays, caniuse-lite approved (rides PR #97's piece); merged at PR #99:]** 117. **[FOR THE CLICK, with one package decision and one finding for your word — the dependency-licence audit gains the shell tree (entry 116, finding (i)); PR #99 (`governance/npm-trees-shell` @ 1c9ac18; the five-line form `scripts/AUDIT-NPM-TREES-PREREGISTRATION.md`; reviewer attempt 1, gate-log 104).]** **What the piece does:** `NPM_TREES` in `scripts/audit-dependency-licenses.mjs` gains `frontends/shell`, so the audit PRE-PUBLIC-CHECKLIST item 4 relies on scans the packaged app's frontend for the first time; `DEPENDENCY-LICENSES.md` is regenerated (966 packages audited against the tracked 721; every tree that was auditable still is; `protocol/transport-bakeoff/web` stays the one not auditable). **Finding for your word (i) — the report is not purely generated.** `DEPENDENCY-LICENSES.md` carries a hand-maintained section, `## Third-party data terms (not a package): the EPSG Geodetic Parameter Dataset (IOGP)`, about 415 lines added over thirteen non-script commits of the release cut (the header's own sentence says do not edit by hand — re-run it, and the script had no code path emitting that section), so a plain regeneration would have deleted the IOGP compliance record silently. The worker stopped rather than commit it; the piece widened on its form before code (template §10 classes 2 and 6, `AUTONOMY.md` §21b's mid-piece clause): the writer now reads the existing report, re-emits that section verbatim at its place, and fails closed — exits non-zero without writing — when the file or the heading is missing. The section's bytes are proven unchanged (sha256 5533202c…b9e1 on both sides; the reviewer recomputes). Your call, not urgent: leave the hybrid as it now is (documented in the report's header), or move the section into its own tracked file — accepted ADR-026 and ADR-030, `README.md` and the release record cite `DEPENDENCY-LICENSES.md` by name for those terms, so a move is a cite-preserving change the custodian would not make alone; filed for the weekly window. **Package decision for your word (ii) — the shell tree's one review row:** `caniuse-lite` 1.0.30001809, declared `CC-BY-4.0`, flagged because CC-BY-4.0 is not on `RECOGNISED_PERMISSIVE` (`scripts/audit-dependency-licenses.mjs:75-91`). It is a browser-support data table pulled in by browserslist for the build, dev-only (no runtime path), a data licence over data — the shape of the 2026-08-07 `webpki-roots` CDLA-Permissive-2.0 decision. The row stays in the report as a question until you add a dated package-scoped entry to `PACKAGE_DECISIONS` (`:120-179`) or decline; the custodian makes no licence decision (ADR-009). **Two environment findings, informational:** cargo's workspace-root search, on finding a root whose `exclude` names the crate, keeps climbing — under `.claude/worktrees/` the next `Cargo.toml` up is the main checkout's, so `cargo metadata` fails for the two workspace-excluded standalone crates in every such worktree (the earlier dependency-audit worktree's five not-auditable trees had this cause); the fix used was moving the worktree outside the checkout, no manifest edited, and an empty `[workspace]` table in each excluded crate's manifest is the permanent alternative if you want it. And the ADR-003 spike's locked `async-trait` 0.1.91 was absent from the local registry cache (the main checkout too), restored by `cargo fetch --locked` with the lockfile unchanged. PLAN node `governance-audit-npm-trees`.]** Filed 2026-09-19.

116. **[RULED 2026-09-20 — see the RULED block at the top; PR #97 lands as one gated dependency piece after the sitting:]** 116. **[FOR YOUR WORD — the shell's npm vulnerabilities under your ruling on entry 115 (c): nothing ships, no patch-level fix exists, every fix is a major bump — draft PR #97 carries the applied fix, its lockfile diff and the licence check for your inspection; not landable without your word.]** **Reachability (phase 1, read-only; the notice generator's own shipped-set definition — the post-tree-shaking Rollup module walk at `frontends/shell/vite.config.ts:40-73` for the frontend, the esbuild metafile at `renderer/bundle-viewer/build.mjs:26-42` for the viewer):** all six advisories — `vitest` 2.1.9 (npm: critical), nested `vite` 5.4.21 (high), `vite-node` 2.1.9, nested `esbuild` 0.21.5 and `@vitest/mocker` 2.1.9 (moderate) in the shell; `esbuild` 0.24.2 (moderate) in the bundle viewer, a sixth found by auditing both trees — are dev-only or build-only: none appears in any shipped set, none has a runtime dependency path (`npm ls --omit=dev` empty for each), the build's own esbuild is `vite@6.4.3`'s 0.25.12 outside the vulnerable range. Group B throughout; by your rule severity is the input and reachability the ruling, so no advisory is urgent. **The fix:** `npm audit fix` without `--force` proposes nothing; every real fix is major — `vitest` ^2.1.8→^5.0.1, `vite-node` ^2.1.8→^6.0.0 (shell), `esbuild` ^0.24.0→^0.28.2 (viewer; npm counts a 0.x caret change as major) — and the shell bump pulls ten new packages into the tree, two of them MPL-2.0 (`lightningcss` 1.33.0 and `lightningcss-win32-x64-msvc` 1.33.0), a licence not on `RECOGNISED_PERMISSIVE` (`scripts/audit-dependency-licenses.mjs:75-91`) and with no dated package-scoped decision (`:120-179`); the other eight are MIT or Apache-2.0. Nothing is custodian-landable under P-033. **Phase 2 (draft PR #97, branch `cut/shell-dependency-audit`; the preregistration `frontends/shell/DEPENDENCY-AUDIT-2026-09-19-PREREGISTRATION.md` references only):** the forced fix applied in both trees; `npm audit` afterwards reports no vulnerabilities in either; the lockfile diff is the PR (shell: 35 `node_modules/*` keys added, 62 removed, 15 changed; viewer: 1 added, 26 changed). The viewer is clean: build green, its notice test green (69 of 69), `dist/NOTICE.txt` byte-identical before and after (sha256 826e55d1…4805fb), one MIT platform stub added. The shell is NOT: `vitest` 5 changes `vi.fn()`'s typing, and `tsc --noEmit` fails at two typed callback sites in `frontends/shell/src/residency/candidateArmSession.test.ts` (lines 2271 and 3335, TS2322), so neither the shell's 1026-test suite nor its build ran post-fix and its `NOTICE.txt` could not be regenerated — recorded, not patched (a test-typing change in a file P3b's closed record cites is not this piece's to make without your word). **Two findings beside the fix:** (i) `NPM_TREES` in `scripts/audit-dependency-licenses.mjs:210-215` does not list `frontends/shell` (stale paths remain), so the licence audit that gates ADR-009's boundary has never scanned the shell tree — a small governance fix, reviewer-gated, the custodian's to propose; (ii) two esbuild postinstall scripts are flagged-but-unapproved by npm (`npm approve-scripts`), informational. **Options:** (A) approve the three major bumps and the two MPL-2.0 package decisions (dated, package-scoped, at `scripts/audit-dependency-licenses.mjs:120-179`) — then the piece fixes the two typed call sites (class-3 test text), rebuilds, regenerates the shell notice set, and takes full gating; (B) decline the shell bump: the advisories are dev-only, nothing ships — record them as accepted risk with a review date (the custodian proposes 2026-10-19) and close the item without a change; (C) the viewer's `esbuild` bump alone now (one package, MIT throughout, build and notice test green, NOTICE byte-identical) and (B) for the shell. **The custodian's reading:** (C) — the viewer's bump is exactly the shape P-033 is written around except that npm labels a 0.x caret change major, which is why it still needs your word — with (B) for the shell until `vitest` 5's test-typing cost is worth paying; and the `NPM_TREES` fix dispatched regardless, since the audit's blind spot is a standing gap in ADR-009's boundary check. PLAN node `decision-shell-dependency-audit-2026-09-19`.]** Filed 2026-09-19.

115. **[Recorded, with three items for your word — the drill's findings (PLAN node `drill-clean-clone`; PR #96, docs-only under delegation).]** A tester cloned main (14c8612) into a fresh directory on D:, regenerated `parcels-5gb.parquet` from `kernel/FIXTURES.md` (size and sha256 both matching the declared values, the hash recomputed independently), ran the workspace suite (rc 0, 647 passed / 0 failed / 38 ignored, 0 warnings), the ignored tier (the literal command stops at the first release-only guard; a labelled no-fail-fast pass recorded the named reasons), the shell build and test (70 files / 1024 tests green once the bundle-viewer is built first), the release build (rc 0, 0 warnings) and the Tauri NSIS build as the workflow runs it (rc 0; the installer's size and hash recorded). The section's own claim that no timing number appears in it was false against its text (quoted watchdog and npm lines; the unguarded test's elapsed clock) and is withdrawn by an appended correction, not edited; none of those numbers is a product measurement. Findings: (a) `kernel/FIXTURES.md`'s regenerate command races itself — the publish-half runs concurrently with the generate-half and fails on 'fixture absent' — and the generator's watchdog fires in a post-write phase after the file is complete; the fixture is correct despite both. Proposed: a small kernel-test piece (the two ignored tests serialised or the publish-half made to wait on the fixture; the watchdog's silence window scoped to the write phase), engine/kernel lane, reviewer-gated, five-line form. (b) `frontends/shell`'s bare `npm run build` fails standalone; the build order (the bundle-viewer first) lives only in the `pretest` script. Proposed: a one-line `prebuild` (or the build script itself) naming the order — polish lane, five-line form. (c) `npm ci` reports five known vulnerabilities in the shell's dependency tree (three moderate, one high, one critical) — recorded, not remediated. **For your word:** dependency changes are yours (P-011; the Red lines); PRECEDENTS.md P-033 lets the custodian merge a Dependabot patch-level bump when CI is green, and nothing more. If Dependabot has opened or opens patch-level PRs for these, the custodian merges them under P-033; anything minor or major, or any manual `npm audit fix`, waits for you. (d) `engine/tests/lod_tier_measurements.rs`'s `wall_time_arm_s_and_arm_p_over_polygons_100k` lacks the release-only guard its three siblings carry and ran without a verdict in a debug drill until the tester terminated it (recorded as inconclusive, not a pass or fail). Proposed: the one guard line, LOD lane (the LOD's own record names the measurement tests; a class-3 test-text row there). (e) The 5 GB rows refuse at this machine's C: disk floor (31.5 GB free against the 40 GiB floor); the drill ran on D: with over 360 GB free throughout — the floor finding of entry 111 stands. **For your word (2):** whether (a), (b) and (d) are dispatched as three five-line pieces this week or queued behind Brief A's close; the custodian's reading: (d) first (one line, it stops an unbounded test), then (a), then (b) — all three inside the short-form route. **For your word (3):** the drill's result section carries the installer's hash for v0.1.0's build at 14c8612 — whether that hash joins the evidence archive (`evidence-archive-v0-1-0`) or stays in RESULTS.md only.]** Filed 2026-09-19.

114. **[FOR YOUR WORD — where generated outputs should live, under your directive of 2026-09-19.]** The custodian's reading of the three shapes. (A) The mechanic alone (now in `AUTONOMY.md` §2): every PR touching `PLAN.yaml` regenerates on merge before the click; no code change; it relies on the custodian doing it every time, and a sibling PR still turns CONFLICTING the moment another lands (as #91 did after #88/#89), so each click can cost a regeneration round trip. (B) A `.gitattributes` merge driver (`merge=ours` on the generated set): GitHub's own merge computation ignores custom drivers, so the PR page would still read CONFLICTING and the click would still be blocked; it helps only local merges. Not sufficient on its own. (C) Generated outputs leave the PRs: `site/index.html` and `site/data/plan.json` are built by the Pages workflow from `PLAN.yaml` and the committed `site/data/health.json` (the machine facts stay a flush commit, as §5 has them) and are no longer tracked; `CUSTODIAN-QUEUE.md/.json` stay tracked (the Stop hook reads the JSON locally; you read the Markdown) but the PR CI check regenerates them against the PR's merge ref (`refs/pull/N/merge`) and fails on drift, so staleness shows before the click rather than as a conflict after one; `verify.mjs`'s site-drift check moves into the Pages build. Cost: one governance piece (a workflow, a `.gitignore` entry, the drift check split), and the Pages source moves from `main:/site` to an Actions deployment — a setting only you can change. (D) CI regenerates on `main` after merge with a bot commit: it fights the branch protection you set (required CI, DCO, no bypass) and puts unsigned generated commits on main; the custodian advises against it. Recommendation: (C), as a governance-lane piece after the current wave lands, with (A) as the standing rule until then. PLAN node `decision-generated-outputs-merge-strategy` (blocked on your ruling).]** Filed 2026-09-19.

113. **[Recorded — round 16 ruled (the RULED block above); P4's first read; one dispatch-shape correction; a fourth wire-only field for your next round.]** (1) P4's reviewer, attempt 1, FAIL (gate-log 89): the substance reproduced end to end — the runner on the engine's real admission path; 17 of 17 hashes; 12 of 12 main-set predictions; M-4's deviation correctly diagnosed; the generated evidence byte-identical on regeneration; the mutation proven; the workspace suite green on a clean run — and four blockers, none touching the result: the runner's `#[test]` above a wrapped `#[ignore]` so verify-mutation sees zero tests (a vacuous pass); Amendment 15's four pins at branch commits without Amendment 13's sentence; the two not-comparable rows absent from the record; the workspace-tests row an unpinned cite, a quotation and narrative whose 'partial' claim the green run supersedes. One correction round to a fresh worker under the unmerged-append revert: the record commit reverted and Amendment 15 re-appended whole (rows only; branch pins named as such; the not-comparable row; a green G-A6 row from a quiet-machine run), the `#[test]` reorder, the should-fixes batched (the hash check as an assertion; §5's predictions resolved by name in the evidence, prediction 6 as `unrun — reason`; sanity levels compared to §3; the second duplicated kernel vocabulary disclosed; §8's wording); its attempt 2 is tagged on the strip (record-led). (2) A custodian correction: P4 was dispatched reviewer-only; its runner is 858 non-generated lines, past `AUTONOMY.md` §21c's bound, so attempt 2 takes the architect alongside the reviewer. (3) `crs.display_convention` is a fourth wire-only field (`frontends/shell/src/skp/types.ts:93-100`: the geographic-degrees display sentence, "Render it verbatim"; no component reads it). It is outside item 1's three fields as you ruled them; Part N's N1 ("read the provenance line") and ADR-013's instance may want it on screen. For your word at the next round, not this one — the describe-fields piece renders exactly the three. (4) P5's worker, round 3, has reverted its two record commits (cbd6b54, 4d36b4b) to re-append the record reduced — the unmerged-append revert in use a third time before its weekly-window proposal. (5) Item 5 executed as its Applied line says; the commit named in the ledger when pushed.]** Filed 2026-09-18.

112. **[FOR YOUR WORD — question round 16 (`state/questions/round-16.md`, five items, the options in the order the round asks them).** (1) P6's sight #2 has no on-screen string: render the three wire fields before Part N under a five-line shell piece (recommended), re-scope the sight, or hold Part N for B2. (2) ADR-029 G1 STOPPED under Rule 7 (gate-log 80, 86): the substance stands; the write-up failed two reads on sentence-level items whose remedies are written out — one more read under a fresh count with those remedies applied as written (recommended), an architect's reduction to the verified facts, or hold. (3) ADR-029's route given G1 — no recommendation: a minimal crate patch exposing the handle; raw ffi end to end for the scan path; upstream first; or defer with Brief B. (4) The superseded-name scanner STOPPED under Rule 7 (81–82, 87–88): the mechanism sound and sharply tested, the second attempt failing on an untested condition, two in-place edits of committed amendment lines, and two unresolvable cites; P3b's done depends on it — one more round under a fresh count by the unmerged-append revert (recommended), or hold with P3b in progress. The custodian's and both gates' boundary position: a claim is never superseded with no replacement. (5) Three baseline dispositions in the checker's ratchet (PR #91): `HOVER-REPICK-PREREGISTRATION.md:53` and the two `A9′`/`A9'` misquotes of your round-9 ruling — owed-correction as filed (recommended), the tool-false-trigger / POLISH-113 readings, or the same plus one erratum row by your word. Filed for the weekly window, not asked: the unmerged-append revert; round 15 (e)'s collision for a piece's own new files; a `replaced by <name>` cell; main's Amendment 16/13 pin rule; a fourth governance piece this week.]** Filed 2026-09-18; PRs #88–#91 open; P4 under its reviewer; P5 in its third round; the drill held.

111. **[FOR YOUR WORD — a P6 sight with no on-screen string; and the first results of the 2026-09-18 wave.** (1) **Three wire fields ship with no consumer.** The session-identity statement (`engine/src/identity.rs` → `kernel/src/skp.rs` → the wire → `frontends/shell/src/skp/types.ts`) and the two CRS provenance fields (`crs.provenance`, `crs.axis_provenance`) are read in the shell by tests only; no component renders them (the Part N worker's finding, confirmed by its reviewer tracing every reader). Brief A's P6 sight "open a keyless single file, read the session-identity statement" cannot be read at the sitting as the build stands; Part N's row N3 says so and discharges nothing. Your call, a red line either way: (a) a small shell piece renders the three fields in the describe summary before Part N runs (a preregistered sight; under ADMISSION's existing record or a five-line form), or (b) the sight is re-scoped. The round-8 pre-commitment exemption let the producer land ahead of a named, gated consumer; P6 is that gate and arrives with the consumer unbuilt — filed for the weekly proposal window as a finding, not a clause. (2) **P3b's six §5 test names.** Marking P3b done bound the §5 names T3–T7 and T9, which the tests carried under other titles; a short-form piece (`cut/briefa-p3b-test-names`, five lines committed before code) aligned the titles to §5 (13eca3c). The rename makes bare italic quotations of the old titles stale in both closed records (shell Amendment 15 rows 2a(ii), 2a(iii), 2a(v), 2b; the instrument-exemption row; engine Amendment 12 item (i) and the G-A2 paragraph) and one kernel doc-comment; its second round appends one class-3 erratum row per record under your round-14 exception (shell Amendment 17, engine Amendment 14 — the custodian had briefed P4 and P5 with colliding engine numbers, corrected to 15 and 16 by message), then the reviewer gate. P3b shows done in PLAN when it lands. (3) **ADR-029 G1** (`spike/adr-029-g1` @ 367d2dd): the vendored crate's C surface `duckdb_query_progress` yields a rows-processed reading, monotone non-decreasing over eighteen runs, reaching the fixture's row count; but `duckdb::Connection` exposes no handle to call it, so the probe drove the raw ffi via `duckdb_pending_execute_task`. Under a scoped reviewer for the verdict line: whether the engine can consume the reading through the crate as vendored, or what a raw ffi connection, a crate patch or an upstream feature would cost — the fact your ADR-029 word rests on. (4) **PR #88** (the handoff recorded once per part) is open for the click; the two Part 4 decisions are ruling nodes under "Waiting on the human" (proposed nodes are never queued). (5) **Part N** failed its first scoped read on three items (three rows promised a row count the session tier never shows; the canvas-clearing read had no row; two "quick"s), all six quoted strings byte-exact; one batched correction round with the same worker, then a second read. (6) **A custodian error, corrected:** a `git add -A` on the shared main checkout swept the G1 worker's stray `Cargo.toml` into 8b88c58; removed at 5632f94; the lesson in memory (stage explicit paths; check porcelain first).]** Filed 2026-09-18; the checker's final round, P4 and P5 still running. **Later the same day:** Part N passed its second read (gate-log 76) and is **PR #89** for the click — nine rows, plan about 30–35 minutes at the machine, one sight owed to your word; the test-names piece's second round appended shell Amendment 17 (sixteen rows) and engine §12e Amendment 14 (three rows), every pin recomputed, and found that the claim scanner has no notion of a superseded name: three historical mentions of T3's old title in append-only text would still turn red once P3b is marked done, so a short-form scanner piece (`governance-test-claims-superseded`: a claim is historical when the same file pins its line as superseded at a rev whose bytes hash true) is dispatched; the test-names branch is under its reviewer gate. **Findings filed from the gates of 2026-09-18 (evening), for the weekly proposal window (earliest 2026-09-25) and the next question round — nothing adopted:** (i) the *unmerged-append revert* — a defective record append on an unmerged branch corrected by a revert commit plus a re-append (never an amend or a rebase; the corrected amendment re-appended whole; append-only proved against the base), named sound by the test-names architect as an extension by analogy of round 15 (f), used twice today (the test-names erratum rows; the checker's Amendment 9); (ii) main's landed P3b erratum rows (shell Amendment 16, engine Amendment 13) declare a pin rule contradicting round 15 (e) and pin at 959bd088156b, a branch commit — a defect on main whose correction rounds are exhausted under the cap; (iii) `verify-cites` resolves `path:line @ rev sha256` pins against the current tree, the same pin-read-as-current class; (iv) the checker's grammar cannot write a reference into a leading-dot path (`.github/**`, `.claude/**`), so such fixes are recorded by path only; (v) ADR-029 G1: the reading exists and is fine-grained, but `duckdb::Connection` exposes no handle and the reading is per-connection, so the engine reaches it only by driving the scan through raw ffi, by a crate patch, or upstream — your ADR-029 word rests on that choice; with defaults there is no reading (the progress-bar pragma is the precondition), DuckDB latches only the percentage, and the count must be latched engine-side; (vi) three dispositions need your ruling, not a worker's: `HOVER-REPICK-PREREGISTRATION.md:53` (the quoted consult is now tracked; the quote drops a trailing citation unmarked), the two misquotes of your own ruling at P3b's shell record `:799`/`:968` (`A9′` re-typed `A9'`; owed-correction with no corrector in a closed record, or the POLISH:113 precedent), and whether a test claim may ever be marked superseded with no replacement (the custodian's and the architect's position: no — an explicit sentence a gate reads, never a mechanical exemption). **PRs opened the same evening:** **#90** (`cut/briefa-p3b-test-names`, its landing confirmed byte-exact against the reduction; the superseded-title sweep now exhaustive at eleven shell sites and three engine) and **#91** (`governance/verify-quotes`, its landing confirmed: Amendment 9 as reduced, the fabricated quotation gone, the checker 103 checked / 73 verified / 29 baselined / 2 hash-baselined on the merged tree). P5's architect (gate-log 85) failed it on G-A4's enumeration (LOD tier sets and the permission audit log are persisted artifacts the scan does not cover) and a composited quotation, reducing its record; one batched round runs; the reviewer follows on a quiet machine. P4's reviewer is running. **ADR-029 G1 STOPPED under Rule 7 (gate-log 80, 86):** the probe is sound and the reading's existence is established (a `u64` rows-processed count on the `read_parquet` path, fine-grained, reaching the fixture's true count in every archived run), but the written conclusion failed two reads — the second on three sentences: a precondition its own cited source refutes (the progress-bar timing pragma gates printing, not the reading; the reviewer's own crate populated the reading without it), the verdict calling the count monotone as an API property when DuckDB latches only the percentage, and an invented cost for route (i) (a statement cache the engine does not have). Round 16 asks whether the custodian applies the reviewer's three sentence remedies and the should-fixes as written for a third read, or the piece holds. The substantive question for your ADR-029 word stands regardless: the engine reaches the reading only by driving the scan through raw ffi, by a crate patch exposing the handle, or upstream; the count must be latched engine-side; the denominator DuckDB reports is an estimate (100,002 for 100,000 rows), which ADR-029 1(c) forbids forwarding. The superseded-name scanner's attempt 2: architect FAIL-reduced (87) to one appended line with the boundary stated honestly (a pin in an append-only record proves age and identity, not disappearance); its reviewer pending.

110. **[The human's word of 2026-09-18 after the four merges (#84, #85, #86, #87), verbatim:** All four merged (#84, #85, #86, #87). Next: (1) verify-quotes: its final round now under the directive — references-only closing amendment, baseline re-derived against final main, one fresh worker, both gates; land it. (2) Brief A P4–P6: dispatch P4 (the corpus run against the preregistered expectations; every deviation a recorded result) and P5 (the gate tests, G-A1 through G-A6); prep Part N as the next sitting — give me its rows and duration; queue the five acceptances (ADR-015 A1, ADR-016 A1, ADR-013 instance, ADR-028 qualification, ADR-032) for P6. (3) Regenerate PLAN.yaml: P3a/P3b done with their PRs as evidence; B1 shows as blocked on Brief A's close, not on P3. (4) Drill and ADR-029 G1 proceed as ready nodes. Report ≤20 lines; the record-round count on the strip is the number I'm watching this week.

Documentation-only handoff: C:\Users\Christopher\Development\Claude\Spatial IDE\HANDOFF-2026-09-18-source-consistency.md. Record each part once in the document it names — Part 1 as one AUTONOMY section; Part 2 as a proposed PLAN node (engine lane, unscheduled) with its brief under state/ beside the P3b preregistration; Part 3 into the Brief B draft, with "accepted product direction" and "awaiting preregistration" kept as separate headings exactly as written; Part 4 into the queue as two entries. No amendments narrating the discussion; no implementation, dependency, schema or benchmark; P3b untouched. One docs PR for my click; report ≤10 lines.**]** **Applied the same day, 2026-09-18:** PLAN regenerated — `briefa-p3-p6` retitled P3a and done (PR #83, merged 2026-09-16; the verdict cite is this entry), `briefa-p3b-owner-side-invalidation` done (PR #86); new nodes `briefa-p4-corpus-run` and `briefa-p5-gate-tests` (in progress, dispatched), `briefa-p6-part-n-acceptances` (Brief A's close: the Part N sitting, 60 minutes planned) and five click nodes `accept-adr-015-a1`, `accept-adr-016-a1`, `accept-adr-013-instance`, `accept-adr-028-qualification`, `accept-adr-032` for P6; `b1-engine-kernel-half` now depends on P6. Dispatched as fresh default-model workers, each in its own worktree: the checker's final round on `governance/verify-quotes` (round 15 item 3's fix list, the baseline re-derived against main, Amendment 9 references only, both gates after the handoff); P4 to a tester (`cut/briefa-p4`: the runner under `engine/tests/`, the table to `engine/ADMISSION-RESULTS.md` as generated evidence, Amendment 3 references only); P5 to a worker (`cut/briefa-p5`: G-A1–G-A4 and G-A6 with recorded mutations, G-A2's post-check route end to end, Amendment 4 / P3b Amendment 17); ADR-029 G1 to a worker (`spike/adr-029-g1`: a probe against the vendored duckdb crate, the written conclusion under `spikes/`); Part N's rows to a docs worker (`docs/part-n-walkthrough`: N-rows with every expected outcome quoted from a shipped string, a blank result log, a sitting estimate as scheduling only); the handoff's four parts to a docs worker (`docs/source-consistency-handoff`: AUTONOMY §22, the proposed node `engine-source-change-watcher` with its brief at `state/briefs/source-change-watcher-brief.md` — a new directory, because `state/drafts/` is archived record whose cites are never gated — Part 3 into the Brief B draft under the two headings as written, Part 4 as two proposed ruling nodes so the queue lists two entries). The drill (`drill-clean-clone`) runs after P4 and P5 free the machine and the disk floor (four merged worktrees swept first: 77.9 GiB free on C:, 386.6 GiB on D:). The two consults committed under `state/consults/` (004820b). The record-round count on the strip after #87: P3b 5, LOD 4, checker 3, record-round count 1, total 13, target 0 — its predicate the custodian's stated choice, the human's to adjust by a one-line note.

109. **[The record-round count (`governance/record-round-count` @ 7d755d8) at attempt 2: reviewer PASS (gate-log 71), architect FAIL (72) on one blocking item — the `record` tag's predicate is written in no tracked text a reader of the gate log or the strip reaches (the plan README's entry shape and metric list; AUTONOMY §15 still names two throughput metrics) — with the record RULED CLOSED under your directive's point (3): no third `Amendment:` line, the figure of record 159/117 at 6a30ea1, the fourth test carried by a pinned reference in record 72's note. The custodian's decision on the predicate, documented in the README this round: the tag marks an attempt that gated a round dispatched after the piece's first gate attempt whose work included the piece's record, whatever else the round carried and whatever the verdict; a first gate attempt is never tagged, even when it fails on the record — the count is rounds spent on the record, the quantity point (3) caps. Under it this node's attempt 1 stays untagged and attempt 2 is tagged (the architect's landing condition asked for both; the predicate decides; if the attempt-3 architect rules the predicate wrong, the question is yours). The round (attempt 3, not a record round): main merged with the gate log resolved by script, the README, one line-count-neutral sentence in §15 (the closed record cites `AUTONOMY.md:345` and `:350` bare), the node summary, the two tags, both gates. Carried, not corrected (the cap): Amendment 1's class label (class 1 fits an ordering defect; class 6 was the attempt-1 report's own prescription); the two bare AUTONOMY cites; Amendment 2's fourth sentence; the NaN-attempt collapse (an undercount unreachable with real records); two uncovered nit-fixes; a stale test title; a bare self-line in a comment. FINDING for the weekly proposal (earliest 2026-09-25), the architect's, filed not adopted: a row added to the published strip is user-visible behaviour under §21c, and a directive ordering the row does not exempt it — such a piece takes full gating from the start, or you adjust the bound by a one-line note. Rule 7 count: reviewer FAIL then PASS; architect one FAIL — a second architect FAIL stops the piece.]** Filed 2026-09-18; nothing merged; the attempt-3 round running. **Corrected the same day, before the attempt-3 gates:** the predicate's wording in this entry ("whose work included the piece's record") was too wide — nearly every fix round appends an amendment, so it would have required tags on the code-led rounds the log rightly leaves untagged (P3a's attempts 2–5, the LOD's attempt 2). The wording committed in `scripts/plan/README.md` on the branch (7d1f70d, the custodian's own two sentences) is what the 23 tags implement: an attempt whose round was dispatched, in whole or in leading part, to correct or re-read the piece's record (the record-fidelity class, or a scoped re-read of appended record text), whatever else the round carried and whatever the verdict; a round dispatched for code, tests or operator text is not tagged even when its findings touch an amendment; a first gate attempt never. The attempt-3 round (cfec693, d4062fe, 7d1f70d) is under both gates. **PR #87 for the click, 2026-09-18:** attempt 3 PASS / PASS (gate-log 73–74) — the predicate ruled sound and consistent with every tag and non-tag, the merge exact, the record byte-untouched, no ADR owed; the architect's four should-fixes at close applied by the custodian in one commit (0e062c1: §15 counts distinct attempts and names the setter; the predicate declared the custodian's stated choice in §21c's form, yours to adjust by a one-line note; the retroactive tagging disclosed — the checker's attempts 2 and 4 rest on the dispatches, not their notes; two paraphrases marked), line-count neutral in AUTONOMY.md; main merged (ee66724) with records 73–74 untagged (mechanism documentation is not a record round, the architect's landing note); evidence pr 87 (87afadb). The strip on the branch: P3b 5, LOD 4, the checker 3, this piece 1, total 13, target 0.

108. **[The RESIDENCY note STOPPED after two failed reads (Rule 7): the first draft's Proof sentence claimed the checker's baseline named it; the fresh draft cited the test-text exception as round 14 item 3 (it is item 2) and misstated the qualifier (the fixed reference profile's refresh rate became 'the one screen this piece's harness targets'). Under the 2026-09-18 directive's point (3) the architect reduces the note to references — the row handle, the pin at a main commit with its hash, the ruling by round and item, nothing restated — and it lands with the reduced record. The LOD reviewer's attempt-6 verdict (gate-log 69) adds one blocking finding to the LOD's erratum row: eight cross-file cites into `engine/tests/lod_tier_builder.rs` from Amendments 8 and 11, stale since 43a3039 / a3f5f2e / d6073d8, certified as resolving by a claim widened past the three re-reads' survey; the remedy is one append-only row retiring them by item. Under the directive's point (5) the erratum rows for P3b (one clause on engine Amendment 11's line; two rows closing gaps 1 and 3) and the LOD (the per-span counts; the tool row naming fbf3d55; the eight cross-file cites retired by item) are written by the architect as references and hashes only, resolved per row (round 15 (a)–(b)), executed by a fresh worker with the resolution step, and then P3b, #84 and — after its one build-fix round — the checker land through PRs for the click.]** Filed 2026-09-18; nothing merged. **Landed for the click, 2026-09-18:** the errata executed exactly (P3b engine 13 / shell 16 at 46cde2c after merging main at c88e9b0; LOD Amendment 14 at f554dbb after merging main at 31caecf; the reduced RESIDENCY Amendment 28 at 875e839), each with the resolution step run before its commit — PR #86 (P3b), PR #84 (the LOD, body refreshed), PR #85 (the note).

107. **[Both re-scopes failed their gates on sentences of the consults' own prose; the checker is STOPPED under Rule 7; the mechanism needs one more step. P3b (`cut/briefa-p3b` @ 959bd08, attempt 6): architect PASS (64) — the record reads cold as its own resolution point, append-only holds as a zero-deletion proof, the pins resolved by name; reviewer FAIL (65) — byte fidelity to the consult exact and every other row resolved, but one engine row mirrors a shell row and attributes to Amendment 11 a 'five classes' miscount it never made (the consult's engine block, `state/consults/2026-09-17-p3b-re-scope.md`, written as a mirror and never resolved against the engine site). LOD (`engine/lod-tier-builder` @ bd8f1f9, attempt 6): architect FAIL (67) — two sentences of its own consult false at the commit: the per-span line counts (nine for seven, four for six, net neutral, no cite moved) and a row about the nine hash references that the round's own checker run contradicted; the reviewer's verdict pending at filing. Both workers executed every step exactly and reported the contradicting evidence; by round 14, item 1 both return to you as mechanism findings. CHECKER (`governance/verify-quotes` @ fbf3d55, attempt 4 FAIL/FAIL, 66 and 68, the fresh count spent): the build closed every attempt-3 finding — the grammar proven against the real records, a non-match no longer silent, the tool's own asterisk bug fixed so the `:220` entry left by the ratchet's mechanism (18, not 19) — and then failed on what it had not read: merging main turns the check RED on the six byte-copied markers your round-12 and round-14 adoption text carries inline (the tool reads only the next line; your bytes are exact); two more reds land with #84; the three end-to-end tests read branch refs that die at merge; a `site.mjs` regression renders a panel from an unparsed file; the record carries ten bare self-lines under the rule you adopted the day before (the branch's tree predates it), one misquote of its own Amendment 7, and the custodian's brief item (d) attributed to you; two baseline reasons say the consult was never tracked, which `state/drafts/` made false. THE NOTE: the first RESIDENCY draft failed its read (a Proof sentence claiming the baseline names it; four should-fixes); re-drafted whole on `record/residency-125-note-2` @ 206b756 by the custodian's decision — a committed amendment is never edited and a correcting append is the failure class — with the first branch left for your disposal; its second read is running. FOR YOUR WORD (round 15, the proposed clauses byte-copied from the gate reports into `state/questions/round-15.md`): (1) the mechanism — the resolution step in the consult mechanism, the per-row resolution before delivery, the tool-commit clause, the source-comment reference form, the durable-reference clause, the restore bound, and gap 2 (class 1 for a withdrawal row); (2) P3b and the LOD — one appended erratum row per document (P3b: the one clause plus two rows closing gaps 1 and 3 against round 14; LOD: the per-span counts and the tool row naming fbf3d55), each sentence resolved against its site before delivery, executed by a fresh worker, then one scoped read of the rows only, then the PRs; or an amend of the unmerged closing commits on the `:1371` precedent, which only you can license; or hold; (3) the checker — hold until P3b and #84 merge, then one round: a closing amendment by re-scope, per-row resolved, plus the bounded build fixes (the marker's reproduction on its own line; a route for hash findings in immutable text; tracked fixture lines with positive assertions; the `site.mjs` revert; the `//` continuation; the header gaps) and the baseline re-derived against the final main, under a fresh count; or the same now; or drop the widening; (4) acknowledgements and one hygiene ruling — `:220` gone from the baseline (the round-14 Applied text now describes a state the tree lacks; a ledger note corrects it when the checker lands); the note's first branch abandoned; the LOD's four hash-reference findings in immutable Amendment 12 text (owed-correction for `:545` via (g); `:541` genuinely unfixable); the six markers in your adoption text (the tool moves); and the untracked e2e reports cited in immutable P3b text — accept the disclosure and date the untracked-Authority rule non-retroactive.]** Filed 2026-09-18; nothing merged; PR #84 open and held; the P3b and checker branches pushed. **RULED 2026-09-18, round 15:** (1) all seven clauses adopted byte-copied; (2) both closes HELD; (3) the checker held until P3b and #84 merge, then one last round whose closing amendment is references only — a further record failure has the architect reduce it to references and the build lands; (4) acknowledged, the abandoned note branch deleted, the untracked-Authority rule dated non-retroactive with the evidence-versus-Authority distinction.

106. **[Both held records have an architect's re-scope; the checker's last attempt runs; four things for your word. P3b (`cut/briefa-p3b` @ e53bb97): attempt 5 FAIL/FAIL (gate-log 59–60) — everything from attempt 4 resolved in content, then the append's own four-line insertion under §9 moved every later line and its superseded index cited the wrong lines throughout; one cite fixed by editing inside Amendment 6; a ledger row naming the wrong entry; a seven-word phrase of the custodian's quoted as yours. Held under round 12, item 2. The architect's re-scope is on record verbatim at `state/consults/2026-09-17-p3b-re-scope.md` (1a0251461aa2): one closing amendment per document — shell Amendment 15, engine Amendment 12 — as a state-of-the-record list keyed by amendment and item, no line cite into its own file, no ledger line cite, no reproduced text, the engine byte at `:1371` restored so append-only becomes a zero-deletion proof; thirteen worker steps; four questions. LOD (`engine/lod-tier-builder` @ d6073d8, PR #84): re-read 3 FAIL (63) on the identical mechanic — the round's own two-line pointer inserted above Amendments 1–11 moved every self-cite by two, four cites inside immutable amendments staled unrecorded, an unmarked reproduction dropped a condition, the class misfit self-disposed. Held under round 13; the LOD's re-scope takes the same form — §10 Amendment 13 as a state-of-the-record list (recorded verbatim in the custodian's scratchpad, to be committed under `state/consults/` when #84 lands: its cites name test files not yet on main) — with three decisions of its own: the pointer d6073d8 inserted under §10's header stays where it is and is recorded with the two-line shift it caused (moving it would delete committed text and renumber the section a third time); the pointer's home is the closing amendment's head and foot, this document having no P6 sight list; and two spans of test text — the mutation record's second pin at 43a3039 and the `#[ignore]` message's missing qualifier — are corrected in the same commit under line-count neutrality, so no cite into that file moves. CHECKER (`governance/verify-quotes` @ 5ba6870): attempt 3 FAIL/FAIL (61–62) — the hash check's grammar matched none of the nine references written today and a non-match was silent; three ledger line cites in the baseline; and a DISCLOSURE: the `engine/ADMISSION-PREREGISTRATION.md:220` entry you accepted in round 12, item 4 as unfindable-by-construction is a tool false trigger — its source is tracked at `state/NEXT-CUT.md:111-127` and the checker's line-initial `*` strip caused the mismatch; the fresh count's last fix round is running. FOR YOUR WORD (round 14): (1) the execution path for both re-scopes — a fresh default worker executes each consult's steps exactly (byte-copy, hashes, the one restore, any comment-line edits the consult allows), then both gates under a fresh count; the worker adds nothing; a FAIL there is a finding about the mechanism and returns to you, not to a worker (recommended); (2) three clauses byte-copied into `state/questions/round-14.md` from the consult — class 7 (an evidence-driven sight-list addition), clause (a′) (ledger text with no round or item cited by entry number; no ledger passage pinned by path:line), the root-cause rule (no bare `:line` into a file the same commit edits; self-references by section, amendment and item; a line that must be carried is pinned at a commit the citing commit does not create) — and the `:1371` restore rather than an exception (all recommended); (3) the baseline: accept the corrected disposition of `:220`, and rule `frontends/shell/RESIDENCY-PREREGISTRATION.md:125` under round 11's rider — its elided span carries a negation about power-saving and variable refresh bearing on the budget the piece scores against — as an owed elision correction to the RESIDENCY piece (recommended) or as tool-false-trigger with the negation named; (4) `state/drafts/` is untracked in a public repository, yet `engine/ADMISSION-PREREGISTRATION.md:7` names a file in it as the preregistration's Authority and ADR-032 cites it as binding — track it (publishing the drafts) or have the citing records declare the Authority out of tree by appended notes.]** Filed 2026-09-17; nothing merged; PR #84 open and held; the checker's branch pushed. **RULED 2026-09-17, round 14:** (1) both re-scopes executed by fresh workers, steps exactly, then gates under a fresh count, a FAIL returning to the human; (2) all three clauses adopted byte-copied, the restore by row, class 3's test-text exception, plus the human's addition — a pin is historical and never silently read as current; (3) :220 accepted as corrected, :125 owed-correction; (4) the drafts tracked under state/drafts/ on three conditions (the custodian's sweep against entry 49's classes; two-line status headers; the twenty citing files updated in the same commit) and the standing rule: nothing tracked may cite an untracked file as Authority. **Sweep recorded 2026-09-18 (condition (a)), the custodian's, against `PUBLIC-AUDIENCE-AUDIT.md`'s classes and its section-2 search terms over the 23 files: username-bearing paths — `C:\Users\Public` (11) and two placeholders (`$name`, `<name>`), none personal; hostnames, IPs, UNC paths — none (the hits are two OIDs and a `desktop-app` topic); credentials — none (prose about token types only); names and identities — the commit-identity line `chris` / `chrys92d@gmail.com` once, in Brief A's header; tool-internal paths without a username — `.claude/worktrees/release-0.1.0` (2, the class the audit cleared) and, in two decision notes, this session's scratchpad folder `%LOCALAPPDATA%\Temp\claude\…\<session id>\scratchpad\` named as where fetched pages were saved (class 5, carrying the session id, no user segment — the class the audit searched under F-1; left as record, disclosed here); class-6 prose candidates left as record for the human's word — "on a phone" once (`briefA-digest.md`) and "during the away window" five times. Same result as the human's on every private class; the tool-path mentions are the one addition, disclosed here and edited nowhere. Condition (c)'s mechanical consequence: `verify-cites` treats citing files under `state/drafts/` as archived record (their broken historical cites advisory, never gated — one rooted cite of a moved `SKP-V0.md` would otherwise have turned main red); two lines in the scan loop, disclosed here.**

105. **[LOD tier builder (`engine/lod-tier-builder` @ a3f5f2e, PR #84, CI green) STOPPED under Rule 7 at the post-PASS fix's record — two scoped re-reads FAIL (gate-log 52, 58) after the piece's own PASS/PASS and the tester's PASS. The fix itself stands — the fixture-regeneration race on a fresh runner closed by serialized, atomic regeneration in `engine/tests/common/mod.rs`; every prior disposition verified in the tree; the suite green (14 passed / 2 ignored). What fails is the record of the fix: re-read 1 — a misattributed quote and an undischargeable rename claim (both corrected byte for byte at a3f5f2e); re-read 2 — the proof test's RECORDED MUTATION comment quotes the `.expect("lock")` line the same commit replaced, and its recorded failure site `:187:5` now lands on a comment, repeated in Amendment 10 (c) whose `common/mod.rs` cites shifted too; five should-fixes (a new 'every exit path' universal — an abort or a kill leaves the fixture at its aside name; S3 absent from the record though the operator-facing ignore message changed; the ignore precondition under-inclusive against the absolute fixture path; 'this same absolute path' over-states the kernel harness; class 4 glossed rather than the template's definition). The checker gap the reviewer names — `verify-mutation` clears its pending state on a multi-line `#[ignore]`, so the proof test is invisible to it — is PLAN node `governance-verify-mutation-multiline-attrs`, already proposed. The class is P3b's: each correction round added prose that needed its own correction. OPTIONS: (1) one final fresh-worker correction under round 12's mechanism — the comment's quoted line brought to the current text, the panic site labelled with the line numbers it was observed at, Amendment 12 recording the shift of Amendment 10's cites by reference within the ceiling, the five should-fixes closed, a superseded index — then re-read 3; a further record FAIL holds #84 for the architect's re-scope of the record, mirroring your P3b ruling (recommended); (2) the architect re-scopes the record now — a shorter amendment set replacing Amendments 10–11's prose with references — before any worker; (3) land #84 now with the record's defects owed in the PR body — a waiver of affirmative PASS on a record, refused for P3b for a reason that applies less here (no ADR acceptance rests on this record) but still yours to give.]** Filed 2026-09-17; PR #84 open, CI green, nothing merged. **RULED 2026-09-17, round 13:** option 1 — one final fresh-worker correction under the mechanism, then re-read 3; a further record FAIL holds #84 for the architect's re-scope of the record; option 3 refused (a mutation record quoting a replaced line is an unverifiable proof, not an owed nit).

104. **[Brief A P3b (`cut/briefa-p3b` @ aa72245) STOPPED under Rule 7 a second time — the fresh count spent (fresh attempt 2 FAIL/FAIL, gate-log 56–57; eight FAILs over four gate rounds, every one on the record; the build green throughout — 627 cargo / 1024 vitest, T10 satisfied in the real app). The round resolved every prior finding: the reviewer script-compared the ADR-016 span, resolved the seven code cites at HEAD and the session-log lines byte for byte, and confirmed additions-only commits. What failed is new, in two classes. (i) STRUCTURAL: the merge of main put rounds 9–10 at the top of the newest-first ledger and moved every later ruling down eight lines; every line cite into the ledger (22 sites across the two preregistrations, by grep at aa72245) is stale now or at the next merge, two of them under 'verbatim' — a line cite into a newest-first file is perishable by construction. (ii) FIDELITY, inside the new text only: engine Amendment 10 quotes Amendment 9's introduction cut before the parenthetical in which Amendment 9 discloses that it quotes a span, then charges Amendment 9 against the broader claim (the round-11 rider, the cut unmarked); shell Amendment 13 introduces 162 characters of a 766-character line as 'the pre-edit line … byte for byte', twice, cut unmarked at both ends — the same defect Amendment 10 charges against Amendment 9. Four should-fixes beside them (a miscount of Amendment 12's corrections against its confirmations; a 'byte-untouched' header clause true only under a reading Amendment 13 denies to the line it judges; 'no class marker' false — the amendment's header marks (b) a sight-list addition; Amendment 7's line-3 cut larger than stated) and two carried items (`kernel/src/skp.rs` `:797-803` on `cut/briefa-p3b`, cited `:796-802` in §12e; Amendment 10 (a)'s 'forty-six resolve' count). The checker reported zero findings in the new text — a truncated quote still occurs in its source; the floor clause held as ruled. The pattern named in entry 103 now stands at six gate rounds and eleven of today's thirteen FAILs. MECHANISM, for adoption in template §10, both checklists and AI_DEVELOPMENT (entry 103's proposal made concrete): (a) ledger rulings are cited by round and item — 'round 10, item 1' — never by line (the checker learns to resolve that form as a follow-on); (b) a passage that must be invoked is cited `path:line[-line]` with the span's sha256 and is reproduced only when byte-copied by a script and marked 'byte-copied from path:line'; the gate recomputes the hash — one command until the checker does it; (c) custodian briefs reference binding text by path:line only and mark anything else paraphrase; (d) a correction states the defect, the corrected reference and the proof in at most three sentences and never restates an earlier amendment's claim; (e) a correction round ends with a 'superseded as of this amendment' index, and 'read the last amendment first' joins the P6 sight list. OPTIONS for P3b: (1) one final fresh-worker append under the mechanism — shell Amendment 14 / engine Amendment 11 reproduce no text: every ledger line cite converted to round + item, the two cuts replaced by references, the carried items and the should-fixes closed in three sentences each — then gates under a fresh count (recommended: the two classes left are the two the mechanism removes, and a round that adds prose has now failed four times); (2) land now — a PR on aa72245 with the stale cites and the two cuts named in the body as owed to a follow-up record piece — which waives 'gates to affirmative PASS', a waiver only you can give; (3) hold P3b until the checker's hash check lands, then (1). Entry 103's questions ride in the same round: the checker's own record round under the mechanism with the hash check as its one named code addition, and the two baseline additions (`engine/ADMISSION-PREREGISTRATION.md:220`, a G-A1 gate-condition blockquote; `frontends/shell/POLISH-87-88-89-PREREGISTRATION.md:113`, the FIND′ line introduced 'verbatim') for your word under round 11's condition (b).]** Filed 2026-09-17; nothing merged; both branches pushed; #84's second re-read in flight. **RULED 2026-09-17, round 12:** item 1 — the mechanism adopted, all five, with riders on (b) (reproduction the exception: the default is a path:line reference with span hash and no reproduced text) and on (d) (three sentences a ceiling, not a form); item 2 — one final fresh-worker append under the mechanism, fresh count, then gates; a further record FAIL holds P3b for the architect's re-scope of the record; option 2 refused (the ADR-016 acceptance rests on this record).

103. **[`verify-quotes` STOPPED under Rule 7 (attempt 2 FAIL/FAIL, gate-log 54–55) — the BUILD passed every finding of both gates on both attempts; the piece's own RECORD failed the rules the check enforces: an elision in its Amendment 6 spliced across your round-11 rider and my "Applied" text, dropping the rider's negation and condition, and used to justify a disposition; my filed question text attributed to you ("two are honest elisions or drift…" is entry 102's question, not your answer); a quotation of AUTONOMY §21c — "count everything non-generated" — that exists nowhere: it is a sentence from MY dispatch brief, lifted as the section's words; the §21c figure understated by one test file (true 1395); and 21 baseline entries stamped `ruling: round 11` where your ruling accepts 17 — two of the four extra fall out at landing (branch-staleness artifacts, findable on main), two do not (`engine/ADMISSION-PREREGISTRATION.md:220` and `frontends/shell/POLISH-87-88-89-PREREGISTRATION.md:113`, both real pre-existing mismatches found by the fix round's stricter normalization) and need your word under condition (b). The reviewer also proved the same-document exclusion sound (tree-wide blanking adds 15 genuine failures) and its residual hole live (a misquote reproduced in a DIFFERENT document's correction still verifies), owed as a disclosed gap. THE PATTERN, stated plainly: five gate rounds today failed on the fidelity of newly written amendment prose across three pieces — P3a's record, the LOD fix record, P3b twice, the checker twice — with fresh workers and self-checks, while every build and suite passed. Each amendment restates, quotes and cites; each restatement is a new chance to misquote; corrections add prose that needs its own corrections; the checker cannot see attribution, splices or elisions. My own hand is in it three times: the §2e ADR-016 quote I filed, the NEXT-CUT row I shifted, and the brief sentence quoted as §21c. MECHANISM PROPOSED for your ruling: (1) QUOTE BY REFERENCE — an amendment that must invoke a ruling or a binding sentence writes `path:line[-line]` plus the span's sha256 and reproduces NO text unless byte-copied by a script and marked "byte-copied from path:line"; `verify-quotes` gains the hash check (mechanical, unambiguous, no normalization); the template's §10 and both checklists say so; (2) BRIEFS — the custodian's dispatch briefs reference binding text by `path:line` only and mark anything else as paraphrase (recorded as my rule, in AI_DEVELOPMENT); (3) FEWER WORDS — a correction amendment states the defect, the corrected reference and the proof in at most three sentences. OPTIONS for the checker: (a) re-authorize one fresh-worker record round (Amendment 7: the four findings, the 17→19 reconciliation with the two additions for your word, the reported-vs-enforced halves of condition (b) stated, the should-fixes) under a fresh count, then attempt 3 — with the quote-by-reference rule applied to that amendment as its first use (recommended); (b) land the build now with the record's defects named in the PR body as owed corrections (the build is what CI runs; the record is what you read) — against the discharge fence; (c) hold. And your word on the two additions: (i) accept both under round 11 (each is a real pre-existing mismatch with an honest disposition), or (ii) refuse and have them corrected by appended notes first.]** Filed 2026-09-17; nothing merged; the branch stays pushed; PR #84 and P3b run beside it. **RULED 2026-09-17, round 12:** item 3 — one fresh-worker round under the mechanism, fresh count: Amendment 7 plus the hash check as the round's one named code addition, every sentence attributed to its author, then gates; item 4 — both baseline additions accepted.

102. **[`verify-quotes` (round 10's tooling piece, `governance/verify-quotes`; attempt 1 FAIL/FAIL, gate-log 49–50, all findings correctable inside its five files; a fresh Sonnet worker is on the fix round) — three things only you can settle, raised by the architect. (1) THE BASELINE. Built, the check finds 17 passages on main that do not match their named source. Both gates read every entry: about ten are unfindable by construction (session logs, gitignored e2e output, an architect consult's text, a runtime-templated string); one is a tool false trigger; two are honest elisions or drift in a rolling ledger file since overwritten; two are re-typed nested quote glyphs in binding ruling text (`engine/ADMISSION-PREREGISTRATION.md:949`, `:993` — to be corrected by appending in P3b's record round, not baselined); one sits in an accepted, immutable ADR (`ADR-020:403` adds a period the ruling does not carry — an appended ADR note owed). No entry waives a substantive misquote of binding text. The instrument: `scripts/plan/verify-quotes.baseline.json`, printed as advisory on every run, matched by file + line + words, unmatched entries reported, new mismatches still FAIL — a recorded ratchet, not a waiver; its entries are the worker's classification pending your sight (the tool no longer says "human-reviewed"). Options: (a) accept the baseline as a ratchet on those terms, its 17 entries listed in the PR body for your click (recommended); (b) no baseline — every offender corrected by appended notes first (two ADRs and five preregistrations, all append-only records) before the check can gate CI; (c) hold. (2) THE RULE VS THE TOOL. Your round-10 rule is "byte-for-byte"; the tool normalizes presentation (whitespace, markdown emphasis pairs, comment prefixes, quote glyphs, dashes) and, for a passage with no `path:line` beside it, proves only that the words occur in the tree's non-quotation text — a floor, not source attribution. The architect's proposed clause for both checklists: "`verify:quotes` green does not discharge this rule — the gate resolves each verbatim passage against its named source; the check is a floor, not the proof." Options: (a) adopt the clause as written (recommended); (b) instead tighten the tool to require the named source for every quote (it would red-line the corpus: most rulings are cited by round and item, not by path — a design change past the piece); (c) hold. (3) ELISIONS. Read literally, "byte-for-byte" fails an honest quotation that marks an omission with an ellipsis (e.g. `frontends/shell/RESIDENCY-PREREGISTRATION.md:125` over `docs/08_Testing.md:5`). Proposed clause: "an elision marked `…` is honest quotation; each retained span must match byte-for-byte." Options: (a) adopt (recommended); (b) no elisions in verbatim quotes — quote whole sentences or label a paraphrase; (c) hold. Also for your approval, carried from the 2026-09-17 directive (proposed, not applied): (i) `.claude/agents/worker.md` and `tester.md` report formats gain one line for the model observed, any override with its reason, and any context handoff, and the four-class pre-gate self-check as a required report item; (ii) one `AI_DEVELOPMENT.md` bullet pointing at `state/directives/2026-09-17-dispatch-efficiency.md` with the operating rule in five lines. (The third proposal — a cite-content advisory in `verify-cites` — is superseded by `verify-quotes --show-cites`.)]** Filed 2026-09-17; the fix round proceeds regardless (it does not depend on these); the tooling PR waits on (1). **RULED 2026-09-17, round 11:** (1) the baseline accepted as a ratchet with three conditions (dispositions with the correcting piece named; append-never — entries leave when corrected, none added except by a ruling; the count on the health strip); (2) the floor clause adopted as written; (3) the elision clause adopted with the rider (no elision may remove a negation, a condition or a qualifier — the gate reads across each `…`); (4) both persistent edits approved.

101. **[Brief A P3b (`cut/briefa-p3b` @ a955bee) STOPPED under Rule 7 — two failed gate attempts at the same step, both DOCS-ONLY, both on RECORD FIDELITY (gate-log 45–48). The substance passed at attempt 1 and was not re-judged: boundary 4 delivered on both routes (residency cleared, picks refused through a labelled state, the not-dismissible block, the owner's sentence beside the engine's fact); the three-valued dead-ticket refusal written against the real `SourceFactory` with an end-to-end test from the real shape and its real caller in the Tauri shell; T10 satisfied in the running app (runs 5/6/7 — green / the mutation red on the residency assertion by name / green — verified by both gates against the reports and the session log byte for byte); 27 recorded mutations; cargo and vitest green; no ADR edited. What failed, twice: attempt 1 — the engine's §12e split record (Amendment 6) still said T10 was never run, and cites in Amendments 4–5 had shifted by the driver's own hook; attempt 2, after a corrections round — the correcting Amendment 7 presents a paraphrase of the sentence it corrects as a verbatim quote; Amendment 6's quote of §12b's G-A2 ends "late batches dropped" (that is G-A3) where G-A2 ends "status text verbatim", so the list of what G-A2 still lacks omits its real third clause (the status text is a P6 wording decision of yours); `state/NEXT-CUT.md:106` cited for the P6/Part N row, which is `:107` — MY OWN doing: the "P3 (split)" row I inserted on 2026-09-16 shifted every later row by one and I never re-derived the cites that named them; four stale cites remain in P3b-new code comments and three corrected ranges are off by one. DIAGNOSIS (the directive of 2026-09-17, point 6, asks for one rather than a third identical attempt): (a) nothing checks quote or cite CONTENT mechanically — `verify-cites` bounds-checks line numbers and proves existence only — and this class has now failed three pieces this week (P3a attempt 2, the LOD builder attempt 1, P3b twice); (b) the same worker wrote every correction from a context of ~750k tokens across six rounds, and its correction rounds introduced new fidelity defects while closing old ones; (c) the P3b preregistration's own cites were filed by me against a tree I then changed (the NEXT-CUT row). MECHANISM in flight: a Sonnet-default pilot is writing `scripts/plan/verify-quotes.mjs` (a gated verbatim-quote check + an advisory cite-content listing) on `governance/verify-quotes`, no PR until you rule; run on the P3b branch it would name both misquotes. OPTIONS: (1) re-authorize the step (a fresh count): a FRESH worker on the configured default (Sonnet — the work is mechanical verification; escalation reason absent) with a six-line verified handoff, one appended correction round — the engine's Amendment 8 (the two quotes corrected, G-A2's three outstanding clauses stated, the NEXT-CUT `:107` correction), the shell's next amendment (the NEXT-CUT mirror, the four code-comment cites and the three ranges, the small-pan/no-query limitation added to §9's P6 sight list) — with the quote checker's output and a cite-content listing as its pre-gate self-check, then gates attempt 3 (recommended); (2) the same as (1) but wait for the checker to land on main first (it needs your approval to land); (3) hold. Not offered: landing with the misquotes recorded as owed — the record is what your ADR-016 click rests on, and a false record under a click is the class you closed in round 5.]** Stopped 2026-09-17; nothing merged; the branch stays pushed; PR #84 (LOD) is open beside it. **RULED 2026-09-17, round 10:** re-authorized under a fresh count — a fresh Sonnet worker, one appended correction round with `verify-quotes` + a cite-content listing as the pre-gate self-check, then gates attempt 3; `verify-quotes` lands as its own reviewer-gated tooling PR wired into CI; the verbatim-quote rule joins the gate checklists; record-correction rounds always go to a fresh worker.

100. **[Brief A P3b (`cut/briefa-p3b` @ 294404c) — its end-to-end driver T10 has failed three times in its own PRECONDITION, never reaching the owner-side assertions; §9 of the accepted preregistration requires T10 green before either gate. P3b itself is BUILT per §2 (owner-side invalidation on both routes; the three-valued dead-ticket refusal with its real caller; the §12e amendment; the four items carried from P3a's gates), 27 recorded mutations, cargo + vitest green. T10 (`frontends/shell/e2e/source-changed.mjs`): open the copy through the real app, touch its mtime, pan, then assert the session-ended block with no `engine.` prefix, resident vertices zero through the new counts-only hook `residentCounts` (the custodian's call, class-2 amendment: §6 named a counts-only read that did not exist), and a hover over a formerly-occupied pixel yielding the refusal, not an id. Three runs, all launched through the harness (`launched: true`, ownership verified, closed by PID, machine quiet, reports under `e2e/out/`, session logs read as content): run 1 timed out at a 60 s step bound (a driver bug, fixed); run 2's centre-anchored hover grid found no pixel that yields an id; run 3 — a bounded third attempt the custodian authorized on the in-tree A9' bisection (read-back-verified pixel find via `capturePixels`, moved to `e2e/lib.mjs` byte-identical and shared) — converged on an 80% non-background patch at buffer (1005,141) of a 1280×200 frame, but 11 of 25 neighbourhood pixels still touch background: at the fit-to-bounds camera the app opens with, NO interior pixel exists anywhere in that frame. That is exactly the half of A9' the authorized scope excluded — A9' wraps the bisection in a zoom-notch loop (real wheel zoom-ins until a pixel interior-verifies). What the three runs DID establish: the branch opens the copy and fills the canvas (10,000 features / 188,665 vertices, identical every run); `residentCounts` returns those real totals from the running app; the copy's sha256 is unchanged after every run. Never evaluated: S5a/S5b/S5c (the owner-side consequences — what P3b exists for) and the recorded mutation. Amendments 5–7 record every run verbatim. OPTIONS: (1) a fourth bounded attempt with the FULL A9' mechanism — the zoom-notch loop around the bisection, as `regression.mjs` runs it — then the three-run sequence (green / mutation red by name / green), gates after (recommended: the mechanism is the suite's own, proven in the K6 re-runs; the failure names the missing half exactly; nothing about the product is in doubt); (2) drop the hover precondition: assert S5c against the interior-verified pixel from the read-back — a weaker 'formerly occupied' grounded in `capturePixels`, not in a hover that showed an id; (3) leave T10 unrun and let the gates weigh the unit/integration evidence with this record — a deviation from §9 of a preregistration you accepted, so yours to grant, and P3b would then claim nothing about what an operator sees; (4) hold.]** Filed 2026-09-17; nothing dispatched; the branch stays pushed; PR #84 (LOD) is open beside it. **RULED 2026-09-17, round 9:** a fourth attempt with the full A9′ mechanism (the zoom-notch loop), then the three-run sequence and the gates; the pre-declared fallback is option 2 with the weakening named ("formerly occupied, not formerly hovered") and the owner-side assertions still evaluated; option 3 not available.

99. **[LOD tier builder (`engine/lod-tier-builder` @ 7e9b0aa) — two items from the architect's attempt-1 gate (FAIL, narrow; gate-log 38; the build itself passed every principle, boundary, operation-class and budget check). (1) A DEPENDENCY EDGE you have not seen (red line: dependency decisions are yours). The round-6 preflight needs a free-disk reading and `std::fs` has none, so the worker made `windows-sys 0.61.2` a direct `[target.'cfg(windows)'.dependencies]` entry of `spatial-engine` (feature `Win32_Storage_FileSystem`, one call: `GetDiskFreeSpaceExW`). No new package and no version moved: the crate is already resolved in both lockfiles on main; the diff is one manifest block and one lockfile line each; no new SPDX id; ADR-030's fail-closed licence step re-run (`engine/LOD-DEP-RESOLUTION.md`, its §7 on the branch); precedent `protocol/transport-bakeoff/src/memory.rs`; the architect verified the substance independently and Arrow is unaffected. Outside the round-3 approval's four named crates — hence filed, as entry 46's sibling was. Alternatives considered: a `sysinfo`/`fs2`-class crate (a NEW package — worse); spawning PowerShell on a shipped path (the in-tree free-space code is test/script-only for that reason — worse). Options: (a) accept the edge as recorded (recommended; the architect's recommendation too); (b) reject — the preflight your round-6 ruling requires cannot then be built on `std` alone and the ruling would need revisiting; (c) hold. (2) THE TIER-CACHE LIFECYCLE (the architect's finding; no ADR governs it): the tier directory is keyed by the source's content hash, so an edited source produces a NEW directory and the old one is orphaned — nothing in this cut removes it; at the 5 GB class each orphan is up to 15 GB (the 3.0× bound); the preflight will then refuse on a volume the cache itself filled, naming the source rather than the garbage; no total ceiling, no eviction rule, no operator view. Nothing accumulates today — `build_tiers` has no product caller until the selection piece. Options: (a) record it as a named, owed decision for ADR-031 (the architect's drafted skeleton carries it: a total-size ceiling declared at its site; an eviction rule — delete-on-supersede or LRU over directories; its runner, never a background job without a scheduler, a priority policy and a cancellation owner; what the operator sees) and a hard precondition of the selection piece — no product path calls `build_tiers` before the lifecycle is declared (recommended); (b) add delete-on-supersede to this piece now (a scope widening: its own amendment and gate round); (c) hold. Also recorded, no decision needed: tier building is Windows-only in this cut (the tier root is `%LOCALAPPDATA%` per your round-3 addition; on any other platform the root does not resolve and the free-space reading is `None`, so every build refuses, typed and fail-closed) — Amendment 8 dates it as a declared gap beside docs/07's macOS/Linux item, the way round 3 dated the geographic-CRS gap. (3) THE CALLER RULE ON `build_tiers` — the two gates DISAGREE, so it is yours (reviewer attempt 1 FAIL, gate-log 39). The module lands with no product caller: its only references outside itself are three test binaries, and the kernel arm's own comment says it is unreachable from the wire. The ARCHITECT ruled it inside the round-4 rule on the round-3 acceptance, quoting the preregistration — §1 "Tier selection — which tier a viewport draws — is not decided, designed or implemented here. It is renderer/shell work under its own gate", §2f "no tier is served to the shell at all — this piece builds and validates tiers in the engine and ships no selection, no residency change and no renderer code", §9 "none in this piece, and the reason is recorded rather than the row omitted" — and wrote: "Round 4 bites on surface a worker invents for a consumer nobody designed; it does not un-approve a piece the human pre-committed with its consumer named and gated." The REVIEWER: those lines state the piece's scope; none licenses landing an acting `pub` entry point with no product caller; on the rule as written it is blocking by name, and since round 4 postdates round 3 the exemption is the human's to grant, not a gate's. Options: (a) the architect's reading — the round-3 acceptance licenses the builder to land ahead of the selection piece; recorded in the PR body and in PLAN.yaml (the selection piece is the named caller; until it lands no product path calls `build_tiers`, which also keeps item 2's cache empty) (recommended — the round-4 rule was written against seams invented for consumers nobody designed, and this consumer was designed, named and gated before the rule existed); (b) the reviewer's reading — the branch stays unmerged until the selection piece supplies the caller and lands with it; (c) hold. Also recorded, no decision asked: your round-6 words "tiers.json + the prepare report" — there is no prepare report in the tree and §9, accepted in round 3, says this piece has no operator surface; Amendment 8 dates the prepare report as owed to the selection piece beside the two labels' shell surface, unless you say otherwise.]** Filed 2026-09-16; the builder's fix round (the kernel seam test, the stale comment, Amendment 8 — the Windows-only gap dated, the four declared values beyond §7, the prepare report owed — the reuse-path re-stat, the unread constant, the unmeasured "minutes", the §6 simplify-time instrument, the suite log) is in flight under the gates; the PR waits on items 1 and 3. **RULED 2026-09-17, round 8:** (1) `windows-sys` accepted as recorded, the `statvfs` counterpart owed with the macOS/Linux gates; (3) the architect's reading — `build_tiers` lands ahead of the selection piece under the caller rule's new second exemption clause ("a producer pre-committed by the human with its consumer named and gated may land ahead of that consumer; it exempts nothing whose consumer is undesigned"); (2) the lifecycle an owed ADR-031 decision and a hard precondition of the selection piece, not widened into this piece.

98. **[Brief A P3a (`cut/briefa-p3` @ 1fcc737, 7 commits, 64 files +4903/−104) STOPPED under Rule 7 a second time — attempt 2 of its own count FAILED both gates (gate-log 36–37). What PASSED, verified by both gates: every removal the split ruling ordered is absent; the three seams stand on the real shape (a real ticket redeemed through the factory the shell installs; the publish fixture and the terminal fixture byte-identical to the Rust pins); your round-5 string is exactly the guidance text, in its own commit; the post-check's bytes are reported from the read already done through the engine's trace marks with the ≤ 8 MiB bound named and no duration anywhere; `slice.rs` asserts `cancel_requested → cancel_observed` and prints the unbudgeted interval with its disclaimer; every declared accessor names a test that exists (11 grep-verified); Amendment 4 is append-only; 619 cargo / 1000 vitest green, zero warnings; no owner-side behaviour crept in. What FAILED — narrow, and all of one class again: (1) the ENGINE's own `SourceChanged` Display text (`engine/src/error.rs`, `:312-318` on `cut/briefa-p3`) still says "Everything read for this session is discarded" — the sentence your round-5 ruling took off the guidance is the primary text the operator reads on every route (the terminal banner; the camera pre-check refusal, which renders no guidance at all; the filter-panel refusal, where it sits one paragraph above the corrected guidance), false because P3a clears nothing, and now PINNED by exact equality on both sides (`typed_terminal_codes.rs`, `terminalShapes.ts`, the SKP fixture); (2) a test narration — "the degradation is shown, never silent" (`engine/tests/session_identity.rs`, `:621-623` on `cut/briefa-p3`) — survived the sweep Amendment 4 (i) records, contradicting `descriptor.rs` and Amendment 4 (ii) in the same diff; plus a stale intra-doc link and a stale mutation record naming the function deleted this round (`engine/src/identity.rs` `:64` and `engine/tests/session_identity.rs` `:118`, both on `cut/briefa-p3`); (3) `SourceDescriptor::without_modification_time_for_test` (`engine/src/descriptor.rs`, `:271` on `cut/briefa-p3`) — a `pub` constructor with only a test caller that ACTS (fabricates a degradation) and names no test, sitting four lines from the exemption the tree applied to delete `ordinal_is_physical_not_scan_ordered`; its duplicated literal means the shipped no-mtime branch has no test of its own words; (4) "reported per cancellation … never silent" is not true of the shipped build — `trace::start` has no product caller and `ENABLED` defaults false, so `POST_CHECK_BEGIN/END` record nothing in a real run and the accessor is test-only — while Amendment 4 (ix) says "Both obligations discharged" (the architect's one-line in-licence fix: carry the bytes + bound on the always-on session line at `kernel/src/lib.rs` (`:426` on `cut/briefa-p3`), which also gives the accessor a product caller; or withdraw the claim); (5) ten in-code cites of the round-5 rulings are off by one item number. THE PATTERN, third time: an amendment states as done something the tree does not do; the gates now catch it every round, but each round costs the machine a day. OPTIONS: (1) a fresh worker (new context) on exactly the five items above plus the reviewer's S-1–S-8 as it judges, then gates attempt 3 — the fix list is fully specified by both reports; (2) the same worker, same list (its context is very long; the over-claim recurred three rounds from it); (3) hold — you read both reports first (filed on request). Not offered: landing with the false Display sentence recorded — your round-5 ruling forbids a false status string on main. Custodian's recommendation: (1).]** Stopped 2026-09-16 (Rule 7, second stop); nothing merged; the branch stays pushed. **RULED 2026-09-16, round 7:** a fresh worker on exactly the five items, gates attempt 3 as P3a's last (a further failure on either gate holds P3a for the architect to re-scope); the engine's Display states the engine's fact only — "the source file changed while it was open (<component>)" — the consequence sentence is the owner's, added by P3b when true; the discharge-claim fence permanent in both checklists.

97. **[LOD tier builder, route B (node `lod-tier-builder-route-b`, branch `engine/lod-tier-builder`, 4 commits, 12 files +3522/−11, pushed, no PR) STOPPED by its own preregistration on a recorded deviation — the human rules before any gate. Built in full per `engine/LOD-PREREGISTRATION.md` §2 as amended (SimplifyVwPreserve only; the 3-tier metre ladder converted via the CRS's ADR-026 linear unit with factor + provenance + definition hash recorded; both CRS refusals; one GeoParquet per tier via the in-tree parquet 58 / arrow 58 in the source's own row order; the sidecar at `%LOCALAPPDATA%\spatial-ide\tiers\<hash>\` with plain-text `tiers.json`; `LodTierKey` all five members in `PartialEq`; found by path / admitted by key; a stale tier never served; lazy per-feature-cancellable rebuild; `lod.tier_stale` / `lod.tier_resident{tier}` declared with a `TierBatch` that cannot exist unlabelled). The dependency gate step re-run against the real workspace: 41 new lockfile entries, none moved, all permissive (I4 silent); every arrow*/parquet crate at 58.4.0 (I1 silent); the notice set's fail-closed licence-text set needs no fifth id. T1–T11 written with recorded mutations; T4/T5/T6/T8/T9/T10 green plus a test proving the rejected simplifier (RDP) emits the five known invalid polygons and VW-preserve none. **THE STOP (O8):** on `polygons-100k` the declared ladder's three tiers total 2.051× the source against the declared `LOD_TIER_SET_MAX_BYTES = 2.0×`; the builder refuses `engine.lod_tier_set_over_disk_ceiling` and keeps nothing (exactly §7's behaviour), so T1/T2/T3/T7/T11 — which assert over a built ladder — are `#[ignore]`d; all five passed in a local probe that bypassed only the set-ceiling check. Vertex reductions matched the spike exactly (O5 met; O1 zero-invalid held; I2 silent) — the ladder is bigger on disk than the route-A-derived expectation because the smallest tolerance removes little on this fixture while every tier still carries identity + a recomputed covering bbox + geometry. The worker did NOT raise the ceiling or shrink the artifact to fit (dropping the tier's bbox column would land at ~99.4 % of the ceiling — one compression change from red); recorded as §10 Amendment 5 (class 2, post-outcome, no figures in the document). **A second finding, no class fits it:** in `geo` 0.33.1 `SimplifyVwPreserve`'s epsilon is a triangle AREA (`simplify_vw.rs:63` "epsilon is the minimum triangle area"), not a length; §2c declares the ladder as a length converted by the CRS's linear-unit factor — numerically harmless on EPSG:2056 (factor 1.0, the only CRS §4 exercises) but wrong for any non-metre linear CRS (the factor must be squared), and "0.1 m" means 0.1 m². Also flagged for sight, all declared at their sites: two refusal ids beyond §7 (`engine.lod_tier_root_unresolved`, `engine.lod_invalid_output`), two bounds (`LOD_TIER_ROW_GROUP_ROWS = 8_192`, `LOD_READ_BATCH_ROWS = 1_024` — the read pull is the other uninterruptible window; T6 passes with the cancel check before it), one `EngineError::LodRefused` variant with its unavoidable arm in `kernel/src/skp.rs` (no wire change), and a dev-profile `opt-level = 2` for the geometry crates in the root `Cargo.toml` (build profile only; the libduckdb-sys precedent). OPTIONS on O8: (1) drop the set ceiling and keep the per-tier ceiling `LOD_TIER_MAX_RELATIVE_BYTES` (which held on every tier) — a class-5 amendment on your ruling; (2) re-declare the set ceiling by a preregistered measurement (the value's own revision rule) — the piece waits; (3) change what a tier carries (no bbox column) — fragile. OPTIONS on the epsilon: (a) declare the ladder's VW semantics honestly — the tolerance is an AREA in the CRS's squared linear unit, the factor squared, the values kept as written (0.1 / 1.0 / 5.0 m²) — what the spike measured; (b) keep length semantics by declaring area = tolerance² per tier (0.01 / 1 / 25 m²) — a different ladder, O5 re-measured; (c) hold. Custodian's recommendation: (1) + (a) — the per-tier ceiling is the one that carries a meaning, the set ceiling was an expectation derived from route A's sizes (route A is out), and (a) states what geo actually does with the numbers the spike measured.]** Stopped 2026-09-16 on Amendment 5; nothing gated, nothing merged; the branch stays pushed. **RULED 2026-09-16, round 6:** the 2.0× set ceiling withdrawn (its premise, route A, is dead) for the by-construction 3.0× bound + a free-disk preflight (`engine.lod_insufficient_disk`) + the built size disclosed; the epsilon declared a minimum triangle AREA in squared units with the factor squared and a synthetic non-metre test; the five tests un-ignored; then the gates.

96. **[Brief A P3 (the session-identity tier, node `briefa-p3-p6`, branch `cut/briefa-p3`, 3 commits over main 355ed9a, 56 files +4035/−81) STOPPED under Rule 7 — two gate attempts, both gates FAIL twice (gate-log records 30–31); no third round on the custodian's authority. What is VERIFIED FIXED by both attempt-2 gates: the typed code now reaches the data-plane terminal as a kernel-side prefix `"<code>: <display>"` (no wire field; the data plane's own contract licenses it; pinned both ends; both streaming arms drop tickets, cancel work and latch on it); the post-check's rule (ii) is effected with the flag written BEFORE any terminal (clean / error / cancelled paths tested; a cancel is never relabelled); pre- and post-check share one "changed or unreadable" rule; `GenerationRegistry` has 8 tests incl. two threads; the ticket map is bounded; your round-3 `crs.source = "format-rule"` ruling is applied exactly and stays clear of ADR-032; corrections 7–14 done; §12e Amendment 2 appended (+110/−0) and honest; 617 cargo / 995 vitest green; no perf words. What FAILED attempt 2 — the same shape as attempt 1, relocated: (i) **owner-side invalidation is not built and not recorded** — `state/NEXT-CUT.md:103` puts "residency cleared, picks refused" in P3's own row; both managers fire a new `onSessionEnded` callback that NOTHING in the product subscribes to (`App.tsx:1096` and `candidateArmSession.ts:889` construct the managers without it), so after `engine.source_changed` the pre-change geometry stays on the canvas and stays pickable; a test narrates "the owner … clears the residency it holds and refuses picks", which is true of the callback and false of the tree; the pre-check refusal route (a thrown SKP error) latches nothing client-side; Amendment 2 item 12 does not name the gap; (ii) **the publish code is discarded by the shell consumer** — `PublishError::code()`/`refusal_detail()` are correct kernel-side, but `frontends/shell/src/publish/formatPublishRefusal.ts:21` hardcodes `code: "publish-refused"` (the directory has an empty diff), so `publish.geographic_crs_not_publishable`'s guidance is still unreachable, and every pin-free publish refusal now renders `"publish.<code>: refused: …"` verbatim to the operator; the only parser of the prefix lives inside a test file; (iii) **the kernel-authoritative dead-ticket refusal is never wired** — `EngineSourceFactory::ticket_only_with_generations`, `SkpHost::generations()` and `SkpHost::invalidator()` have zero callers, two carrying false "Has a real caller" docs; the shell installs `ticket_only`, so the redemption guard always takes the `None` branch — and where it WOULD run, it fabricates a source change for any UNKNOWN handle (an aged-out or replayed `sh_` handle is told "its source was observed to have changed") — the exact collapse this cut already recorded and fixed elsewhere (Amendment 1 item 8; docs/01 principle 8); (iv) a `Drop` backstop comment claims the producer's post-check always ran (it does not join the producer; the behaviour is still fail-closed via the next pre-check). Should-fix carried by both gates: `unreachable!()` on a production open arm where the cut's own `InternalInconsistency` fits; two doc comments cannibalised by inserted functions (one now puts an ADR-016 §3–§6 cite on a glob helper); a broken intra-doc link `refuse_unreadable`; a recorded mutation that would not fail its test; the post-check's new position adds bounded file IO inside the cancel-acknowledgement window (undisclosed, unmeasured — no number claimed); the sibling `publish::preflight` site still crosses without a code. THE PATTERN, named for your judgment: three rounds in which the worker wrote each module-boundary seam to an interface the other side does not have, with a test encoding the imagined interface — producer→terminal (attempt 1), post-check→kernel (attempt 1), publish→shell (attempts 1 AND 2), manager→owner (attempt 2). The engine and kernel halves are sound. OPTIONS: (1) authorize ONE more bounded round (attempt 3) on exactly the items above, with a hard condition the gates can check — every cross-module path is exercised end-to-end in a test that starts from the real producer shape, and no callback, code path or pub item lands without a product caller (a grep both gates run) — then reviewer + architect attempt 3; (2) split P3: land the engine + kernel + SKP 0.3 half now under its own gate (with owner-side consequences and the publish/redemption wiring recorded as a P3b piece in a §12e amendment for your sight), and P3b as a separate shell/kernel-wiring piece with its own preregistration; (3) hold — you read both reports (gate-log 30/31; the reports are in this session's transcript and can be filed to `state/` on request) before ruling. Custodian's view: (2) is the honest split — the sound half stops accruing rebase debt and the unbuilt half gets a definition the gates can hold it to; (1) risks a fourth "imagined interface" unless the end-to-end condition is enforced; (3) costs a day of the machine.]** Stopped 2026-09-16 (Rule 7); nothing merged; the branch stays pushed at its attempt-2 head. **RULED 2026-09-16, round 4: "Split P3, on conditions"** — P3a lands the sound half with the unwired/false parts removed and the publish consumer fixed; P3b (owner-side invalidation, the dead-ticket refusal wired correctly, the §12e split amendment) under its own preregistration; no release includes P3a without P3b; the seam rule made permanent in the worker brief and both gate checklists.

95. **[Pan anchoring in the shell canvas — the grabbed point should stay pinned under the pointer through a drag, and it drifts slightly sideways. Surfaced at the 2026-09-15 sitting, Row 6 (c); the human verbatim: "when i pan i'd like for the pointer to be still in the exactly same position from where i pressed to start the pan, now it goes slightly sideways, but everything else is perfect." The felt verdict on the pan RE-PICK (entry 75 (1)) passed; this is a *distinct* anchoring-precision defect, not the re-pick. HYPOTHESIS, not yet confirmed by inspection: the pointer-space/scale family entry 86 fixed in the bundle viewer (a devicePixelRatio or backing-store-vs-CSS-px factor making the screen→world pan delta slightly off), here in the shell's Deck canvas — "slightly sideways" reads like a small consistent proportional error, to be MEASURED, never assumed. **MEASURED 2026-09-15** (architect consult → preregistration `frontends/shell/PAN-ANCHOR-PREREGISTRATION.md` → worker, two headless instruments): normal panning is geometrically **pinned — 0 measured excess drift** at every zoom/direction, there-and-back ~0. The devicePixelRatio hypothesis is **REFUTED**: at the host's DPR 1.0 deck's logical viewport, the drawing buffer and the CSS client box all coincide (ratio 1.000); WebView2 would not take a CDP DPR>1 override (disclosed limitation). The reproduce found instead a **distinct, larger defect — candidate 3**: a mid-drag origin recenter that desynced deck's uncontrolled pan on threshold-crossing zoomed-out pans (grab-point invariant broke ~1100–18160 buffer px, a runaway; a WebView2 page-close risk) — **FIXED in PR #78** (defer the recenter to gesture end; reviewer + architect PASS, gated against the preregistration). **The human's "slight sideways" is NOT that fix.** Its likeliest cause is **candidate 2**: deck's controller anchors the pan at the first pointer-MOVE, not at the press, so the grab sits a few px off where the press landed (standard mjolnir behaviour, not headlessly reproducible as a measured residual). PENDING THE HUMAN (asked 2026-09-15): (1) pursue candidate 2 — anchor the pan at pointerdown (a deck-controller change, fiddly, overrides standard behaviour); (2) land #78, re-feel at tonight's sitting with the recenter bug gone, then decide; (3) accept it (sub-pixel-ish standard behaviour). The **felt re-verdict on Row 6 (c) is the human's, at a sitting.****RULED 2026-09-15, round 1: "2. Re-feel first"** — the human re-feels the pan at tonight's sitting with #78's recenter fix in, then decides on candidate 2; nothing built on it meanwhile.]** Surfaced 2026-09-15; #78 fixes candidate 3; candidate 2 held for the re-feel (ruled).
...
92. **[ADR-029 + the operation lifecycle — the decision draft is filed for your sight
    (`state/drafts/post-tag/architect-consult-adr-029-operation-lifecycle.md`); three
...
90. **[Static CRT as a v0.1.1 build piece (your directive of 2026-09-13: "Static CRT: if the spike passed,
    ...
    **2026-09-14 — the sourced licence note (RULED 2026-09-14 question set C, C1) STOPS the piece and returns it to the human, per their own condition** ("If the sourced terms restrict static distribution or the System Library reading does not hold, the piece stops and returns to me."). The note: `state/drafts/decision-notes/static-crt-licence-note.md` (70 lines; 15 sources fetched read-only with URL, date and SHA-256, one unlocated — no standalone VS 2022 Build Tools EULA exists on Microsoft's license-terms index, so the EULA governing this machine's toolchain is marked unverified; two local licence files read; 25 quotes re-verified by literal grep; nothing built, no setting changed). Findings against the expected posture: **(i) NOTICE set — partly confirms:** the CRT joins the notice set under ADR-030's own reopen condition, not because Microsoft's terms demand notice; the premise that `libcmt.lib`/`libvcruntime.lib`/`libucrt.lib` sit on the VS 2022 Distributable List is refuted — that list carries no `.lib` entry. **(ii) System Library — NOT confirmed (contested):** the FSF's only text on point blesses dynamic linking and says libraries "can only qualify as System Libraries as long as they're not distributed with the program", which reads against static linking; AGPL §1 itself carries no such sentence, so the contest is narrow and real. **(iii) the Excluded License clause — confirms, conditionally:** it bites only on source-code distribution, so it does not on its face forbid shipping CRT object code inside an AGPL program, but only while (ii) holds. The Windows SDK EULA expressly permits `.lib` files distributed as "the results of running such Distributable Code through a linker" (covers `libucrt.lib`); no equivalent VS-side grant was found for `libcmt.lib`/`libvcruntime.lib`; Microsoft's flow-down requirement sits unresolved against AGPL §10. **Status: returned to the human with the note's numbered decision points; the build setting does not land; entry 69's residue waits with it.**
...
79. **[Brief B stage B1 consult — three items routed to you, for B1's close and not before (Brief
    ...
    `state/drafts/post-tag/architect-consult-b1-adr-023-decision.md`; the ADR-004 review of
...
78. **[Repository description + topics — public-facing; applied only on your word: the drafted
    ...
    `state/drafts/decision-notes/repo-description-and-topics.md` with three alternatives and
...
77. **[Code signing for the Windows installer — your decision, docs only: the options note is
    `state/drafts/decision-notes/code-signing-options.md` (Azure Artifact Signing, the 2026
    ...
    **Post-tag note filed (2026-09-13, on your ruling "post-tag evaluate SignPath OSS first, Certum second"):**
    `state/drafts/decision-notes/signpath-evaluation.md` (92 lines; read-only, nothing applied for).
...
70. **[The post-tag ordering, and the drafts' open questions — for the sitting: which of the
    ...
    queue?]** The four drafts and two architect consults are under `state/drafts/post-tag/`
