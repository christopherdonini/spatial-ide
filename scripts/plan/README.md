...
**WITHDRAWN** (round 20 item 1; row grammar per round 22 item 3 and round 23 item 4): a claim reads
the same reference grammar again, this time marked `withdrawn-test` (never a bare `withdrawn`, which
already means something else on a round-15(g) withdrawal row) and restricted to one pinned line.
...
pinned reference, another file's path, no commit-id rev, or a line range — is a binding finding naming
that reason; an accepted row is further checked for its own pin conditions (the rev resolves to a real
commit, the pinned line's hash recomputes, that line names at least one test, and the rev is shown to
be an ancestor of `origin/main`, SKIPPED rather than failed when `origin/main` does not resolve at
all) — a row failing any of these is the same kind of binding finding, naming that condition's own
