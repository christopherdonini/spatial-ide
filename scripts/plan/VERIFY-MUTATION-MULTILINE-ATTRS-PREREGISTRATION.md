...
Authority: RULED 2026-09-24, question round 20, item 1 (entry 134); `governance-test-claims-withdrawn-marker` (branch `governance/test-claims-withdrawn`). This node's twelve historical property-test names (Amendments 2 and 4, lines 17, 39, 45, 61, 63) are obsolete under the union design (RULED 2026-09-24, question round 18, item 4), which makes "no loss" true by construction rather than by a self-comparing test. Each row below pins one claim line at this piece's own merge-base with `origin/main`.
...
Discharged: `node scripts/plan/verify-test-claims.mjs` at this piece's HEAD reads these five lines as `withdrawn` (0 findings), proven by `a_withdrawn_test_row_with_a_resolving_ruling_and_carrier_is_advisory_not_a_failure` (`scripts/plan/verify-test-claims.test.mjs`) and by the run recorded in `governance-test-claims-withdrawn-marker`'s own PR.
