...
// [{"relPath":"X-PREREGISTRATION.md","line":3,"name":"an_old_test_name_here","reference":"`X-PREREGISTRATION.md:3 @ <sha> sha256:<hash>`"}]" then "1 !== 0". This same mutation also fails
...
// [{"relPath":"X-PREREGISTRATION.md","line":3,"name":"an_old_test_name_here","reference":"`X-PREREGISTRATION.md:1 @ <sha> sha256:<hash>`"}]" then "1 !== 0" -- the reference's own `:1` shows the wrong-line span won.
...
// no @ rev must not exempt: [{"relPath":"X-PREREGISTRATION.md","line":3,"name":"an_old_test_name_here","reference":"`X-PREREGISTRATION.md:3 sha256:<hash>`"}]" then "1 !== 0".
...
// reference then falls straight into the pin-condition branch, which the fixture's own valid rev/hash/
// name pass) -- applied for real, run via `node --test scripts/plan/verify-test-claims.test.mjs`, then
// reverted; fails at its FIRST assertion (the claim wrongly lands in `withdrawn`): "AssertionError
...
// (!slice.includes(name)) continue;` in place of the `claimLine` check) -- applied for real, run, then
...
// the negation guard along with it) -- applied for real, run, then reverted; fails: "AssertionError
...
// containment test) -- applied for real, run, then reverted; fails: "AssertionError [ERR_ASSERTION]:
...
// hash check (`slice === null` alone gates the refusal) -- applied for real, run, then reverted; the
// row-level check itself now wrongly holds (`ok: true`), but the SEPARATE per-claim exemption
// (`findMarkedSpan`, unaffected by this mutation) still refuses on the same wrong hash, so the claim
...
// RECORDED MUTATION: in withdrawnRowPinCondition, drop the `isAncestorOfMain`/condition-(e) check
...
// return { ok: false, reason: 'refused: rev is not a commit' };` check -- applied for real, run, then
