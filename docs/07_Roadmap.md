...
- **Spatial indexing.** A revision-keyed in-memory index over the covering-bbox columns **is built** (`engine/src/index.rs`), and it is **deliberately not in the product plan**: the planner is `IndexUse::Off` and the index is reachable only through a `#[doc(hidden)]` experimental seam. That is a **measured** decision, not an unfinished one — `kernel/RESULTS.md`'s second section records that with the index in the path both filtered viewports were *slower* (first batch **35.6 %** and **17.5 %**; totals **21.7 %** and **15.5 %**), because the index answers exactly the predicate DuckDB's scan already computes and the bbox comparison is kept alongside it so the result set stays provably identical. **Scope, carried rather than left behind in RESULTS.md:** one machine, one file, one session, one data shape, **n = 7 per point per path**; the whole-file case is unchanged, as it must be. The index is correct and does not pay for itself on this shape. What is open is **an index that prunes actual IO**, which is a separate architect-first design with its own preregistered gate; no claim is made for it here.
