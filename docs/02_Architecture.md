...
- **ADR-025 (proposed — decision deliberately open)** — Publishing above the reader's ceilings: no publish-side refusal or warning exists when preflight could predict the artifact exceeds the bundled viewer's declared ceilings — kernel-proven (RESULTS finding 2) and operator-demonstrated at the UI (Part H8b: 3.3M rows / 6,636 partitions published unwarned, then refused typed by the viewer). Filed with the evidence cutting both ways (a hard preflight refusal would weld the writer to one reader's limits; silence lets the product's own flow produce artifacts its own viewer refuses) and NO recommendation; refuse/warn/stay-silent is the human's call at acceptance. No implementation licensed by the filing.
