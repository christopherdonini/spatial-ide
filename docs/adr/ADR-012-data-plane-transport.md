...
**What Phase 3 fixed, and what it then found.** The consumer-side SHA-256 left the timed path and measured throughput rose by more than an order of magnitude from Phase 2's 33–35 MB/s — **456.83–542.22 MB/s across the two admissible blocks**, which is the only range quotable here. Wider ranges appear in §20; every figure outside this one comes from a block §19.8 declares inadmissible, and §8 makes those "not 'worse data' — not data". The rise **confirms §17.7's direction but not its magnitude**: §17.7's own correction predicts 86–94 MB/s after removing the hasher, so at least 2,317.9 ms per run at configuration S is unexplained by the hasher alone (§20.9, §21 Q6). §19.3's paired symmetric estimator did what §16.5's could not: it classified a near-perfect null at configuration M as **equivalence** instead of rejecting it as confounded.
...
| **N=2** — concurrency | **inadmissible** | Two §19.8 invalidators bind — no producer facts and no hashing flag in the artifact. Three further §19.7 obligations are also undischarged (no verification transfer; aggregate resident bound vacuous by construction; N+1 refusal absent from the block), which are not themselves §19.8 invalidators (§21 Q2–Q5) |
...
**The N=2 result is recorded and is explicitly not evidence. Its figures are deliberately not reproduced in this ADR**, because a number printed in an ADR gets quoted, and §8 makes an inadmissible figure not data. §20.8 holds them.

Qualitatively: with two concurrent streams, Candidate **B** was substantially faster on aggregate throughput, **inverting the N=1 ranking**. Why that is worth carrying despite being inadmissible: the block fails on an **accounting** defect — the artifact lacks producer facts, the hasher flag and a verification transfer — and not for any reason to think the timings themselves wrong. A mechanism diagnostic (§20.8, "D1") eliminated the credit window, the JS main thread and the Rust producer by elimination, leaving the cause inside **WebView2's opaque WebSocket receive path** — §19.9 rule 6's UNKNOWN region, where rule 6 forbids counting an unknown as a win for either side. D1's own limits are load-bearing and stated here rather than left in §20: it is **not** a block of record, it ran on a **later browser build** than the blocks it diagnoses, and it ran **after** the record was read, so it is a hypothesis-forming exercise and not a preregistered measurement. Inadmissible-for-a-decision and informative-as-a-question are different claims; this is the second, and it points opposite to every other measurement in the phase.

**H1's grade changed in Phase 3, and the change is recorded here rather than by rewriting the requirements table.** §19.5 moved the cryptographic hasher off the timed path, so digest identity is established **per candidate per configuration on a dedicated untimed verification transfer** (8/8 at S, M and L; **not established at N=2**), tied to the timed runs by an identical structural digest — not on every timed run as in Phase 1. A timed run detects truncation, reordering, frame-boundary corruption and a length field that lies; it does **not** detect interior byte corruption. One residual is sharper than §19.5 stated: the verification transfer runs the same transport **roughly 17–20× slower** than the transfer it certifies, so a **rate-dependent** corruption — the class F4's silent WebSocket truncation belonged to — would be invisible to it.

**Two limits a reader must carry forward, both from §21:**

- **No Phase-3 block should be treated as reproducible between sessions.** A *valid* pre-fix L block selects A decisively; the post-fix L block of record is inconclusive at θ = −5.26 %. **Correction, from the third review pass:** an earlier version of this bullet blamed the five-line admission-slot change. The artifacts refute that. The **untimed verification transfers — a fixed, hasher-dominated, transport-insensitive workload — ran 26–35 % slower in the later session at every configuration and on both candidates.** The machine itself was slower, which accounts for essentially all of Candidate A's fall; the thing actually needing explanation is Candidate **B's insensitivity** to a machine-wide slowdown. The limit on trusting any single block stands and is if anything firmer, but its cause is a session effect §19 does not control for, not the code change (§21 Q1).
...
|---|---|---|
| H1 | Payload correctness — identical digest across adapters and runs, exact row count, CRS tag on every batch. *(Phase 3 lowered the grade at which this is established — see the Phase 3 outcome section.)* | A transport that delivers a *different* payload has not delivered the payload |
...
1. **The concurrency inversion is unresolved and points the other way.** At two concurrent streams Candidate B was substantially faster on aggregate throughput, inverting the N=1 ranking (§20.8). The block is inadmissible on an accounting defect, and the cause is diagnosed by elimination to WebView2's opaque WebSocket receive path — measured nowhere. **This ADR should be re-opened if a properly accounted N=2 block reproduces it**, and the hero slice sustains exactly the transient two-stream overlap `docs/05`/`docs/06` describe, so this is not a hypothetical configuration.
2. **The decision's only measured basis may be removable — but smaller batches do not remove it.** Phase 2 measured configuration S (~244 KB batches): B still paid 1000–1001 whole-payload copies per run, the copy having relocated from reassembly into the Arrow parse via misaligned chunk views. **Phase 3 measured the large-batch direction and it did not remove the copy:** exactly one whole-payload copy per batch per transfer at M (100/100) and L (20/20), against zero for Candidate A, in both sessions. An incremental Arrow reader that does not require a contiguous message might still eliminate it; **that remains untested**. If it can be eliminated, criterion 1 stops discriminating and this ADR should be re-opened rather than quietly retained.
3. **Throughput is uncharacterised for both candidates.** The workload is generation-bound at 97.2–98.9 %; neither transport's ceiling was measured. Any future throughput claim needs a re-run with a pre-generated payload that decouples generation from transfer. **No throughput-based claim may cite this ADR.**
4. **Between-session reproducibility is now measured, and it is poor.** Phase 3 obtained what Phase 1 could not: the same configuration run in two sessions. The verification transfers — a fixed, transport-insensitive workload — came out **26–35 % slower** in the later session, and configuration L moved from a decisive A-wins to inconclusive across it (§21 Q1). **No single block in this study should be treated as reproducible**, and no phase so far has had a control for this.
5. **One machine, one GPU, one Edge build.** Intel UHD 630 — not the GTX 1650 also present — and Edge 150.0.4078.105.
6. **Cross-platform validation.** Every figure is Windows-only. macOS/WKWebView and Linux/WebKitGTK are unmeasured, and `docs/07`'s hardware-validation gate covers them.
7. **Interruptible IPC channels remain unmeasured** — ADR-004 amendment 2's third candidate class. A future comparison needs its own harness run, not an argument from these results.
8. **Peer authentication on loopback** is unaddressed: the token authenticates a session, not a process. Any local process running as the same user can reach the port — and on the manual (non-`--launch`) path need not attack the token at all, since it can read `launch-url.txt`. The production transport must not write the credential to disk.
