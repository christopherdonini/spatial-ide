...
**The guard, and its reach, stated rather than assumed.** Two checks run at generation and at `check:dist-notice`, both throwing rather than degrading: every pinned file's sha256 is re-verified against the bytes on disk, and the manifest's work list is compared in **both** directions against the `third_party/` listing inside the pinned crate's own tarball; the resolved `libduckdb-sys` version is compared against the manifest's. The guard is directory-name- and version-level; it does not prove the tarball's sources are byte-identical to the tag's tree, and nothing here claims that.
