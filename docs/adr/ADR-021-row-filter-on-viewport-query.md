...
`autoload_known_extensions=false` at every connection open (`engine/src/pool.rs:423`;
...
`a_known_extension_reference_fails_closed_on_every_lease_class`,
`the_shared_configuration_sets_all_three_settings_on_a_fresh_connection`). The property now
reads: **the admission parser is statically linked AND runtime extension autoload/autoinstall is
disabled by configuration at every engine connection — no engine connection can fetch or load an
