# Wave 1, A3 (calibration): the launched prompt

- **Session ID:** session_01STxb5ot2J5RaYpnzffmSq6 (https://claude.ai/code/session_01STxb5ot2J5RaYpnzffmSq6)
- **Launched:** 2026-09-25T06:41:30Z, from claude.ai/code in the browser. The environment was Default and the repository christopherdonini/spatial-ide, on branch main at 9f764c8; that commit carries PR #120, and the dry run passed there first. The model was Opus 5.5 at Medium effort, as the composer and the session showed at launch.
- **Why the browser:** `claude --cloud` refused from the custodian's shell with "Error: --cloud requires an interactive terminal.", and created no session. claude.ai/code is the other launch surface the docs name (https://code.claude.com/docs/en/claude-code-on-the-web).
- **Assembly:** built by script per `state/cloud/wave1-prompts.md` §7, as amended by its deviation 1. A1 is copied in full; the branch name (the only place A1 carries its item id) and the TASK block are swapped for A3's; "[paste §3's schema, worker fields only]" is replaced by §3's worker fields, from `# WAVE1 REPORT` through the `### Stops (if any):` line. Read from `state/cloud/wave1-prompts.md` at 9f764c8.
- **Exactness:** 63 lines, sha256 a408c1d81ed3bd6552e8024312c7a76c93f093774dc83bdac7812da616eefe46. The same hash was computed from the clipboard (LF-normalised) and from the composer's paragraphs joined by newlines, just before sending.
...
cloud/wave1-A3 and push it. Open no PR. Edit no product source file.
