*Custodian's filing note (2026-09-25): the architect agent's revision of the test-claims follow-ups form to RULED 2026-09-25 — question round 23, item 4, filed as returned (extracted by script from the agent's hand-back; byte-identical). Everything below the rule is the agent's text. Section 1's form, its three placeholders filled by script at main d90f919 (both lines unmoved), is committed as `scripts/plan/TEST-CLAIMS-FOLLOWUPS-PREREGISTRATION.md` on `governance/test-claims-followups`, before any code.*

---

**Verdict:** pass with notes. The ruling resolves every OPEN marker. O1 takes the use-versus-mention route. Row position can be defined cleanly as one anchored predicate: the row grammar's own leading bytes (`state/consults/2026-09-24-withdrawn-marker.md` §1). Both mention lines on main fail it on their first bytes (`fourteen`, `## `). All five accepted rows on main pass it, and so does every fixture row built by `withdrawnFixture`. No backtick pairing is involved, because nothing can come before column 2. The hash-pinned fallback would add a tracked exemption file that the predicate makes unnecessary. I had no shell, so nothing below was run. No ADR is needed.

## 1. The form, `scripts/plan/TEST-CLAIMS-FOLLOWUPS-PREREGISTRATION.md`

Nothing in it is reproduced, so it needs no script marks. The three `{…}` placeholders in §2.7 are filled by script before the commit. `{MAIN_40HEX}` is main's full id at fill time, and each `{SHA256_n}` is `git show {MAIN_40HEX}:<path> | sed -n '<n>p' | sha256sum`. If either line has moved at that commit, stop.
...
7. Lines already on main (round 22, item 3's condition, by the method round 23, item 4's O1 line gives). None is edited. The two scanned lines that carry the token outside a backtick span are mentions under §2.1(a) and yield no finding. Each pin is historical: authoritative for the bytes at `{MAIN_40HEX}`. The tree is authoritative for the current bytes. This piece changes neither, so they agree at the PR head.
   - `scripts/plan/TEST-CLAIMS-WITHDRAWN-PREREGISTRATION.md:97` @ {MAIN_40HEX} sha256:{SHA256_97} — Amendment 2's Discharged clause.
   - `scripts/plan/VERIFY-MUTATION-MULTILINE-ATTRS-PREREGISTRATION.md:138` @ {MAIN_40HEX} sha256:{SHA256_138} — Amendment 7's heading.
...
No corpus. Unit fixtures only: temp git repos, as in the file's existing tests. Test 14 is built from real bytes: the two §2.7 lines, read by `git show` at `{MAIN_40HEX}` and checked against their pinned hashes inside the test. The tree runs are §5.
...
1. **Header.** The OPEN-markers line is gone. Authority adds round 23, item 4. "Drafted by" now reads at 3bb1bf2.
2. **O1, §2.1 and §2.7.** Use versus mention, with row position defined as §7's anchored predicate. Only row-position lines are attempts or can be accepted. §2.7 no longer corrects anything: its two lines are mentions, and their pins stay as evidence for P2(b) and test 14. §1 and §8 item 5 now forbid any landed-record byte. §8 item 6 bars the unused fallback.
3. **Colon.** The colon is left out of row position on purpose. A row that drops its colon is still an attempt, which closes the silent class the draft's alternative (b) opened.
4. **Precedence.** The exclusion keeps containment (§2.1(e)), so `a_withdrawn_test_row_also_marked_superseded_does_not_exempt_as_superseded` keeps its assertions. Only its comment's reason changes (§2.9). There are now two changed tests, not three.
5. **O2.** Both riders are read on a refused line, grammar reason first.
6. **O3, new §2.2.** Row-level (r), (c), (d) and (e) on accepted rows, in the same finding shape. A pin-failed row joins `invalidRowLines`. My row-level reading of (d) is "the pinned historical line names at least one test", because (d) is per-name and a row has no single claim in view. SUPERSEDED rows are not widened: the ruling's reason is scoped to withdrawal.
7. **O4, O5, O6, O7, O8, O9.** Each written in as recommended, at §2.10, §2.9, §2.6, §7, §7 and §2.10. O8's `budget_minutes` is re-estimated at 240.
8. **Tests.** Nine new tests: 11–14 for O1 (14 built from the two real lines) and 15–19 for O3. Nineteen in all.
9. **Predictions.** The old P1 (two findings) is gone. The new P1 is 0 findings at the last code commit and at the PR head. P2 is now the two-set enumeration. P3 counts 54 tests.
10. **Budget.** Re-declared: 520 + 2 lines becomes ≤ 720 over the three counted files, and at most 7 non-generated files becomes at most 6.
11. **Invalidators.** Split: a P2(a) mismatch stops; a P2(b) mismatch is recorded only.
12. **Block-on-sight item 7** (now 8) is narrowed to "in row position". A mention in this form is harmless under the rule.

## 3. Stop list

None. Round 23, item 4 closes every point the ruling was asked. Three readings are mine and are written in the form for the gate to judge. None is reserved to the human:
- the row-level meaning of (d) (§2.2);
- SUPERSEDED pin failures staying unnamed (§2.2's last sentence);
- the narrowing of acceptance to column-0 `- ` rows, disclosed in the README by §2.8(f). Main has no row outside that shape (P2(a)).

Files:
- C:\dev\spatial-ide\state\consults\2026-09-25-test-claims-followups-form.md
- C:\dev\spatial-ide\DECISIONS-PENDING.md
- C:\dev\spatial-ide\state\consults\2026-09-24-withdrawn-marker.md
