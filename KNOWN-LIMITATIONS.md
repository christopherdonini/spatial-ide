...
24. **The advisory source-change watch runs on Windows only.** Off Windows, arming a watch always
    ...
    <!-- engine/SOURCE-WATCHER-PREREGISTRATION.md §2a (paraphrase: off Windows, PlatformWatch::arm returns ChecksOnly naming the platform, and never Watching) and §1 (paraphrase, joining its "May not claim:" bullet with the "anything off Windows (tier note, §4)" item beneath it); engine/src/watch.rs (PlatformWatch::arm's non-Windows arm) -->
...
25. **A signal for an in-place write is not guaranteed before the writer's own handle closes.** The
    ...
    <!-- engine/SOURCE-WATCHER-PREREGISTRATION.md §0.3 (paraphrase, markdown stripped, of H2: "An in-place write is signalled no later than the writer's handle close.") and §1 (paraphrase, joining "May not claim:" with "any OS delivery deadline, event ordering against data-plane frames, or a latency"); Amendment 3 item 6 ("H2 by A1 passing", byte-exact) -->
...
26. **Only one ancestor above the watched file is watched, and a symlink retargeted after admission is
    ...
    <!-- engine/SOURCE-WATCHER-PREREGISTRATION.md §2a ("Unless P is a volume root (round 17 item 2, S2 as recommended), open P's parent, G, and issue its read", byte-exact) and §5 (paraphrase, joining "Declared unchanged:" with "the descriptor's comparison and refusal paths, the pre-check and the post-check") -->
...
27. **A rename that changes only a character whose uppercase form is not exactly one character is not
    ...
    <!-- engine/SOURCE-WATCHER-PREREGISTRATION.md §7 (paraphrase, its "Name comparison" table row, of "UTF-16 decoded, compared per `char` through `to_uppercase` where 1:1, otherwise exact; against the long name and the 8.3 short name" mapping to "a match. The residual goes to KNOWN-LIMITATIONS"); engine/src/watch.rs (`names_match`'s `fold` closure, the `(Some(u), None) => u, _ => c` arm) -->
