...
- **The GitHub release.** The human reports it published as a pre-release with the asset's hash verified after download. At this writing the repository's releases endpoint returns an empty list and `/releases/latest` returns 404, so no release is visible to the custodian under `christopherdonini/spatial-ide`; the URL is to be recorded here when it is visible (the tag is visible: `refs/tags/v0.1.0` → b391e43).
