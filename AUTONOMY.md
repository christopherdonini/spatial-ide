...
3. **A claimed-test-exists check** (`scripts/plan/verify-test-claims.mjs`) — where a document or a
   ...
   binding again the moment a node naming it is `done` (binding is sticky: a gate file any `done` node names is never planned, whatever other nodes name it). **Superseded claims (2026-09-18):** a claim at line L of file F is printed as advisory (`superseded — pinned by <reference>`) instead of failing when F's own text carries a `superseded`-marked hash reference to `F:L` at an explicit `@ <rev>`, whose bytes recompute and whose claim's own historical line (not the whole span) contains the claimed name, and whose `<rev>` is shown to be an ancestor of `origin/main` (SKIPPED, not failed, when `origin/main` does not resolve in the scanned tree at all) — an append-only record's own proof that the name is historical. It proves a rename happened, never that the replacement exists; that half is the gate's to read. **Withdrawn claims (round 20 item 1):** a claim at line L of file F is printed as advisory (`withdrawn — pinned by <reference>`) instead of failing when F's own text carries a `withdrawn-test`-marked hash reference to exactly `F:L` whose ruling and carrier citations each resolve against `DECISIONS-PENDING.md`; it proves the two citations resolve, never that the ruling names the removal or that the carrier carries the evidence — both are the gate's to read. **Every line in ROW POSITION is checked on its own (round 21 item 2; round 22 item 3):** a withdrawal attempt is a line whose first two bytes are `- `, the marker following immediately, then a byte outside `[A-Za-z0-9_-]` or the line's end (the colon after the marker is not part of this); a line carrying the marker anywhere else is a mention, never an attempt, and is neither accepted nor refused. Whatever the state of the claim on its pinned line and whatever its node's status, a line yielding no accepted row — no pinned reference, another file's path, no commit-id rev, or a line range — is a binding finding naming that reason; an accepted row is further checked for its own pin conditions (the rev resolves to a real commit, the pinned line's hash recomputes, that line names at least one test, and condition (e) above), and a row failing any of these is the same kind of binding finding, naming that condition's own reason; a missing or unresolvable `ruling:`/`carrier:` is read in the same call and appended after the grammar or pin reason, or stands alone when those hold. Neither kind of row-level finding is ever planned, and a grammar-accepted row that fails its own pin conditions or riders is not also reported as an ordinary claim finding on the same pinned line.
