...
- **Health strip** (§15): CI on `main` (badge), drift (`verify:plan`'s result at generation time), disk free, stray processes, open-PR age, waiting-on-human age — read from `site/data/health.json`, which `scripts/plan/health.mjs` writes on the dev machine; the strip shows the timestamp of its last refresh so a stale strip reads as stale.
...
The release asset is **the tagged commit's CI build**, downloaded from the workflow run on the tag, hashed, and the hash recorded in `RELEASE-<version>.md`; the dev machine is for headed work only. `RELEASE-DAY-CHECKLIST.md` §4 gains the step; `product-ci-shell.yml` gains a tag-triggered build that uploads the installer as a workflow artifact (tooling, reviewer-gated; no signing — entry 77 is open). **Done (2026-09-14, `chore(ci): release-artifacts-from-ci`):** `product-ci-shell.yml`'s `tauri-build` job now runs on a pushed `v*` tag as well as its existing triggers, uploads the installer as `spatial-ide-<tag>-x64-setup`, and writes the installer's file name, byte size and SHA-256 to the run's job summary (`$GITHUB_STEP_SUMMARY`) alongside the job log, so `RELEASE-DAY-CHECKLIST.md` §4 can compare a local hash against the run's own without re-downloading to compute it first.
