...
> was on a bandwidth-constrained connection: no installs, no fetches, no toolchain or browser downloads). Where a
...
**Four things to note, none blocking:**
...
4. **2026-09-02 — a second, smaller instance of this table's own line 20 finding** (the CI-log
   pointer to `CUT-STATE-adr009-checklist.md` above): three more commits (`de67713`, `8211723`,
   `0e4449c`, all within the viewport-residency cut, well after 2026-08-10) also reached an open
   PR (#16) without `Signed-off-by` — caught live by the DCO gate this time, not found by a later
   history sweep. Not rewritten: the hashes are load-bearing measurement-chain provenance cited
   verbatim by `RESULTS.md`. Retroactively certified by the human instead, in a PR #16 comment;
   the check stays red on that PR by design (the honest state, not a fixed one) — see
   `DECISIONS-PENDING.md`'s resolved entry 26 for the full resolution and the class fix
   (`AI_DEVELOPMENT.md` mechanics item 12; `CONTRIBUTING.md`'s local commit-msg hook).
5. **2026-09-06 — one more single-commit instance, resolved the entry-26 way per the human's
   entry-39 ruling ("no-rewrite entry-26-style"):** `3e653f0` reached PR #22 unsigned —
   `git revert --no-edit` adds no `Signed-off-by`, and the entry-26 fence sat INERT because
   `core.hooksPath` was unset in the working environment (arming it is now a boot step,
   `AI_DEVELOPMENT.md` mechanics). Not rewritten; retroactively certified by the human in a
   PR #22 comment; the check stays red on that PR by design (the honest state, not a fixed
   one) — see `DECISIONS-PENDING.md` entry 39.
6. **2026-09-07 — one username-bearing absolute path reached public history** (the class this
   table's line "No path names a user directory or this machine" defined and both earlier sweeps
   found empty): `spikes/residency-debt-fix-live-probe/probe-thrash.mjs:9` carried
   `C:\Users\Christopher\.claude\jobs\…` from `fc0b49f` (2026-09-06) until the fix-forward commit
   that follows `PUBLIC-AUDIENCE-AUDIT.md` F-1 (relative, script-derived paths). The string stays
   in public history — a rewrite is a named red line (`AI_DEVELOPMENT.md`); the account name equals
   the author's public first name, no credential, no machine name. Fix forward, ruled by the human
   (entry 49).
...
deferred until bandwidth allows (requires `npm install`); **counsel review** — per
...
`protocol/transport-bakeoff/web` audit when bandwidth allows; full trademark-register
search pre-1.0.
