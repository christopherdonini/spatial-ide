...
// the accurate form.
//
// **The scope sentence names FOUR sets, not three** (architect Q4, this fix batch). It listed the
// viewer, the packaged frontend's npm dependencies and the linked Rust crates -- an enumeration that
// was complete when the wider file had three sections and stopped being complete the moment the
// fourth landed. A reader of THIS file, inside a packaged install, would then have been told the
// beside-the-executable notice covers three things when it covers four, which is the same class of
// error "application-wide" was chosen to avoid. Nothing about the viewer SECTION's own bytes changes
// here: this is the header block, outside the "THE VIEWER" through end-of-viewer-section range
// `noticeByteIdentity.test.ts` holds identical between the two artifacts.
function viewerOnlyHeaderLines() {
  return [
    ...
    'frontend\'s own npm dependencies, the Rust crates statically linked into the application,',
    'and the third-party works inside DuckDB\'s own amalgamated C/C++ source tree --',
...
//
// **The ADR-030 reference states its own status** (reviewer should-fix S2, this fix batch). Every
// other reference to ADR-030 in this repository's code calls it "candidate (a)", which was accurate
// while the ADR was open; a conveyed artifact citing an ADR should say what standing that ADR has
// rather than leave a recipient to guess. The header now reads "ADR-030 (accepted at this piece's
// landing; candidate (a))" -- the acceptance is the custodian's own commit on this branch, appended
// separately, and the "candidate (a)" label is kept because it is what the surrounding code, the
// preregistration and DECISIONS-PENDING entry 62 all name.
function extraHeaderLines() {
  return [
    ...
    'tag; ADR-030 (accepted at this piece\'s landing; candidate (a)),',
    'docs/adr/ADR-030-conveyed-artifact-notice-set.md). It lists what was actually',
