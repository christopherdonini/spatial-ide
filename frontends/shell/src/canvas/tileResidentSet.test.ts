...
describe("planTileEviction (item D)", () => {
  ...
  // Close-out fix piece F1, pre-committed unit test 1 (entry 44, ADR-028's architect-gate
  // clarification 3 / Amendment 1's own geometric rule -- "never evict a tile intersecting the
  // current viewport"; NOT the accepted Decision's own item 3, which is cross-tile de-duplication).
  // This layer already enforces the
