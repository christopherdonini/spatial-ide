...
export class TileViewportStreamManager {
   ...
   * (`candidateArmSession.ts:1217-1278`) -- which this manager never sees a terminal for. Before
  ...
  private async mintAndStart(key: TileKey, tileKey: string, epoch: number): Promise<void> {
    ...
    try {
      ...
      // this dataset's session (`kernel/src/skp.rs:666-672`), and that refusal lands here -- where
