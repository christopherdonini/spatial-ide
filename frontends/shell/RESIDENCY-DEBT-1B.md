...
## Item C amendment — BS7's escape hatch taken (entry 47; 2026-09-10, appended AFTER entry 47's piece was built and gated)

Item C's Design ("re-run the declared pick-resolution threshold at the new zoom … never keep a stale id, never fabricate a coordinate") and BS7 remain true as written; they are what landed at the K6 re-aim recorded in `RELEASE-0.1.md`. BS7's escape hatch — "if honest re-evaluation needs more than re-running the declared threshold at the new zoom — e.g. a full re-pick because the feature under the stationary pointer changed — Item C exits to the decision queue rather than expanding 1b silently" — **was taken**: DECISIONS-PENDING entry 47 (the human's ruling, quoted verbatim in `frontends/shell/HOVER-REPICK-PREREGISTRATION.md` §2) rules a full re-pick at camera settle, preregistered in that file and built on `cut/hover-repick-settle` (merge held until after the v0.1.0 tag). From that landing, Item C's mid-gesture rule stays exactly as this record states it, and the settle-time re-pick is that preregistration's piece, not 1b's. The `WorkingCanvas.tsx:1221-1244` hover-site cite above is an archived-record cite: true of the tree of 2026-09-05, not re-derived here. Nothing in this record is edited.
