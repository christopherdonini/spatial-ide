...
//! block or pump inside `setup()`" mechanic, added the same day as this rewrite).
//!
...
//! everywhere else. `PROXY_DEV_SERVER` (`cfg!(all(dev, mobile))`) is inert for this reasoning: this
//! crate is desktop-only (`Cargo.toml` carries no mobile target), so that condition is always
//! `false` regardless of `is_dev`.
//!
//! **Why the mirror cannot disagree with Tauri's own choice.** Both `is_dev` here
//! (`tauri::is_dev()`, `src/lib.rs:308-310`: `!cfg!(feature = "custom-protocol")`) and
//! `AppManager::get_app_url`'s own `#[cfg(dev)]`/`#[cfg(not(dev))]` branch descend from the
//! identical single build-script emission: `tauri` **2.11.5**'s own `build.rs:255-261` computes
//! `dev = !has_feature("custom-protocol")` and prints `cargo:dev={dev}`; `tauri-build` **2.6.3**'s
//! `is_dev()` (`src/lib.rs:425-429`) reads that emission back via `DEP_TAURI_DEV`, and
//! `cfg_alias("dev", is_dev())` (`src/lib.rs:519`) is what gives *this* crate's own build the
//! `#[cfg(dev)]`/`tauri::is_dev()` bit in the first place. One bit, two consumers — the mirror and
//! Tauri's internal window-URL resolution can never read it differently.
//!
...
mod tests {
    ...
    fn dev_and_production_branches_are_not_interchangeable() {
        // Mutation note: if `expected_origin_from_config`'s `if is_dev { .. } else { .. }` branches
        // were ever swapped, THIS test is the one that fails -- `dev_url` and `frontend_dist`'s URL
        // are deliberately distinct values here, so a swap makes `is_dev: true` return the
        // production origin (or vice versa) instead of erroring or matching by coincidence, the
        // way `production_default_is_tauri_localhost_over_http` above could not catch (its `dev_url`
        // is `None`, which a swap would turn into `DevUrlNotConfigured` either way).
        let dev_url = Url::parse("http://localhost:5180/").unwrap();
        let dist_url = Url::parse("https://example.test/app/").unwrap();
