...
## Amendment 1 to the Results (appended by the custodian, 2026-09-14, AFTER the architect consult PASS — records the consult's two record-only notes; decides nothing, changes no code)

The architect consult of 2026-09-14 returned **VERDICT: PASS** with two merge-order conditions (A: this branch must not land ahead of the K6 (ii) harness contract — done, `5b6f5dc`, and its sibling `0e7ebdc`; B: entry 89's 2 → 9 px change owes the `POLISH-87-88-89-PREREGISTRATION.md` §4.4 (4) "K6 re-run in full" — the custodian runs it on the branch head and records it there) and two **record-only notes**, named here so a later reader is not misled:

- **Two removal paths §4 invariant 4 / §9 do not enumerate.** (a) A *real* `onHover` (a genuine pointer move) cancels the pending settle and then emits a fresh pick on every branch (`WorkingCanvas.tsx:1928-1959`). This is a re-pick *result* at the current camera in the intended sense — the readout is replaced by a freshly resolved outcome, never by a bare stale id — but the phrase "removed only by a re-pick result" reads narrower than the code, which also removes it on a fresh hover. Read invariant 4 as: the labelled state is only ever *replaced by a freshly resolved readout* (a settle re-pick, or a new hover pick) — never left standing as a bare id, never cleared except by the disarm case. (b) Unmount cancels a pending settle with no M4 clear (`WorkingCanvas.tsx:1849-1857`); this is visually unreachable because the readout slot lives inside `{admitted && …}` (`App.tsx:1339`, `:1418`) and `admitAndResetStaleUiState` nulls the hover state before the slot can re-mount, so no marker survives an unmount to a next mount. Neither path violates B1; both are recorded rather than left implicit.
- **Two residues of the placeholder word outside the declared constant.** The marker wording is `HOVER_CONFIRMING_MARKER_TEXT` (`HoverReadoutView.tsx:19`), declared a placeholder in M3 and §10 (1) and asserted against the symbol, not the string. Two other places name the placeholder word: `HoverReadoutView.test.tsx:69`'s literal `"confirming"` (a re-word makes that assertion vacuous, not false — acceptable) and `hoverReadoutTraceLabel`'s `confirming id <n>` trace string (`WorkingCanvas.tsx`). Both are hereby declared to **track the placeholder**: when the sitting fixes the marker's wording, whoever applies the verdict updates the constant and, if they wish the trace to match, that label — neither is a second source of truth, and the render and the harness both key on class, never on the word.

Nothing above changes behaviour, a guarantee, authority, scope, or a property under test; it is the §B record the consult's notes asked for.

## Amendment 2 to the Results — the K6 re-run (custodian, 2026-09-14, AFTER both gates PASS; records a fact, decides nothing)

The full regression suite was run once on this branch's head (`bde2eb3`), on a quiet machine, reusing the warm `CARGO_TARGET_DIR` (this branch changes no Rust). It **discharges `POLISH-87-88-89-PREREGISTRATION.md` §4.4 (4)** — "K6 re-run in full, its zoom targets re-derived" — the owed preregistered test for entry 89's 2 → 9 px change, and it is the **first live exercise of the K6 (ii) labelled-state contract**.

- Run: `npm run e2e:regression`, 2026-09-14 11:21:03Z → 11:26:33Z, harness **exit 0**, every step PASS (A1′ A3′ A4′ A7′ A8′ A9′ K6 K7 CURSOR′ FIND′ ABSENTCRS′); zero FAIL in the log. Preserved at `frontends/shell/e2e/out/k6-rerun-confirming-marker-2026-09-14.log` (untracked evidence dir; the render-trace ledger beside it).
- **K6 (ii) — the labelled state, verbatim from the instrument:** *"the labelled "confirming" state was sighted mid-gesture at 1/2 notch(es) that had an id standing (0 mid-gesture read(s) landed on a settle's own different id, 0 on the same id), and it never outlived a settle"*; K6 (iii) re-pick confirmed an id "named re-picked by its own confirming trace"; **K6 (v) release edge:** the standing refusal, then a drag and one wheel notch back with the pointer stationary → *`readout {"state":"clear","text":null,"id":null,"marker":null}`, and no confirming re-pick line naming an id since that case's own mark* — the marker did not outlive the settle, and no bare stale id was served. K6 (i) continuous unchanged.
- The 9 px threshold held across the run (A9′ found its interior candidate, K6/K7 derived their zoom targets from the constant); no step regressed. FIND′ PASS again (it is on this branch too, unchanged).

The marker's WORDING is still the placeholder `HOVER_CONFIRMING_MARKER_TEXT` (M3, §10 (1)); the sitting's felt verdict fixes it, applied as a follow-up. The STATE is exercised and green.
