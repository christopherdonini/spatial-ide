...
**Amendment 1 (2026-09-10) — written AFTER the implementation exists; records a fact, decides nothing, and invalidates no work.** §3 D1 and §4 say "the canvas constants file" in the definite; no single such file existed. The named bindings live in a new topic-scoped module, `frontends/shell/src/canvas/hoverRepickConstants.ts`, following `tileGridConstants.ts`'s precedent — `limits.ts` was not used because its declared subject is capacity ceilings and neither binding is one. No decision in §3 is changed. *(The architect gate's wording, appended verbatim by the custodian.)*

**Amendment 2 (2026-09-10) — written AFTER the implementation exists and AFTER the architect gate of the same day found a falsifier in it; recorded BEFORE the fix commit.** The gate's finding, in its words: deck.gl does not deliver `onHover` while a mouse button is down (`@deck.gl/core/dist/lib/deck.js:153-156` — "Do not trigger onHover callbacks if mouse button is down"), so during a drag pan neither the pointer capture nor the cancel runs while the camera-change site arms and schedules on every drag frame; a mid-drag or post-drag settle would pick at the **pre-drag** pixel and emit a confirmed id for a feature not under the pointer — §9's first falsifier, and D5's premise ("a settle always has an explicit, current answer to 'where is the pointer'") false in fact. **D11, added:** while any pointer button is down on the canvas — deck's drag, observed through `pointerdown`/`pointerup` on the canvas element (or deck's `onDragStart`/`onDragEnd`) — arming is blocked and any pending settle is cancelled; the first camera change after the button is released arms as D1/D3 say. One `WorkingCanvas.test.ts` case pins it: a button-down camera burst followed by settle emits nothing; the same burst after release emits exactly one pick. §5 (iv), corrected: the discriminator loop treats the named refusal as a non-terminal outcome (it keeps panning) and fails by name if it exhausts having seen only refusals — the refusal is what the mid-gesture rule emits on its own, so it cannot discriminate a re-emitted retained readout from a fresh pick (block-on-sight 13). No ceiling, cadence, constant or non-goal changes; entry 75 (the human's) receives this finding as input, unruled here.

**Amendment 3 (2026-09-10) — written AFTER the implementation and AFTER the re-gate that found a residual case of the same falsifier; recorded BEFORE the fix.** D11 closes arming *while* a button is held, but the stored pixel survives the release: deck delivers no `onHover` during a drag, and a standing below-pick-resolution refusal is the one readout the mid-gesture rule leaves standing across one, so the first camera change after release could arm and settle at the pre-drag pixel. **D11 is extended:** the release edge invalidates the pointer capture — after `pointerup`/`pointercancel` no settle may pick until a real `onHover` has re-answered where the pointer is; the first camera change after release still arms as D1/D3 say, and its settle emits nothing if nothing has re-captured. Refusal-to-act, the same answer D4 gives a changed framebuffer; no new operator-visible state, no constant, no non-goal changed. Also recorded: the release is observed on the **window**, not the canvas element as Amendment 2's wording said, so a button released off-canvas still clears the flag. *(The architect gate's wording, appended verbatim by the custodian on 2026-09-11 under the human's ruling of DECISIONS-PENDING entry 82, verbatim: "82: (a), capped — further residual → hold and narrow to zoom-settle only, no fourth attempt." So: this is the third and last attempt under the pan-and-zoom reading; a further residual found at the third architect gate holds the piece and narrows it to zoom-settle only — `HOVER_REPICK_ON_PAN = false` — with no fourth attempt.)*

**Amendment 4 (2026-09-11) — written AFTER the third architect gate PASSED the design (no residual of §9's first falsifier remains) and BEFORE the test it names is written; it adds a test, changes no design, and touches no product line (the human's cap on entry 82 forbids a fourth attempt at the fix, not a test).** The unit case that pins Amendment 3 (`WorkingCanvas.test.ts`, the D11 describe) is seam-scoped by §5's own declaration and mirrors the release handler rather than exercising it, because that suite cannot mount the component; nothing on the branch would fail if only the product line at the release edge were deleted. **§5 (v), added:** a fifth `stepK6` case in `e2e/regression.mjs` — establish an above-threshold hover; `mouse.down` → `mouse.move` (a drag pan) → `mouse.up`; then one wheel notch with the pointer never moved; assert the readout is not an id and that no confirming `readout_confirmed … id …` trace was emitted since the step's mark. Only that level runs the real window listeners. It strengthens §5 and is reviewer-gated as a K6 contract addition (§7). The component-seam alternative (extracting the release body to an exported function) is **not** taken: it would edit the fix. Also recorded, cheap-end only: the step prose that says "at that camera" is reworded to "since this step's mark", the mechanism unchanged. **Known limitation, recorded here for the human's word on a KNOWN-LIMITATIONS line, not fixed under the cap:** if neither `pointerup` nor `pointercancel` is delivered after a drag (focus lost mid-drag), the button-down flag stays set and every later camera change cancels — the re-pick goes inert for the session; it can never pick at a pixel the pointer has left, so this is refusal-to-act, not a falsifier.

**Amendment 5 (2026-09-11) — written AFTER case (v) as Amendment 4 wrote it was built and shown NOT to bind (its mutation run passed with the release-edge line deleted), and BEFORE the re-aimed test; it corrects the case's construction to the falsifier's own start state and changes no design and no product line.** Amendment 4's case (v) started from an above-threshold hover with an id standing. That start state cannot reach the residual Amendment 3 closes: the drag's own camera changes clear a standing id under the mid-gesture rule, so the first post-release change finds no standing readout, never arms, and the settle returns before the capture is read — the stale capture is unreachable from there (the worker's finding, verified against the served module: with `WorkingCanvas.tsx`'s release-edge statement deleted, case (v) passed byte-identically). Amendment 3's residual is reachable only from **a standing below-pick-resolution refusal**, the one readout the mid-gesture rule leaves standing across a drag. **§5 (v), corrected to the architect gate's own repro:** establish an above-threshold hover (an id standing); wheel OUT past the threshold with the pointer stationary until the named refusal is standing; take the step's mark; `mouse.down` → drag pan → `mouse.up`; then wheel IN one notch, back above the threshold, with the pointer never moved; settle. **Assert:** the readout is not an id (absent or the named refusal), and no `readout_confirmed … id …` trace was emitted since the mark. Binding proof: with the release-edge line deleted, this case must fail by name (the first post-release change arms because the refusal was standing; the settle picks at the pre-drag pixel and confirms an id). If it still does not bind under this construction, the piece stops and the question goes to the human — not to a fourth attempt at the fix. Everything else in Amendment 4 stands.
