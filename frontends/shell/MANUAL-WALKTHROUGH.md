...
| `B2'`/`B3'` | B2, B3 | `openPath` refuses with `engine.crs_undeclared`; the panel shows that code, the verbatim message, the cut-2 remediation note, no dismiss control, and `.describe-summary` is absent (the "no summary" half of B2's own claim). **Fact that changed (re-aim, `engine/ADMISSION-PREREGISTRATION.md` §4):** `no-crs-refused.parquet` now writes an explicit `"crs": null` (F-3, §2b R-C3, unchanged refusal) rather than an absent `crs` key — since Brief A's P1/P2 an absent key admits under GeoParquet's own OGC:CRS84 default and, in this fixture's metre-domain coordinates, would instead refuse `engine.format_default_contradicted` (F-2), not this code; see `ABSENTCRS'` below for that file | "no canvas change" (B2's other half) — that a canvas already showing a previously-admitted dataset is left pixel-for-pixel untouched by this refusal is not asserted |
| `ABSENTCRS'` | — (no Part B step; the absent-key refusal's own operator row is queued, not sequenced by this piece — rule 11, operator rows are the human's to sequence) | `openPath` refuses `absent-crs-contradicted.parquet` (F-2, §4) with `engine.format_default_contradicted` and its verbatim message; the panel shows that code and message, no dismiss control, and `.describe-summary` is absent | whether a remediation form renders for this code at all (`AdmissionPanel.tsx`'s `formFamilyForCode` starts a family only for `crs_undeclared`/`identity_unusable`) — recorded as an INFO line in this step's own output, not asserted either way; that is Brief A's later-parts question, not this step's |
...
| B2 | Select `no-crs-refused.parquet` and confirm. | No summary, no canvas change. A red-bordered refusal panel appears instead, showing the code `engine.crs_undeclared` and the message **"refused: the file declares no CRS and none was asserted by the caller (no \`geo\` metadata CRS on the primary geometry column). This engine does not apply GeoParquet's OGC:CRS84 default (docs/05, no silent conversion)"** (verbatim). **Fact that changed (re-aim):** this fixture's `geo` metadata now carries an explicit `"crs": null` — GeoParquet's own "no CRS" declaration (`engine/ADMISSION-PREREGISTRATION.md` §4 row F-3) — rather than an absent `crs` key; since Brief A's P1/P2 an absent key is no longer refused this way: it admits under GeoParquet's own OGC:CRS84 format default, and only refuses (`engine.format_default_contradicted`, §4 row F-2) if the file's own coordinates contradict that default. The absent-key operator row for that shape is queued — not sequenced by this piece (rule 11); see `ABSENTCRS'` in the `e2e/regression.mjs` coverage table above, which exercises it E2E-only for now. |
...
| E4.5 (new) | **[CORRECTED 2026-09-07 — RELEASE-0.1 item 7, MUST-FIX 6 (reviewer gate): E5's own item (1) below quotes the baseline-arm-only ceiling-refusal banner as an expected, non-defect outcome. The default residency arm is now `"candidate"`, under which an over-ceiling view is never that error-shaped refusal at all (`WorkingCanvas.tsx`'s own doc comment: "the candidate arm never refuses a batch (item B)") — reaching E5's own item (1) as written needs an explicit baseline pin, added here as its own step, the same D0-style row Part D got.] Reload the app window (Ctrl+R) to return to idle (E4 left `filter-zoned.parquet` open, and `setResidencyArm` is refused while a dataset is open — the same "close it first" contract `residencyArm.ts` names). Open DevTools and run `await window.__SPATIAL_E2E__.setResidencyArm("baseline")`; confirm it resolves `{ok:true}`.** | This is the only step this insertion adds; E5 below is otherwise the original script, unedited, now describing the baseline arm's own reachable-only-in-dev behavior for its own item (1). |
| E5 | Click **Open GeoParquet…** and select `slow-filter-scan.parquet`. Wait for the first (unfiltered) look to settle, then type `id > 3999900` into the filter input and click **Apply**. | Two things, in order, both expected and stated openly — neither is a defect to report: **(1)** the unfiltered first look overflows the declared ceiling **by construction** (this fixture's true vertex total is kept far over `MAX_RESIDENT_VERTICES` on purpose, the same as Part D's fixture) — expect the same red-bordered ceiling banner plus a persistent status line reading `<N> of 4000000 features rendered — declared ceiling reached (MAX_RESIDENT_VERTICES)` (the automated suite's own runs against this exact fixture consistently read `163440 of 4000000 features rendered — declared ceiling reached (MAX_RESIDENT_VERTICES)`; your own `<N>` may land elsewhere, but the surrounding text must match exactly) **[CORRECTED 2026-09-07 — this item (1) is now reachable only because E4.5 pinned baseline immediately above; on an ordinary launch with no console action, the shipped candidate default shows the declared partial view instead (Part K's own K2), never this banner]**. **(2)** After Apply: a **Cancel** button appears immediately next to Apply/Clear, and shortly after, a liveness line reads the literal text **"Filtering — scanning, no matching rows yet"** — no percentage, no ETA, no "N of M" figure. The canvas itself does not change (the predicate matches only the last 100 of 4,000,000 physically-ascending ids, so nothing has matched yet). **Judge:** does this read as *working*, not *hung*? **(3)** Design note, not part of this script's own click-path (E6 below cancels the scan before it finishes): Apply now behaves exactly like opening a dataset — it issues an unrestricted look and resets the canvas's fit anchor for the new filter generation (2026-08-15 walkthrough Part E, E5 finding: the matches used to be unfindable, off the viewport the unfiltered look happened to leave the camera at, and Zoom to layer was inert for the same reason). So *if* a filtered scan is left to run to completion rather than cancelled, the camera automatically lands on the matching features once they arrive — the same one-shot fit a fresh dataset-open performs — and **Zoom to layer**, clicked while that filter is still active, re-fits to the filtered results specifically, not the whole dataset. `e2e/filter-panel.mjs`'s `FIND'` step encodes exactly this scenario end to end (fresh open, Apply, let the scan finish, assert the canvas is not blank) — note: `FIND'` itself runs on the shipped default now (RELEASE-0.1 item 7); it is `SLOW'/CANCEL'` (moved to `e2e/refusal-contract-baseline.mjs`) that owns item (1)'s own baseline-only assertion. |
...
| G11 (added 2026-09-14 by rule 11 — entry 86's own preregistration queues the row into **this** Part, verbatim: *"one walkthrough row queued for the next sitting bundle (**Part G**), written as "zoom anchored at a window size that is not 1280×900""* (`renderer/bundle-viewer/ZOOM-ANCHOR-PREREGISTRATION.md:55`). Numbered **after G9 and not before G10**, because G10 is the decision block below, not a table row — nothing is renumbered. **Needs a bundle published by a build that carries entry 86's fix (PR #53):** a bundle published earlier keeps its own frozen copy of the viewer, verbatim — *"it is absent only from bundles published by a build that carries this fix"* (`:47`) — so re-publish through G1–G5 on a current build rather than reusing an older bundle.) | Serve that bundle with G7's own command (`node scripts/serve-bundle.mjs "<the destination you just published to>" <a fresh port>`) and open the printed URL. **Resize the browser window so the viewer's canvas is clearly NOT its native 1280×900** — do the whole step once at a window much LARGER than that and once much SMALLER. In each size: put the pointer well **off-centre**, over a polygon corner or another feature you can name to yourself, hold the mouse completely still, and wheel-zoom IN two or three notches, then back OUT the same number. | The preregistration's binding rule, verbatim (`renderer/bundle-viewer/ZOOM-ANCHOR-PREREGISTRATION.md:14`): **"The pointer and the view share ONE pixel space, and the conversion into it happens in exactly one function."** What to look at (this row's own paraphrase of that rule, not a quote): **the point under the pointer stays under the pointer** — the picture grows and shrinks about that point at both window sizes, and the corner you named does not walk off toward a window edge. Coming back out the same number of notches puts the view back where it started (verbatim, `:10`: *"Not cumulative: in-then-out returns exactly."*). **PASS/FAIL per the preregistration** — §3's post-fix discriminator, verbatim (`:30`): *"all three sizes read no displacement beyond the declared tolerance, in and out, and the solved anchor equals the pointer's world point."* **FAIL** is the pre-fix shape §3 records, verbatim (`:29`): *"large window → NW on zoom-in, SE on zoom-out; small window → reversed; CSS box forced to exactly 1280×900 → none."* — in the operator's terms (paraphrase): the named point sliding away from under the pointer one way going in and the other way coming out, at either non-native window size. **No speed, smoothness, sharpness or quality judgment is asked for here, and none may be recorded** (that preregistration's own block-on-sight list, `:51`). |
...
| Fixture | Path | Purpose |
|---|---|---|
| No CRS | `C:\dev\spatial-ide\target\fixtures\manual-walkthrough\no-crs-refused.parquet` | I1–I3 — reused from Part B; declares an explicit `"crs": null` (GeoParquet's own "no CRS" — `engine/ADMISSION-PREREGISTRATION.md` §4 F-3; changed from an absent key, re-aim above) |
...
| Both remediations needed | `C:\dev\spatial-ide\target\fixtures\manual-walkthrough\bothneeded-refused.parquet` | I5 — **new this cut**: an explicit `"crs": null` (re-aimed from an absent key, same reason as the No CRS row above — `engine/ADMISSION-PREREGISTRATION.md` §4 F-3) AND no `id` column, together. Regenerate: `cargo test -p spatial-kernel --test manual_walkthrough_fixtures generate_the_bothneeded_refusing_fixture -- --ignored --nocapture` |
...
| Automated step | Walkthrough step(s) | Covers | Does not cover |
|---|---|---|---|
| `ASSERT'` | I1, I2 | `engine.crs_undeclared` refusal verbatim + CRS form present; the pinned catalog is reachable via `crsCatalog()` (two entries, `epsg-2056` and `epsg-3857` — item-8 3857 piece, 2026-09-08 — non-empty definitions); asserting the first entry admits, canvas renders, and the summary's CRS line shows `caller-asserted by`/non-empty by-at/`catalog:epsg-2056@sha256:` (prefix only) | I2's own claim-vs-fact legibility judgment; the "Full definition" disclosure's own click-to-expand interaction |
...
**Honest note — reaching the candidate arm has no shipped UI control.** **[CORRECTED 2026-09-07 —
RELEASE-0.1 item 7, DECISIONS-PENDING entry 52 = (a), MUST-FIX 5 (reviewer gate): this note's own
claim is now BACKWARDS. The default residency arm flipped to `"candidate"` — reaching it needs NO
console action any more, the same as every earlier Part; what now has no shipped UI control is
reaching BASELINE (K1's own reference shape below), the ARM THIS NOTE DESCRIBES BELOW as needing
DevTools. Read every sentence below with "candidate" and "baseline" swapped from how it was written,
or read K1/K2's own dated corrections directly.]** Unlike every earlier Part, this one cannot be
reached by clicking through the ordinary app. `residencyArm.ts`'s `currentArm` is in-memory JS
state, defaulting to `"baseline"` on every load **[CORRECTED 2026-09-07 — FALSE as of the flip:
`DEFAULT_RESIDENCY_ARM` is `"candidate"`, `residencyArm.ts`'s own doc comment]**, changeable only
through the same dev/E2E hook `residency-harness.mjs` itself drives over CDP —
`window.__SPATIAL_E2E__.setResidencyArm` — reachable from a human operator only by opening the app's
own DevTools console (WebView2, since this is Tauri) before opening any dataset. This is not a
shortcut around the product; it is the literal absence of a shipped control, itself worth the human
noticing while running this Part — the candidate arm this Part exercises is not yet something an
ordinary user could reach at all, only an operator with DevTools open **[CORRECTED 2026-09-07 — this
was true of candidate before the flip; it is now true of BASELINE instead, per the correction above]**.
The same hook refuses (`{ok:false, code:"dataset-open"}`, not a crash) if a dataset is already open; a
full window reload (Ctrl+R) returns to a clean, no-dataset state between arm switches.
...
control is unchanged and still worth noticing). **MANDATORY after EVERY Ctrl+R (added 2026-09-06,
after the first half-sitting's verdicts turned out arm-unverifiable): the arm silently resets to
`"baseline"` on every reload, and baseline on this fixture shows Part D's ceiling refusal
("accepting this batch would carry N resident vertices…", "N of M features rendered — declared
ceiling reached") where the candidate shows a declared partial view — so before each open, run
`await window.__SPATIAL_E2E__.getResidencyArm()` and require it to print `"candidate"`. A step
whose arm was not verified this way is recorded as arm-unverified, not as a verdict.** Also
verified this sitting (`engine/src/fixture.rs:618`): this fixture is laid out row-major by id, so
the candidate arm's 10,000-row first look is a bottom STRIP of the layer, and "Zoom to layer" fits
that strip (the fit anchor is the union of what has rendered — DECISIONS-PENDING entry 42), not the
dataset. Every "at fit" expectation below means that strip view.

| # | Step | Expected outcome |
|---|---|---|
| L1 | **Block A reference, not itself judged. [CORRECTED 2026-09-06 after the sitting's trace — the original expected the over-budget settled suffix; on this fixture under 1b the fit view is WITHIN budget.]** Arm candidate/fine per K2 (verify the arm!), open `polygons-100k.parquet`, let it settle, then click **Zoom to layer** and let the whole fill run to quiescence WITHOUT touching Cancel. | The button fits the rendered strip (see the fixture note above), whose covering set fits the render budget — 1b's P6b made the truncated first look evictable, which frees ~900k vertices (the sitting's own trace: `evictedTileKeys: 1`, resident 19,082 → 9,352). So: **no status line at all during the fill** (a within-budget fill emits `candidate-fill-progress`, reduced to no status — entry 36's own "re-earn at settle" rule), tiles arriving three at a time in row-major order over tens of seconds to minutes with only the liveness indicator as a signal, then at settle *"Showing all `<N>` features in view"* — IF the covering set was not truncated (entry 43: a truncated within-budget settle is silent by design). The over-budget sentence and string 4 do not appear here at all; they belong to a zoomed-OUT view. **Form the felt reference:** how does a long, text-less fill read? |
...
| L5 | **Block B. [CORRECTED TWICE, 2026-09-06 — the original expected the paused suffix; a first correction (same day, code-read only) expected the over-budget sentence with a transient stall; the sitting's own trace then showed the fit view is WITHIN budget, so neither applies. This row now states what was observed and verified.]** Reproduce L1's fill (Ctrl+R, **verify the arm**, reopen, Zoom to layer) and watch the status line DURING the fill. | **[CORRECTED 2026-09-07 — the "none at settle" half predates F2 (PR #24): a truncated within-budget settle now speaks string 6, *"Filling has finished for this view — some areas were not loaded; pan or zoom to load them."*, sighted by the operator on the 48-(a) build.] Expect no status text DURING this fill; at settle, string 6 if the covering set was truncated** — the fill's silence is by 1b's own design, not a miss: a within-budget fill emits `candidate-fill-progress` → no status (`residencyStatus.ts:192`), and a within-budget settle that is incomplete by truncation is *"deliberately left silent"* (`candidateArmSession.ts:693-696`) — recorded as DECISIONS-PENDING entry 43 because it is silence representing state, the class entry 36 ruled against. The `stalled` suffix (string 1) cannot appear in a within-budget view; its home is an over-budget view whose in-viewport tiles are unevictable (the 5 GB shape, behind entry 40). String 1's wording is ruled and BS3 unit-pinned; nothing in the close gates on sighting it live here. **What to watch instead:** whether the canvas fills to the edges of the view or stops with space left (the sitting saw it stop — entry 43's own case) — record which. The optional probe (paste BEFORE opening the dataset, then Open GeoParquet, then Zoom to layer — its printed "click Zoom to layer now" line skips the open step; ignore that wording): `(async()=>{const out=[];const t0=performance.now();const timer=setInterval(async()=>{const q=await window.__SPATIAL_E2E__.residencyQueuedTileCount();const el=document.querySelector(".residency-status");const text=el?el.textContent:null;const last=out[out.length-1];if(!last||last.q!==q||last.text!==text)out.push({t:Math.round(performance.now()-t0),q,text});},50);window.__L5_PROBE_STOP__=()=>{clearInterval(timer);console.table(out);return out.filter(s=>s.text&&s.text.includes("paused")).length+" paused sightings";};console.log("L5 probe armed — open the dataset, then click Zoom to layer; run __L5_PROBE_STOP__() after settle.");})();` — a queue count that reaches 0 with the canvas still incomplete is the truncation signature. |
| L6 | **[CORRECTED 2026-09-06.]** From L5's settled strip view (already within budget), zoom in several notches so the covering set is small enough not to truncate, and let it settle. **Then zoom OUT past the strip toward the whole layer and watch.** | Zoomed in: *"Showing all `<N>` features in view"* at settle — structurally gated on settled-complete (Item B: never while any covering stream is in flight or queued). **Judge:** does the claim arrive at a moment that feels *earned* — the fill visibly done — rather than optimistic? Zoomed out: the over-budget sentence *"Showing `<N>` features — the farthest areas of this view are not drawn, to stay within the render budget. Zoom in to see more detail."* and, at settle, string 4's suffix. **Known finding, record don't chase (entry 44):** at this zoom the sitting observed tiles appearing for a moment and disappearing, one after another, ending with a single tile standing — ADR-028 Amendment 1's own reopen condition (partial in-viewport tiles evicted by the next admission). If you see it, describe it in your own words for the log. *(String 5, the failure-partial wording — "Filling has finished for this view, but part of this view failed to load; pan or zoom to retry." — is NOT reachable on demand without fault injection and is deliberately not staged here; it is ruled and unit-pinned. If it appears naturally at any point, record the step and judge it then.)* |
...
arriving; an id surviving L7's zoom-out at a camera where you can no longer tell which feature is
under the pointer (entry 47 makes a RE-PICKED id correct there, a retained one still a defect); the
ceiling-refusal banner appearing under the candidate arm): stop, record the exact step and what you saw, and report — do not continue
assuming it was unrelated. L4's missed window, L5's paused suffix not appearing (a within-budget
view has no over-budget sentence to carry it), a text-less fill and a silent truncated settle
(entries 36/43 — by design, recorded), the zoom-out appear/disappear cycling (entry 44 — the
Amendment-1 reopen condition, recorded), and string 5 never appearing are all named expected
outcomes of THIS sitting, not deviations to stop for. The ceiling-refusal banner under a
VERIFIED candidate arm remains a stop-and-report deviation.
...
M8–M11 all carry that ruling in their own row headers: M10 and M11 from the item-10 branch that
rewrote them, M8 and M9 by the custodian's sweep on main at PR #34's landing (2026-09-09) — until
then they had kept a stale "[pending the human's entry-53 word]" bracket the ruling had already
overtaken. Items 2/3/3e proceeded either way (per Amendment 3's own closing line).

**M12 and M13 were added/rewritten on 2026-09-08 by release-cut item 1 (b)** (the ADR-020 config
mirror), on the human's ruling of `DECISIONS-PENDING.md` entry 61: M12 becomes the origin
**equivalence assertion** across all three build modes (it previously said only "not run here,
depends on item 1"), and M13 is new — a deliberate refusal on a **release-built** executable, the
one residual ADR-020 Amendment 1 §(b) states and its own §(f) could not close. Neither row needs the
clean profile: M13 in particular is run from the ordinary development machine, and only M12's mode 3
uses this Part's installed artifact.
...
| # | Step | Expected outcome |
|---|---|---|
| M1 | Install | Run the NSIS installer (`Spatial IDE_0.1.0_x64-setup.exe` — the v0.1.0 candidate built 2026-09-09 from main `998be05`, SHA-256 `712b30632e2c27078686f13defd1fe5462e723dbd4d9124fc010d6958b6aa45a`, 10,559,184 bytes, unsigned; confirm the hash before installing) as the clean profile's own ordinary (non-elevated) user. No UAC/admin prompt appears — `installMode: "currentUser"` (tauri-utils 2.9.3 `NSISInstallerMode::CurrentUser`: "doesn't require Administrator access") installs per-user, under this account only. |
...
| M3 | The Notices view, and `NOTICE.txt` beside the executable | Click the collapsed **▸ Notices** disclosure (bottom of the window, beside **▸ Console**). It expands (**▾ Notices**) and renders the shipped notice text. Separately, in the install directory (found via the Start Menu shortcut's own "Open file location", or `%LOCALAPPDATA%\Spatial IDE\` — the per-user NSIS install root: the generated `installer.nsi` sets `$INSTDIR` to `$LOCALAPPDATA\${PRODUCTNAME}` in `currentUser` mode, and `productName` is "Spatial IDE"; an earlier draft of this row guessed `spatial-ide-shell`, corrected 2026-09-09 from the script — confirm from the shortcut if it differs), open `NOTICE.txt` in a text viewer. **Both** carry every string below, verbatim. They are listed **in the order they occur in the file**, with the line each was found at on the build this row was last regenerated against (62,081 lines in all), so an operator can read straight down rather than hunting: **(1) The header, at the top of the file (line 23).** **Re-quoted 2026-09-08 (RELEASE-0.1 item 9 landing; the two "OWED, not yet done" sentences this row previously quoted are REMOVED from the regenerated header, not merely reworded)** — quoted verbatim from the regenerated `notice()` header's scope paragraph, ABOVE everything else this row quotes — **re-quoted again 2026-09-08 (release-cut fix batch, MUST-FIX 3: the sentence this row quoted claimed "every third-party work actually compiled into three things", which over-claimed, since DuckDB's amalgamation compiles in sources no build manifest reports; the header is narrowed and the gap named)** — **re-quoted a third time 2026-09-08 (DECISIONS-PENDING entry 62 = (a): the named gap is CLOSED; the 26 works now carry their full upstream licence texts, so the scope sentence names four sources and no longer names a gap)**: *"This installed copy scopes wider, because the installed application carries more than the viewer alone: it enumerates every npm package compiled into the bundle VIEWER, every npm package compiled into the packaged frontend's OWN build (frontends/shell/dist, conveyed via tauri.conf.json's `frontendDist`), every Rust crate statically linked into the kernel/data-engine/renderer/protocol binary this application also embeds, and every third-party work embedded inside DuckDB's own amalgamated C/C++ source tree, which the data engine compiles in and which none of those three build manifests can see -- each in its own section below. That fourth section states its own, different source where it begins; it is the one set here that no build manifest reports."*; **(2) About 700 lines in (line 721, and line 743).** Verbatim: *"Geodetic Parameter Dataset, © IOGP (International Association of Oil & Gas Producers), used under the EPSG Terms of Use: https://epsg.org/terms-of-use.html"*; and, further down, under **"THE SPATIAL IDE APPLICATION, WHEN DISTRIBUTED AS AN INSTALLED PROGRAM"**, *"Its Corresponding Source, as AGPL-3.0 §6 (conveying non-source forms) requires, is available under route §6(d) (access from a network server) at: https://github.com/christopherdonini/spatial-ide"*. **(3) The first three of the four per-set sections, in this order:** the viewer's own **"THIRD-PARTY WORKS COMPILED INTO THIS VIEWER"** (line 751), then **"THIRD-PARTY WORKS COMPILED INTO THE PACKAGED FRONTEND (frontends/shell/dist)"** (line 3,338) and **"RUST CRATES STATICALLY LINKED INTO THE PACKAGED APPLICATION"** (line 5,000), each followed by real package/crate entries (react, `@deck.gl/core`, `duckdb`, `tauri`, … — the exact set is a build-manifest fact, not asserted here as a fixed list). **(4) Last, and far below all three** -- more than 54,000 lines below the Rust section, because those three carry their own licence texts too -- the FOURTH section, at line 59,854, its heading **"DUCKDB'S BUNDLED THIRD-PARTY SOURCES"** — **26** entry lines shaped `<lib> (third_party/<lib>) — <licence id>` (`brotli (third_party/brotli) — MIT`, … `zstd (third_party/zstd) — BSD-3-Clause`), each followed by that work's **full licence text**, not merely its name; its intro must state where the set comes from, verbatim: *"This set is therefore"* / *"enumerated from DuckDB's own UPSTREAM SOURCE TREE at the version the pinned crate corresponds to"* and *"It is"* / *"a manifest of a kind, and a checkable one, but it is not a build manifest."*; and the section must END with the line **"END OF THE DUCKDB BUNDLED THIRD-PARTY SOURCES SECTION"** (the sentinel `check:dist-notice` bounds the embedded notice by), which is the LAST line of the file (line 62,081): nothing follows it, so an operator who reaches it has reached the end and has seen all four sections. **The sentence this row quoted until 2026-09-08 — that the 26 works' *"license texts are NOT carried in this file"* — must NOT appear; nor must *"This gap is tracked, not silently shipped"*; and `*** BOOTSTRAP PASS` appears nowhere in the file** (the provisional-header sentinel a build must supersede within its own `npm run build`; `check:dist-notice` and `noticeByteIdentity.test.ts` both refuse a surviving one, and ADR-030's appended note lists "no bootstrap sentinel" among what this row must show — so an operator checking the installed artifact confirms it there too, on the bytes actually conveyed). **Byte-identity is the claim under test, scoped correctly, and already real** — `frontends/shell/src/notices/noticeByteIdentity.test.ts` asserts the shared VIEWER portion of `src/generated/NOTICE.txt` (from the "THE VIEWER" heading through the end of "THIRD-PARTY WORKS COMPILED INTO THIS VIEWER") is byte-identical to `renderer/bundle-viewer/dist/NOTICE.txt`'s own text — the two files are no longer identical AS WHOLES, by design (this package's own copy carries two more sections the published bundle's own copy never does) — plus a dist-level check (`npm run check:dist-notice`) that matches the generator's own per-entry line shape at true line start, inside the right section, and requires each section's entry count to equal its own build manifest's (release-cut fix batch, SHOULD-FIX 5: the previous "every name appears" wording was a substring test, and measurement showed it could not fail for a dropped entry); this step is the felt/visual confirmation on the real, installed artifact. **Also open the THIRD notice file the installer places (release-cut fix batch, MUST-FIX 1): `bundle-viewer\NOTICE.txt`, inside the install directory's own `bundle-viewer\` folder** (`tauri.conf.json`'s resource glob copies the whole published viewer `dist/` there, `NOTICE.txt` included). It is the bundle viewer's OWN notice, not the application's, and its header must say so: present verbatim, *"Installed inside the packaged application, at bundle-viewer\NOTICE.txt (tauri.conf.json's own resource glob), this file is the VIEWER's OWN notice"*, and — **re-quoted 2026-09-08 (item 9's closing commit, architect advisory A2: "complete" asserted more about the file it points at than that file asserts about itself, which names an open gap outright; "application-wide" states the scope instead)** — **re-quoted again 2026-09-09 (release-cut fix batch on the entry-62 piece, architect Q4: the sentence named three sets, which stopped being complete the moment the fourth section landed; it now names four)** — *"The application-wide notice set -- covering this viewer, the packaged frontend's own npm dependencies, the Rust crates statically linked into the application, and the third-party works inside DuckDB's own amalgamated C/C++ source tree -- is the separate NOTICE.txt installed beside the executable"*. That second sentence is the one an installed reader needs and the one this file used to get wrong (it previously said those further sets were owed and not yet done, which stopped being true when the generator started producing them). **Declared observation, and the only number this row asserts:** the ▸ Notices disclosure must actually EXPAND and RENDER on the installed artifact, not merely exist — the panel's payload is `src/generated/NOTICE.txt` imported `?raw`, **3,186,905 bytes** on the build this row was last regenerated against (the entry-62 amalgamation piece as landed with its fix batch, 2026-09-09; 3,186,855 B at that piece's first regeneration and 3,073,859 B before it — nearly all of the difference is the fourth section: ~107 KB of pinned upstream licence text for the 26 works, plus the section's intro and the widened header, and the last 50 bytes are the fix batch's own header wording), so it is a single ~3.2 MB string handed to the DOM at click time, which is exactly the size at which "the disclosure opens but paints nothing" is a plausible failure and only a human on the real install can rule it out. |
| M4 | Open a GeoParquet in an admitted CRS | Click **Open GeoParquet…**, select the `100k-happy-path.parquet` fixture (EPSG:2056/CH1903+ LV95 — one of the CRS catalog's two admitted entries since PR #32 landed EPSG:3857 on 2026-09-08, ADR-026; an earlier draft of this row said "one admitted entry"). A summary appears exactly as Part A's A3 describes; the canvas renders. |
...
| M8 (entry 53 ruled (a) on 2026-09-08 — publish ships, re-confirmed on the artifact here) | Publish — the plain-outcome sentence | Reopen `100k-happy-path.parquet` (M4's own dataset — a fresh open, since M7 moved on to `polygons-100k.parquet`) and reapply M5's filter and M6's style if you want the published bundle to carry them. Click **Publish…**, pick a destination, and read the approval dialog in full (Part G's G3). The shaded plain-language sentence names the concrete outcome exactly as G3 describes (e.g. *"This will create a folder named … containing the selected rows as one or more data partitions, the interactive viewer page, and a manifest."*). Type the destination's own final path component and confirm; the dialog closes to a quiet **"Published."** summary (G5), no duration anywhere. |
| M9 (entry 53 ruled (a) on 2026-09-08 — publish ships, re-confirmed on the artifact here) | `--audit-show`, re-confirmed on the packaged artifact | **The audit log is PER-USER, not machine-wide** (`kernel/src/permission/audit/log.rs`'s `resolve_log_path`: `%LOCALAPPDATA%` resolves under the CURRENT Windows account) — a release-cut fix batch correction to an earlier draft of this row, which wrongly said reading it as the operator's own ordinary account would find M8's own attempt. It will not: M8 ran as the CLEAN profile's own account, so its log lives under THAT account's `%LOCALAPPDATA%`, invisible to a `--audit-show` run as any other account pointed at ITS OWN default. Two ways to read it correctly, either is fine: **(a)** run the dev checkout's `--audit-show` FROM the clean profile itself, if a Rust toolchain is available there (`cargo run -p spatial-kernel --bin publish-bundle -- --audit-show` from a checkout, or a copied `publish-bundle.exe`); or **(b)**, simpler when it is not, run it from the operator's OWN ordinary account with `SPATIAL_IDE_AUDIT_LOG` set to the clean profile's own absolute path — `C:\Users\<clean-profile-account-name>\AppData\Local\spatial-ide\audit\publish.jsonl` (the packaged installer ships no `publish-bundle` CLI itself; only the shell's Tauri executable and `bundle.resources`, so `--audit-show` always needs a dev checkout SOMEWHERE, same precondition Part G's G6/Part H's H9 already carry). One plain sentence per intent/outcome pair, G6's own format: the legibility claim under re-confirmation is that a packaged-app attempt reads exactly as legibly as a dev-tree one already does, not a new property. |
| M10 (entry 53 ruled (a) on 2026-09-08 — publish ships, re-confirmed on the artifact here) | The ADR-025 refusal, on a source above the reader's ceilings — now refused BEFORE the pin (RELEASE-0.1 item 10) | With `parcels-5gb.parquet` admitted FRESH in this Part (Part H's fixture, 3,300,000 rows — above `MAX_FEATURES` = 2,000,000; a fresh admission, not H6's — `ContentPin` lives in memory, per app instance, not persisted across sessions), click **Publish…**, choose **Whole dataset**, and pick a destination. **This row's own shape changed with RELEASE-0.1 item 10 (DECISIONS-PENDING entry 7's ruled pre-fix), and this is where that shows up live**: the predictable ADR-025 checks now run BEFORE the pin, via a pin-free kernel function (`spatial_kernel::publish::preflight_pinless`), and this fixture's feature count is already known without one — `IdentityMode::NativeUnique` means `Dataset::open`'s own uniqueness scan already counted every row (`kernel/tests/scale_pass.rs::spec_5gb`), so `ds.identity().verified_rows()` is `Some(3300000)`, predictable with no hash at all. Expect the refusal to render without a "Preparing…" wait this time, because the whole-file SHA-256 (`ContentPin::take`) never runs for this attempt: the ceiling refusal fires first and the pin phase is never reached. (A source that is BOTH unpinned and over-ceiling now refuses `ReaderCeilingExceeded` before `SourceNotPinned` would ever have had a chance to fire either — the reordering's whole point.) The native picker's own path is already resolved and converted by this point (it ran first, in `binding_publish_prepare`), so what is true is narrower than an earlier draft of this row claimed: **the approval (typed-phrase) dialog never opens**, not that nothing was acted on yet. The typed refusal renders in the Publish panel instead (verbatim, `kernel/src/publish/error.rs`'s `PublishError::ReaderCeilingExceeded` Display, unchanged by this piece): *"refused: this publish is predicted to exceed the bundled viewer's declared ceiling MAX_FEATURES — limit 2000000, predicted 3300000 — before any bytes were written (ADR-025: refuse, typed, at preflight; reopens when a second reader exists). Instead, publish the current-viewport bbox instead of the whole file (the viewer's ceilings apply to what a bundle carries, not to what the source dataset holds)"* No destination is created — confirm the folder is absent or empty. **The named residual, stated rather than exercised here**: a dataset admitted `DeclaredNotVerified` (skip-uniqueness-check route) has no predictable feature count either way — its pin still runs and only the bundled viewer's own load-time check catches it over ceiling; `parcels-5gb.parquet`'s `NativeUnique` identity means this walkthrough step does not exercise that residual, by construction. **By design, this refusal produces NO audit record at all** (`kernel/src/permission/boundary.rs`'s own "what is not audited" list; `boundary::execute`'s first fallible call is `preflight(req)?`, before the audit log is even opened for writing — and this refusal, from RELEASE-0.1 item 10 on, does not even reach `publish::prepare`/`preflight`, let alone `boundary::execute`, so it is outside that enumeration's scope by construction, not merely covered by it) — M9's `--audit-show` run will not show this attempt, and that absence is correct, not a gap to chase. |
| M11 (entry 53 ruled (a) on 2026-09-08 — publish ships, re-confirmed on the artifact here) | The bundle opened in a browser | Serve M8's own bundle (`node scripts/serve-bundle.mjs "<M8's destination>" <a fresh port>`, Part F/G's own command, run from a checkout of `renderer/bundle-viewer` — this step alone needs the dev tree, since the packaged app does not ship that server script). The bundle viewer loads and renders M6's own style, exactly as Part G's G7 describes. |
| M12 (rewritten 2026-09-08 by release-cut item 1 (b)) | The origin equivalence assertion, in all three build modes | **The assertion, in full:** in each of the three build modes — (1) `npm run tauri dev`; (2) `npx tauri build --debug --no-bundle`, the produced executable run **directly** from `frontends/shell/src-tauri/target/debug/spatial-ide-shell.exe`, not installed; (3) the **installed** `tauri build` artifact this Part's M1 installs — the app's session log (`%LOCALAPPDATA%\dev.spatialide.shell\logs\session-*.log`) carries a pinned-origin line, `data-plane expected origin (config mirror): <origin>`, and an `origin-self-check ok pinned=<origin>` line, and **the two name the same origin**; AND, in the same session, **one data-plane upgrade is admitted**: open a fixture (M4's `100k-happy-path.parquet` is fine) and confirm one `viewport_query` stream actually reaches the canvas — features drawn, no 403, no admission refusal in the log. Expect `http://localhost:5180` in mode 1 and `http://tauri.localhost` in modes 2 and 3 (ADR-020 Amendment 1's config mirror; the origin differs by mode, the AGREEMENT is what this row asserts). **Observed so far, stated exactly:** the log-line PAIR only, and only in modes 1 and 2, on `b78fa6d` (`eb46429` before this branch's second rebase) — `session-1788855035.log` (mode 1) and `session-1788855843.log` (mode 2), quoted in ADR-020 Amendment 1 §(f)(i)/(ii). **No admitted upgrade has been observed in ANY mode**, and mode 3 has not been run at all: both are this row's own work, here, on the real installed artifact. |
| M13 (added 2026-09-08 by release-cut item 1 (b)) | A deliberate refusal on a release-built executable (the `blocking_show()` residual) | **Why this row exists:** ADR-020 Amendment 1 §(b)'s fail-closed bullet states a reliance — `refuse_to_start` calls `blocking_show()` on the main thread, against `tauri-plugin-dialog`'s own "should *NOT* be used when running on the main thread context" — and an honest residual: that path has only ever been exercised on a **dev** build (§(f)(iv)), never on a release-built one, where `main.rs`'s `windows_subsystem = "windows"` leaves no console and a hang would be **silent**. This row exercises it. **Steps:** (a) write a config overlay setting `app.windows[0].url` to `"https://example.test/"` — copy `app.windows[0]` out of `src-tauri/tauri.conf.json` and change only `url`, the idiom `e2e/lib.mjs`'s `writeConfigOverlay` and `e2e/writeMeasureConfigOverlay.mjs` already use, writing it under `e2e/out/` (gitignored); (b) `npx tauri build --no-bundle --config <that file>` from `frontends/shell` (`--config` is accepted by `tauri build`, verified 2026-09-08 against `@tauri-apps/cli` 2.11.4's own `--help`: *"JSON strings or paths to JSON, JSON5 or TOML files to merge with the default configuration file"*; `--no-bundle` skips the installer-bundling step — `tauri.conf.json:27`'s `"targets": ["nsis"]` — which this row has no use for); (c) run `frontends/shell/src-tauri/target/release/spatial-ide-shell.exe` **directly**, by double-click or from a shell. **Expected outcome, all three together:** the native dialog **"Spatial IDE could not start"** appears (there is no console in this build — nothing is printed anywhere); dismissing it exits the process with code **1** (`$LASTEXITCODE` from PowerShell, or `echo $?`); and the newest `%LOCALAPPDATA%\dev.spatialide.shell\logs\session-*.log` contains **exactly one line**, an `error` line naming `WebviewUrl::App` — the same shape §(f)(iv) quotes verbatim from `session-1788864076.log`. **A hang instead of a dialog is the finding this row is looking for** — if the process sits with no window and no exit, record that: it is the residual coming true, not an operator error, and the log line written *before* the dialog is what proves the refusal happened at all. |
...
| M16 (added 2026-09-14 by rule 11; DECISIONS-PENDING entry 87. The promise this row discharges, verbatim — `frontends/shell/POLISH-87-88-89-PREREGISTRATION.md:63`: *"one walkthrough row per entry (87: the settled sentence under a filter; 88: the hover through a zoom gesture; 89: the refusal's onset and the cursor), committed with blank result logs and queued into the next sitting bundle (AI_DEVELOPMENT rule 11)"*. That clause names **no Part** — see the placement note below this table.) | The settled residency status under a filter, on a build carrying entry 87's fix | **This is M15's own step, re-run.** With `100k-happy-path.parquet` open, M14's filter (`id < 100`) applied and the view fitted, wheel out several notches and wait for the status line to settle. **Record the sentence verbatim.** **Expected — the preregistration's §2.4 test (4), verbatim (`POLISH-87-88-89-PREREGISTRATION.md:24`):** *"under the filter and after the zoom-out, no PREMISE BROKEN record, every covering tile reaches a terminal, and the status settles to the within-budget sentence"*. At this row's own level (paraphrase, since §2.4's words describe the automated assertion, not the screen): the within-budget sentence *"Showing all `<N>` features in view"* is what settles, and **not** the settled-partial sentence *"Filling has finished for this view — some areas were not loaded; pan or zoom to load them."* (both quoted from M15's own row, which quotes the shipped strings). The partial sentence appearing here **is the finding** — entry 87's defect still standing — and is recorded as such, not as a new defect. **The refusal half (optional, record-only):** open the newest session log (`%LOCALAPPDATA%\dev.spatialide.shell\logs\session-*.log`, M12's own path) and look for `tile-stream-mint-refused` lines. Such a line is **not by itself** a deviation: §2.3 (i) exists so that a refusal is named rather than dropped — before it, verbatim (`:15`), the shell's bare `catch` *"deletes the tile's state with no log, no trace, no status, no requeue"*. A *spurious* refusal is one that stands while the status still settles to the partial sentence; record the count and one line verbatim if any appear. |
| M17 (added 2026-09-14 by rule 11; DECISIONS-PENDING entry 88. **Runnable once `shell/hover-confirming-marker` lands** — at this row's writing that piece is in flight, so the row is written against the RULED behaviour, not against anything built.) | The hover readout across a camera change — the labelled "confirming…" state | Zoom in until individual polygons are clearly larger than a pixel (L7's own setup), hover one until its `id <number>` readout shows, then — **pointer completely stationary** — wheel ONE notch and watch the readout *through* the notch and again after the camera settles. Repeat three ways: a notch after which the same feature is still under the pointer; one after which a *different* feature is; and one after which nothing is (or everything is below the threshold). **Expected — the human's ruling, quoted verbatim from `DECISIONS-PENDING.md`, "RULED 2026-09-14 — question set B of 4", B1:** *"Between a camera change and its settle re-pick, the standing id stays visible with a plain, muted marker — "id 6430 · confirming…" — never the bare id. The marker is removed only by a re-pick result: if the re-pick confirms, the marker drops; if it finds a different feature, the readout changes to it; if it finds none or below-threshold, the readout becomes the refusal or clears. A stale id is never served without its marker (ADR-010 rule 5), and the marker never outlives a settle."* **PASS/FAIL** (this row's own reading of those words, labelled as such): FAIL is a bare, unmarked id shown at any moment between the notch and the settle; a marker still standing after the camera has settled; or a marker that disappears without a re-pick result behind it. PASS is the three repeats each ending as the ruling names. **Also sighted here, and only here** — B1's closing sentence, verbatim: *"Wording sighted live at the sitting; the state itself is ruled now."* Judge the marker's **wording** (*"id 6430 · confirming…"*) in your own words; the state itself is not up for judgment at this row. |
| M18 (added 2026-09-14 by rule 11; DECISIONS-PENDING entry 89 — two halves. Half (a) is runnable now; **half (b) is runnable once the 9 px constant lands** — in flight on the same branch at this row's writing.) | (a) the crosshair cursor while hovering; (b) the refusal's onset at features of roughly 5, 9 and 15 px | **(a) The cursor.** Move the pointer over the canvas with no button down, then press and drag it. **Expected — the preregistration's §4.3, verbatim (`POLISH-87-88-89-PREREGISTRATION.md:44`):** *"`getCursor` returns `"crosshair"` when no button is down and `"grabbing"` while dragging — exactly two values, no `"pointer"` (there is no click affordance to signal)"*. FAIL is any third cursor — in particular a pointing hand, which would signal a click affordance that does not exist. The human's own word on this half, B2's closing sentence verbatim: *"Crosshair cursor while hovering: no objection."* **(b) The threshold — the sitting row the ruling itself asks for.** Quoted verbatim from `DECISIONS-PENDING.md`, "RULED 2026-09-14 — question set B of 4", B2: *"9 px sighted as the declared threshold — a declared choice recording the 2.27 px measured figure and its caveats beside it, never presented as a measurement. Rationale in the constant's comment: human pointer targeting, not pixel resolution, decides whether "which feature" is answerable. Revisable only by a walkthrough-recorded felt verdict (a sitting row hovering features of roughly 5, 9 and 15 px), never by a guess."* **Do it:** find three cameras at which a typical polygon reads as roughly **5 px**, roughly **9 px** and roughly **15 px** across — by eye; the code compares against, verbatim (`POLISH-87-88-89-PREREGISTRATION.md:40`), *"the **average** resident feature's own geometric extent"*, so "roughly" is the operative word and no measuring is asked of you. Hover at each and **record verbatim what the readout says**: the ordinary `id <number>`, or the named refusal *"Features here are below pick resolution — zoom in to inspect them."* (L7's own quotation of the shipped string). **Expected** (this row's paraphrase of the declared comparison, not a quote): the refusal at the ~5 px camera, an id at the ~15 px camera, and either one at ~9 px — what appears at the line is recorded, not judged. **The felt verdict this half exists for:** does the refusal arrive too early? The tension is pre-recorded in §4.2, verbatim (`:42`): *"entry 47's sharpened criterion (`DECISIONS-PENDING.md:1188-1194`) pulls the other way — an earlier refusal necessarily refuses at some cameras where the operator could still tell which feature is under the pointer. The trade is the human's."* Your words here are the only thing that may move the value (B2, above: revisable *"only by a walkthrough-recorded felt verdict"*, never by a guess). |

**If anything deviates:** stop, record the exact step, and report it, same as every earlier Part.
M10's refusal and M9's audit-log format are the expected outcomes of THIS Part, not deviations.

**M16–M18 — where these rows sit, and why (the rows' own placement note, written with them on
2026-09-14).** The polish preregistration's Operator clause
(`frontends/shell/POLISH-87-88-89-PREREGISTRATION.md:63`) promises one walkthrough row per entry
but **names no Part**, so this placement contradicts nothing in it and nothing in it needed
correcting — that file is append-only and was not edited. They are here because Part M is the live,
still-unrun Part whose result log is open, and because M14/M15 — added by the release/0.1.0 fix
batch for entries 85/84/87 — are the same filter/residency family M16 continues. **What they do NOT
inherit from this Part:** none of the three asserts anything about the packaged NSIS artifact or the
clean profile, and none of them needs either. Run them on whatever build the next accumulated
sitting bundle runs (`AI_DEVELOPMENT.md` rule 11) — `tauri dev` is fine — and record that build
beside the result. M17 and M18 are the hover family of L7/L8, whose own re-runs are already queued
for that same bundle; running the four in one block is the sensible order.

---

## Part N — Brief A: admission, provenance and the session identity tier (P6)

This Part is the walkthrough two preregistrations require — it discharges them once it is run and its
result log is filled, not before: `engine/ADMISSION-PREREGISTRATION.md` §12b, gate G-A7 (Part N
operator-verified); and P3b's own record of what it owes this file,
`frontends/shell/OWNER-INVALIDATION-PREREGISTRATION.md` §9's Operator bullet. **No duration in any
row or expected outcome below** (`docs/08_Testing.md`; `engine/ADMISSION-PREREGISTRATION.md` §1,
boundary 10/A6, ADR-018) — every expected outcome is a shipped string, quoted verbatim with its
source `path:line`, never a time, a rate, or a smoothness judgment (the Sitting estimate, below, is
scheduling for the custodian, not a claim about the app). All `path:line` cites below were re-read at
commit `3b23a20c220c47f2774d27f976546039e2a6c249` (`3b23a20c220c`, origin/main at this rewrite — a pin,
not a claim origin/main reads this way forever). The prior pin was `158d450`; PR #93's describe-fields
landing changed two files this Part cites, each re-read whole at the new commit:
`frontends/shell/src/admission/describeSummaryText.ts` and `frontends/shell/src/admission/DescribeSummary.tsx`
(N1's and N3's own rows, below); the file this Part's N3 row previously cited,
`frontends/shell/src/skp/__tests__/fixtures.test.ts`, is no longer cited by any row of this Part. Every
quote was copied from the cited file by direct read, never from memory or from a preregistration's
paraphrase — the reviewer byte-compares each.

**Fixtures this Part reuses — nothing new is written for it.**

| Fixture | Path | Used by |
|---|---|---|
| Compat-corpus file #8 (`crs:format-default`) | `C:\dev\spatial-ide\target\fixtures\compat-corpus\gdal\ogr2ogr-epsg4326-default.parquet` — sha256 `79dabf9db66acad5c3adf6c001d1be2a68ecb4ea08afd73690dfaa1519a96bb7` (`target\fixtures\compat-corpus\MANIFEST.json`'s own `sha256` field; first 12 hex `79dabf9db66a` matches `engine/ADMISSION-PREREGISTRATION.md` §3 row 8) | N1, N5 |
| Compat-corpus file #3 (`axis:format-override`) | `C:\dev\spatial-ide\target\fixtures\compat-corpus\geopandas\gp-epsg4326-covering.parquet` — sha256 `bbb8d0bcb0c3ef76687cf81cebcdabbe67c12bfa6e03b3a97a2c9cb6d25de265` (first 12 hex `bbb8d0bcb0c3`, §3 row 3) | N2 |
| Compat-corpus file #1 (declared, x-first, session-ordinal identity — no CRS-provenance question) | `C:\dev\spatial-ide\target\fixtures\compat-corpus\geopandas\gp-epsg2056-intkey.parquet` — sha256 `66afc49203cb97ba72fd347cd27964832965bd8bffbf8db498aa18b5f6ab38f9` (first 12 hex `66afc49203cb`, §3 row 1) | N3 |
| Explicit-null (F-3) | `C:\dev\spatial-ide\target\fixtures\manual-walkthrough\no-crs-refused.parquet` (Prerequisites table, above — unchanged since Part B/I) | N4 |
| 100k happy path, a scratch copy of it | `C:\dev\spatial-ide\target\fixtures\manual-walkthrough\100k-happy-path.parquet`, copied to a scratch filename before the sitting (Before N1, below) | N6, N8 |

All three compat-corpus files carry 240 rows each (`MANIFEST.json`'s own `observed.row_count`) — a
240-row open, not a 100k-scale one.

| # | Step | Expected outcome |
|---|---|---|
| N1 | Click **Open GeoParquet…** and select the file above (compat-corpus #8 — an absent `crs` key over source data whose own upstream declared EPSG:4326 lat-first; §3 row 8's own `collection_note`). | The summary appears; the **Row count** line reads `not established (not-established)`, not a `240 (...)` figure — `DescribeSummary.tsx:12-15`'s ternary takes its `value === null` branch because the session tier's `verified_rows()` is `None` (`engine/src/identity.rs:221-229`'s `new_session_ordinal`), which `kernel/src/skp.rs:1019-1023` maps to `RowCount { basis: "not-established", value: None }` (the `None` arm, `:1023`). The native/mapped tier's own basis string, `identity-uniqueness-scan-full-file` — used only when a scan actually ran — is the `Some` arm's own text (`kernel/src/skp.rs:1021`); it does not appear on this file. (240 is the fixture's true row count, per the fixtures table above — not a number this screen states.) The **CRS** line reads `OGC:CRS84 — format-rule, axis order longitude,latitude` — the template `` `${crs.identifier} — ${crs.source}, axis order ${crs.axis_order}` `` (`frontends/shell/src/admission/describeSummaryText.ts:27`), filled with `identifier` `OGC:CRS84` (`FORMAT_DEFAULT_CRS`, `engine/src/geoparquet.rs:44`), `source` `format-rule` (a file-arm CRS is re-stamped to this label only when the format DEFAULT applied — `engine/src/dataset.rs:467-471`; the label string itself is at `engine/src/crs.rs:107`), and `axis_order` `longitude,latitude` (`AxisOrder::LongitudeLatitude`, the absent-key R-C2 branch, `engine/src/geoparquet.rs:524-539`). No refusal panel. The **CRS provenance** row reads `crs:format-default, axis:format-override` — `crsProvenanceLine`'s `` `${crs.provenance}, ${crs.axis_provenance}` `` template (`frontends/shell/src/admission/describeSummaryText.ts:47-49`, rendered at `DescribeSummary.tsx:47`), filled with `crs.provenance` `crs:format-default` (`CrsProvenance::FormatDefault`, `engine/src/geoparquet.rs:95`) and `crs.axis_provenance` `axis:format-override` (`AxisProvenance::FormatOverride`, set alongside the CRS default itself by the absent-key R-C2 branch, `engine/src/geoparquet.rs:535`, `.as_str()` at `:114` — confirmed against this exact file by the corpus run, `engine/ADMISSION-RESULTS.md:19`). The **Session identity** row is on screen here too — this fixture is session-ordinal (`engine/ADMISSION-RESULTS.md:19`, compat-corpus #8) — and its reading is N3's. The **Display convention** row is on screen here too, last in the list (`DescribeSummary.tsx:56-61`) — this fixture is a geographic-degrees CRS (`OGC:CRS84`, above) — reading, verbatim, `no coordinate value is transformed; the display convention is equirectangular` (`engine/src/crs.rs:54`, `GEOGRAPHIC_DISPLAY_CONVENTION`, carried through unchanged by `displayConventionLine`, `describeSummaryText.ts:67-69`). This row's own cites into `DescribeSummary.tsx` and `describeSummaryText.ts` were re-read at `037c69e`, after this Part's own preamble pin. Record whether the **Display convention** label reads clearly — the same prompt N3 poses below for its own two row labels — its wording being the human's to settle. |
| N2 | Click **Open GeoParquet…** and select the file above (compat-corpus #3 — the SAME real-world CRS as N1, WGS 84 longitude/latitude, reached because THIS file itself declares `EPSG:4326` lat-first and the format's WKB axis override applies; §3 row 3). | The summary appears; the **Row count** line reads `not established (not-established)`, the same session-tier rendering N1's row explains (`DescribeSummary.tsx:12-15`; `kernel/src/skp.rs:1019-1023`, `None` arm `:1023`) — this file's identity is session-ordinal too (§3 row 3). The **CRS** line reads `EPSG:4326 — file, axis order longitude,latitude` — same template (`describeSummaryText.ts:27`), filled with `identifier` `EPSG:4326` (the file's own declared identifier), `source` `file` (unchanged from N1's `format-rule`: only a format-DEFAULT admission is re-stamped, `dataset.rs:467-471` again — this file's CRS is declared, so `crs::admit`'s file arm keeps `CrsSource::File`, `engine/src/crs.rs:199-213`), `axis_order` `longitude,latitude` (the file's own declared `latitude,longitude` order is flipped to the WKB-established x-first reading here — `data_order_under_wkb_rule`, `engine/src/geoparquet.rs:499-508` and `:567-573`; the ORIGINAL declared order is retained internally per boundary 1 but is not rendered anywhere in this build — `crs.provenance`/`crs.axis_provenance` are declared at `frontends/shell/src/skp/types.ts:89,92`, read by no component). **The sight itself:** N1 reads `OGC:CRS84 — format-rule, ...`, N2 reads `EPSG:4326 — file, ...` — distinguishable by identifier and by source word (not by the axis-order text, which is identical on both). Record whether this reads as clearly distinguishable to you at a glance. |
| N3 | Click **Open GeoParquet…** and select the file above (compat-corpus #1 — a declared, x-first EPSG:2056 file whose only integer column is `parcel_id`, not `id`; admitted-as-declared, session tier; §3 row 1). | The summary appears; the **Row count** line reads `not established (not-established)`, same as N1/N2 (`DescribeSummary.tsx:12-15`; `kernel/src/skp.rs:1019-1023`, `None` arm `:1023`). The **Identity** line (`DescribeSummary.tsx:28-29`) reads `session-ordinal:file_row_number — by-construction-within-generation` — the template `` `${identity.source} — ${identity.uniqueness}` `` (`describeSummaryText.ts:38`), filled with `identity.source` (`format!("session-ordinal:{FILE_ROW_NUMBER_COLUMN}")`, `engine/src/identity.rs:84`, where `FILE_ROW_NUMBER_COLUMN` is `"file_row_number"`, `:73`) and `identity.uniqueness` (`"by-construction-within-generation"`, `:133`). Below it, the **Session identity** row (`DescribeSummary.tsx:49-54`, rendered only when `sessionStatementLine(describe.identity)` is non-null) reads the fuller session-tier sentence, `SESSION_IDENTITY_STATEMENT` (`engine/src/identity.rs:145-149`), carried over the wire verbatim as `describe.identity.session_statement` (`frontends/shell/src/skp/types.ts:124`) and passed through unchanged (`sessionStatementLine`, `describeSummaryText.ts:56-58`) — verbatim from `protocol/skp/tests/data/v0-describe-response-session-ordinal.json:29`; the quoted span sha256, over the span's own bytes and not the whole committed line, ed0a8bd55110f08c29cc69424f445ad5be891d8abc9e6dd84846b4e98e993f76: *"this dataset has no admissible identity column, so features are identified for this session only, by their position in the file. The identity does not survive this open, a reopen, or a change to the source, and it is never saved or published. Declare an identity column to get an identity that does."* **P6's second named sight (this Part's own phase text, `state/NEXT-CUT.md:107`; the same row in the draft at `state/drafts/post-tag/DRAFT-2-BRIEF-A-admission-and-session-lifecycle.md:95`) is the human's to feel at the sitting, not discharged by the screen rendering the sentence** — record whether the Identity line and the Session identity row together read as a clear statement of what this identity is and is not, and whether the two row labels PR #93 introduced — `CRS provenance` (`DescribeSummary.tsx:46`) and `Session identity` (`DescribeSummary.tsx:51`) — read clearly, their wording being the human's to settle. |
| N4 | Click **Open GeoParquet…** and select `no-crs-refused.parquet` (F-3 — explicit `"crs": null`). | Refused, code `engine.crs_undeclared`, the same verbatim message Part B's B2 and Part I's I1 already quote (`engine/src/error.rs:248-252`, `{detail}` filled from `engine/src/crs.rs:286-288`). Below it, the CRS assertion form renders (`AdmissionPanel.tsx:27` maps this code to the `"crs"` family), whose claim notice reads, verbatim — *"This records a CLAIM you are making about this file's coordinate reference system -- recorded with who asserted it and when. Nothing is saved: reopening this file will ask you to assert again."* (`frontends/shell/src/admission/CrsAssertionForm.tsx:62-66`; the source's `&#39;` renders as the plain apostrophe quoted here). This row is Brief A P6's own required sighting of ground Part I already covers by the same fixture and the same form — confirm it still reads this way rather than re-deriving it. |
| N5 | Reopen N1's file fresh. Expand **▾ Publish**; click the **Whole dataset** radio (Current view starts selected by default but disabled with no settled view yet — `PublishPanel.tsx:326` defaults `scope` to `"current"`, `:579-594` is the disabled Current-view radio and its reason paragraph, `:569-578` is Whole dataset). Click **Publish…**, pick any destination under `target\`, and confirm the native picker. | **No approval (typed-phrase) dialog opens** — the same shape Part M's M10 already shows for a different refusal: `preflight_pinless_parts` (called by `preflight_pinless`) computes the degrees check (`ds.crs().identifier()` and `CoordinateUnitSource::as_str`, `kernel/src/publish/mod.rs:435-443`), and the refusal fires inside `prepare_with_progress`, after the native picker resolves but before the pin phase or anything approval-related runs (`frontends/shell/src-tauri/src/publish.rs:404-409`); a request that instead passed this check would continue through the pin phase to the execute phase, where the typed confirmation is turned into an approval (`ShellApproval::new`, `:694`) — none of that is reached here. A typed refusal renders directly in the Publish panel instead, through `RefusalBlock` (`frontends/shell/src/publish/PublishPanel.tsx:652`): the code chip reads `publish.geographic_crs_not_publishable`; the message, verbatim with its placeholders filled `OGC:CRS84`/`unit:format-rule` (`kernel/src/publish/mod.rs:436-442`, `engine/src/geoparquet.rs:257-258`) — *"refused: OGC:CRS84 is a geographic CRS whose coordinates are in degrees (unit:format-rule), and the bundled viewer has no degrees path — it renders in the dataset's own CRS, while this shell's degrees display is a view-time convention a bundle does not carry. Publishing it would hand a recipient a bundle nothing can render correctly. Reproject the source to a projected CRS and open that, or wait for the reader change that adds the degrees path"* (`kernel/src/publish/error.rs:323-331`; the em dash before "it renders" is the source's own character). Below it, a guidance line whose wording the human has not yet settled (one of the four P6-sight strings enumerated with their pin at `frontends/shell/OWNER-INVALIDATION-PREREGISTRATION.md:535-539`; no test asserts it verbatim, `formatRefusal.ts:107-114`) reads, verbatim — *"This dataset's coordinates are in degrees, and the bundled viewer has no degrees path: it renders in the dataset's own coordinate reference system, while the degrees view you see here is a display convention applied at view time, which a published bundle does not carry. Nothing has been written. Reproject the source to a projected CRS and open that."* No dismiss button anywhere on this block. No destination is created — confirm the folder is empty or absent. **Judge the guidance line's wording** (the placeholder this row exists to sight) — does it read as useful, distinct from the message above it, and true? |
| N6 | **The T10 recipe, by hand** (`frontends/shell/e2e/source-changed.mjs`; its steps summarized here in this row's own words, not quoted — it is product code, not a governing document, but its comments are prose, not a shipped operator string). Open the scratch copy made before the sitting (Before N1, below); let its first view settle — pan or zoom once so features are actually resident (Part A's A4 shows what "settled" looks like). Then, in a terminal: `(Get-Item '<scratch path>').LastWriteTime = (Get-Date).AddMinutes(2)` — an mtime touch only, bytes untouched (mirrors `source-changed.mjs:726-739`'s own S3 step). Then pan or zoom to an area CLEARLY outside what was already resident (not a small pan — that is N8) so a fresh query is actually issued. | A `.canvas-session-ended` block appears in the canvas status stack, rendered through `RefusalBlock` (`frontends/shell/src/App.tsx:1566-1570`). **The canvas itself changes too, not only the status stack:** the resident features are cleared and nothing more is drawn from the ended generation — the test the record names at `frontends/shell/OWNER-INVALIDATION-PREREGISTRATION.md:367` (`a_source_changed_terminal_clears_the_working_canvas_residency`) asserts this for the baseline stream, and the one at `:372` (`a_source_changed_terminal_on_either_stream_clears_every_resident_tile`) for the candidate tiles, on a tile stream's terminal and on the untiled first-look sink alike. The code chip (`.admission-refusal-code`, `RefusalBlock.tsx:28`) reads `engine.source_changed`. The message (`.admission-refusal-message`) reads, verbatim, with `{detail}` filled `{mtime}` (only the modification time differed — `engine/src/descriptor.rs:248-257` names the differing components, `:279-285` builds the refusal) — *"refused: the source file changed while it was open ({mtime}). This check does not establish snapshot consistency, cannot detect every in-place modification, and may detect a change during a query only after that query has finished reading"* (`engine/src/error.rs:342-347`). Below it, the guidance (`.admission-refusal-guidance`, the owner's consequence — settled, asserted verbatim by a test, `formatRefusal.test.ts:150-159`) reads, verbatim — *"The source file changed while it was open. What this canvas had read from it has been cleared, and features here can no longer be identified; reopen the dataset to continue."* (`frontends/shell/src/admission/formatRefusal.ts:97-99`). Neither the message nor the guidance carries a machine prefix beyond the code chip itself (both read as plain English). **No dismiss control anywhere on this block** — `RefusalBlock` renders no `<button>` by construction (`RefusalBlock.tsx:14`, confirmed by its JSX, `:24-43`). |
| N7 | Hover the pixel that used to show a feature before N6's mutation (any point clearly inside what was resident before the change). | The hover readout (`.hover-readout.hover-readout-session-ended`) reads, verbatim — *"The source file changed while it was open, so what was on the canvas was cleared and features here can no longer be identified — reopen the dataset to continue."* (`frontends/shell/src/canvas/HoverReadoutView.tsx:67-73`; the em dash before "reopen" is the source's own character) — never an `id <number>` line, and never silence. |
| N8 | **The small-pan/no-query limitation.** Reopen the same scratch copy fresh (a full reopen, not a re-selection of N6/N7's refused state); let its first view settle exactly as N6 did. Touch its mtime again with the same one-liner as N6. This time pan only a SMALL amount, staying clearly INSIDE the area that is already resident (a short drag well within the visible field of parcels, nowhere near an edge). Then, to close the row, pan further out past the resident edge. | After the small pan: nothing changes — no `.canvas-session-ended` block appears, the canvas keeps showing exactly what it showed before the pan, and hovering a still-visible feature still answers with its ordinary `id <number>` line, not the session-ended refusal. This is not a missed detection: it is `frontends/shell/OWNER-INVALIDATION-PREREGISTRATION.md` Amendment 11 (b)'s own finding (referenced by path, not quoted) — a pan that stays inside the tiles already resident issues no fresh query, and the check that notices a changed source runs only against a query that is actually issued. Optional, record-only: open the newest `%LOCALAPPDATA%\dev.spatialide.shell\logs\session-*.log` (Part M's M12 own path) and confirm no new `viewport_query` line was written for the small pan. After panning further out: the session-ended block from N6 appears again, confirming the refusal DOES arrive once a query is actually issued — closing the row on an explained "nothing happened," not a silent one. |
| N9 | **KNOWN-LIMITATIONS rows to be sighted — read only; do not edit `KNOWN-LIMITATIONS.md`.** Open `KNOWN-LIMITATIONS.md` at the repository root (`C:\dev\spatial-ide\KNOWN-LIMITATIONS.md` — a bare basename resolves against `frontends/shell/` in this tree's own citation convention, so the root is named explicitly; 16 numbered items plus one unnumbered paragraph between items 15 and 16, v0.1.0) and confirm none of the five limitations below appear among them yet. | None do, confirmed by a full read at this commit. Five rows are owed to whoever next revises that file for a release carrying Brief A — named here, not drafted: **(1) the change-detector limitation** — N6/N7/N8 above are its felt shape: mtime/size/footer-length/footer-hash only, no snapshot claim, and the small-pan/no-query gap N8 just showed; **(2) single-file only** — a partitioned source refuses `engine.identity_ordinal_partitioned_unsupported` by name, unexercised by this Part (no partitioned fixture exists yet, boundary 7); **(3) the degrees-publish refusal** — N5's own finding, `publish.geographic_crs_not_publishable`; **(4) sanity-check limits** — a sanity check convicts, never confirms (`ADMISSION-PREREGISTRATION.md` §1, referenced not quoted); a projected file inside ±180/±90 is not caught by it; **(5) the small-pan/no-query limitation** — N8 again, named on its own because it is a distinct operator-facing gap from (1)'s broader change-detector limit. Record whether, having now met all five live, their eventual KNOWN-LIMITATIONS phrasing would read as honest to you. |

**If anything deviates:** stop, record the exact step, and report it, same as every earlier Part. N5's
and N8's small-pan step's "nothing visibly happens" are the EXPECTED outcomes of those two moments,
not deviations — the deviation would be the OPPOSITE (an approval dialog opening at N5, or a refusal
appearing after N8's small pan).

**Sitting estimate — scheduling only, not a claim about the app:** plan about 30–35 minutes at the
machine for N1–N9: three corpus opens and reads (N1–N3), one reopen-and-read (N4), one publish
attempt with a native destination picker (N5), one hand-mutation sequence spanning two separate opens
and two PowerShell one-liners (N6–N8), and a `KNOWN-LIMITATIONS.md` read-through (N9).

**Before N1.**
- **Build:** `npm run tauri dev` from `frontends/shell` (Prerequisites table, above — unchanged).
- **Corpus hashes:** before opening any of N1–N3's three files, verify each against
  `target\fixtures\compat-corpus\MANIFEST.json`'s own `sha256` field for that file's `path` — e.g.
  `Get-FileHash -Algorithm SHA256 <path>` in PowerShell, compared to the hash in the fixtures table
  above. A mismatch invalidates the corresponding row (the preregistration's own standing rule that
  every corpus file is hash-verified before a run, `ADMISSION-PREREGISTRATION.md` §8, referenced not
  quoted) — stop and report rather than continuing.
- **The scratch copy:** before N6, copy `100k-happy-path.parquet` to a scratch filename (e.g.
  `100k-scratch-partN.parquet`) under `target\fixtures\manual-walkthrough\`, so it exists ready for
  both N6 and N8 without pausing mid-sitting — N8 reopens the SAME scratch copy N6 used (a reopen
  re-baselines the descriptor at whatever mtime the file carries at that moment, and N8's own touch
  moves it forward again from there; no second copy is needed).
...
- **G11 (entry 86 — the bundle viewer's zoom anchor, added 2026-09-14):** queued for the next
  accumulated sitting bundle (`AI_DEVELOPMENT.md`, rule 11) — **not yet run.** Result log,
  verbatim when it is run:
  - **Date run:**
  - **Run by:**
  - **Build/commit (and the bundle's own publish build):**
  - **G11, window LARGER than 1280×900:**
  - **G11, window SMALLER than 1280×900:**
...
- **K7 (entry 66 (b), added 2026-09-10; expected text extended 2026-09-11 for the second batch's
  `fits` inversion):** queued for the next accumulated sitting bundle
  (`AI_DEVELOPMENT.md`, rule 11) — **not yet run.** Result log, verbatim when it is run:
  - **Date run:**
  - **Run by:**
  - **Build/commit:**
  - **K7:**
...
### Part L — first half-sitting, 2026-09-06 (split sitting; L5-L9 pending)

- **Date run:** 2026-09-06, human-present at the machine (no RustDesk — the first Part run
  locally, no degraded-channel caveat on any judgment below).
...
- **L5, first report:** *"in L5 it wasn't giving me any verbatim when zooming to layer."* A
  ...
  fill is text-less by design, and the fill was still running when judged "ended". Second report,
  verbatim: *"whn i just press zoom to layer it blocks rendering after few seconds (eventhough
  there's still space in the canvas), no status appear. If i zoom out, it starts to render a bit
  more, it's still going."* — the truncated-settle silence, DECISIONS-PENDING entry 43.
- **Zoom-out, the sitting's headline finding, verbatim:** *"If i zoom out at a certain point it
  stop, so you get what it did render till that point, and then you see that he's rendering tiles
  for half a second, then they disappear, and a new one appear and then disappear, constantly and
  then the last one rendered in the north, left corner and stayed there."* — ADR-028 Amendment 1's
  reopen condition met, felt: DECISIONS-PENDING entry 44 (reopen finding 3 as a defect; fix on the
  LOD line). Also surfaced: entry 42 (Zoom to layer fits the rendered strip, not the layer — the
  human: *"The zoom to layer should take into account the whole extent of the features, max, min
  coordinates. Not what just rendered."*), and the plain-launch session log writing nothing
  (CUT-STATE finding (i), separate).
- **Custodian errors during the sitting, named:** two wrong diagnoses issued before the trace
  ("Zoom to layer does nothing" — it worked, there was no text; "the fit anchor is never written
...
### Part L — the zoom-out-only re-run on the 48-(a) build, 2026-09-07 (PR #28 merged; main @ 8d69887; the human's words verbatim)

Launched by the custodian from the main checkout at 18:48:57Z (plain `npm run tauri dev`); the
human at the machine over RustDesk. Session log `session-1788806962.log`: **arm proof present**
(`candidate-grid-frame-established` ×1), **0 baseline ceiling refusals**, **0 evictions of
`initial-untiled-look`** (the 48-(a) mechanism, in the log), 4 `candidate-covering-truncated` lines.

- **L5:** *"L5 finishes to render what's in view and then it says "Filling has finished for this
  view - some areas were not loaded ecc""* — string 6 (F2, PR #24; entry 45 ruled A as shipped)
  sighted by the operator at a truncated within-budget settle. This row's 2026-09-06 expectation
  ("none at settle if the covering set was truncated") predates F2 and is corrected below.
- **L6:** *"L6 works properly now, nothing disappearing."* — the entry-48 defect (already-rendered
  content disappearing on zoom-out) is FIXED as felt; the log confirms zero first-look evictions.
- **Hover (L7/L8 territory), verbatim:** *"One thing have not been fixed though. Once i zoom in to a
  feature and hover over one, even if the feature is as big as the screen whole itself, if i zoom
  out by just one step, so the feature is still big AF, the id status text disappear. Now this is
  wrong, as long as I can tell on which feature I'm hovering, there's no reasono to remove the id."*
...
**M16–M18 (entries 87, 88, 89 — the polish rows added 2026-09-14):** queued for the next
accumulated sitting bundle (`AI_DEVELOPMENT.md`, rule 11) — **not yet run.** They assert nothing
about the packaged artifact or the clean profile (the placement note above the result logs says
so); record the build they actually ran on. Result log, verbatim when they are run:

- **Date run:**
- **Run by:**
- **Build/commit (and which build mode — dev, debug-built, or installed):**
- **M16 (the settled sentence under a filter, verbatim; any `tile-stream-mint-refused` lines):**
...
- **Row 4 — entry 86 (the bundle viewer's zoom anchored at the pointer):** **PASS** (2026-09-15 continuation) — verbatim: *"Row 4 is fine"*. Re-run after the migration settled, the bundle served from the C: copy of the 100k bundle (`scripts/serve-bundle.mjs`, viewer refreshed with the current dist). The 2026-09-14 *"not found"* was the mid-robocopy window, not the anchor verdict; the zoom now anchors at the pointer with no drift, across window sizes.
- **Row 5 — M18 (a) (entry 89 cursor, crosshair/grabbing):** verbatim: *"R5 is good"*. **PASS.**
- **Row 6 — PASS with one filed detail** (2026-09-15 continuation, `npm run tauri dev`, `100k-happy-path.parquet`) — verbatim: *"Row 6 is now perfect, just a tiny detail … but everything else is perfect"*.
  - **(a) M17 / entry 88 / 91 (b) — the confirming-marker WORDING:** PASS. The wording *"id 6430 · confirming…"* sighted live and accepted ("perfect"); the marker state itself was ruled, not judged here.
  - **(b) M18 (b) / entry 91 (c) — the 5/9/15 px felt verdict:** PASS. The refusal did not arrive too early; **the 9 px declared threshold STANDS**, now confirmed by the walkthrough-recorded felt verdict that alone can move it (`DECISIONS-PENDING.md` entry 91 (c) / B2).
  - **(c) entry 75 (1) — the pan re-pick feel:** the re-pick PASSES ("everything else is perfect"), but a distinct pan-**anchoring** detail was raised, verbatim: *"when i pan i'd like for the pointer to be still in the exactly same position from where i pressed to start the pan, now it goes slightly sideways"* — filed as **`DECISIONS-PENDING.md` entry 95** (the grabbed point should stay pinned under the pointer through the drag). The sitting is CLOSED; entry 95 is the only defect it opened.
...
| # | Step | Expected outcome |
|---|---|---|
| P1 | Verify the fixture's hash against `MANIFEST.json` as Part N's own "Before N1" note describes. Click **Open GeoParquet…** and select the file above. The candidate arm is the default (`residencyArm.ts:50`'s own `DEFAULT_RESIDENCY_ARM`) — no arm switch needed. Let the first view settle (Part A's A4 shows what "settled" looks like), then open the newest `%LOCALAPPDATA%\dev.spatialide.shell\logs\session-*.log` (Part M's M12 own path) and find its `candidate-grid-frame-established` line (`frontends/shell/src/residency/candidateArmSession.ts:1218-1221`). | Nothing on screen changes from what Part N's own N1 row already shows for this file — same summary, same CRS/provenance/display-convention lines, same rendered features. The session log's `candidate-grid-frame-established` line reads `baseSpan=0.8005154172768005…` (the JavaScript number for `2*Math.max(0.40025770863840027, 0.39821605223940537)`, approximately 0.8005; it was `baseSpan=2` before this piece — `MANUAL-WALKTHROUGH.md`'s own Part N run record above and the consult's P0 Step 3, `state/consults/2026-09-24-crs-unit-fact-and-bounds.md`) — the degree-keyed `MIN_ANCHOR_SPAN` (`1e-6`, `frontends/shell/src/canvas/tileGrid.ts`) applied to this file's own ~0.40°×0.40° extent, padded x2, instead of the metre-basis floor of 1. Record the line's exact `baseSpan=` value. |
