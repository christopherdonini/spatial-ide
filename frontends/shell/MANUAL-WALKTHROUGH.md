...
| M12 | `tauri build --debug`'s data-plane admission | **[Depends on item 1 (the ADR-020 origin-selector fix), a separate piece.] Not run here.** Item 1 stopped under rule 7 after two failed reviewer/architect gates on the pump-based design (`RELEASE-0.1.md` Amendment 4) — the origin-selector DESIGN itself is `DECISIONS-PENDING.md` entry 55, pending the human's word between the pump (as shipped) and the config-derived mirror the architect ranked above it. Nothing about item 1 has landed on this branch. When it does, this row becomes: install/run a `tauri build --debug` artifact and confirm the webview admits against the data plane (no 403), per item 1's own declared check. |
