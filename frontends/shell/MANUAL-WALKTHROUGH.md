...
"Part N operator-verified"**; and P3b's own record of what it owes this file, `frontends/shell/
OWNER-INVALIDATION-PREREGISTRATION.md` §9's Operator bullet. **No durations anywhere in this Part**
(`docs/08_Testing.md`; `engine/ADMISSION-PREREGISTRATION.md` §1, boundary 10/A6, ADR-018) — every
expected outcome below is a shipped string, quoted verbatim with its source `path:line`, never a
time, a rate, or a smoothness judgment. All `path:line` cites below are against
**`origin/main` @ `158d4503f9c14c05cb19938ab4cd2502e5aad3b4`** unless a different commit is named
(the two commits between that tip and `004820b`, the tree this Part's source facts were actually
read against, touch only `state/CUT-STATE.md`, `PLAN.yaml`, `CUSTODIAN-QUEUE.*`, `DECISIONS-
PENDING.md`, `site/`, and a new spike directory — none of the files cited below); every quote was
copied from the cited file by direct read, never from memory or from a preregistration's paraphrase
— the reviewer byte-compares each.
...
quick open, not a 100k-scale one.

| # | Step | Expected outcome |
|---|---|---|
| N1 | Click **Open GeoParquet…** and select the file above (compat-corpus #8 — an absent `crs` key over source data whose own upstream declared EPSG:4326 lat-first; §3 row 8's own `collection_note`). | The summary appears (240 rows, `identity-uniqueness-scan-full-file` absent since this is the session tier — see N3). The **CRS** line reads `OGC:CRS84 — format-rule, axis order longitude,latitude` — the template `` `${crs.identifier} — ${crs.source}, axis order ${crs.axis_order}` `` (`frontends/shell/src/admission/describeSummaryText.ts:27`), filled with `identifier` `OGC:CRS84` (`FORMAT_DEFAULT_CRS`, `engine/src/geoparquet.rs:44`), `source` `format-rule` (a file-arm CRS is re-stamped to this label only when the format DEFAULT applied — `engine/src/dataset.rs:433-437`; the label string itself is at `engine/src/crs.rs:107`), and `axis_order` `longitude,latitude` (`AxisOrder::LongitudeLatitude`, the absent-key R-C2 branch, `engine/src/geoparquet.rs:524-539`). No refusal panel. |
| N2 | Click **Open GeoParquet…** and select the file above (compat-corpus #3 — the SAME real-world CRS as N1, WGS 84 longitude/latitude, reached because THIS file itself declares `EPSG:4326` lat-first and the format's WKB axis override applies; §3 row 3). | The summary appears (240 rows). The **CRS** line reads `EPSG:4326 — file, axis order longitude,latitude` — same template (`describeSummaryText.ts:27`), filled with `identifier` `EPSG:4326` (the file's own declared identifier), `source` `file` (unchanged from N1's `format-rule`: only a format-DEFAULT admission is re-stamped, `dataset.rs:433-437` again — this file's CRS is declared, so `crs::admit`'s file arm keeps `CrsSource::File`, `engine/src/crs.rs:199-213`), `axis_order` `longitude,latitude` (the file's own declared `latitude,longitude` order is flipped to the WKB-established x-first reading here — `data_order_under_wkb_rule`, `engine/src/geoparquet.rs:499-508` and `:567-573`; the ORIGINAL declared order is retained internally per boundary 1 but is not rendered anywhere in this build — a repo-wide check shows `crs.provenance`/`crs.axis_provenance` are read only inside `describeSummaryText.ts`'s own type imports and test files, never rendered). **The sight itself:** N1 reads `OGC:CRS84 — format-rule, ...`, N2 reads `EPSG:4326 — file, ...` — distinguishable by identifier and by source word (not by the axis-order text, which is identical on both). Record whether this reads as clearly distinguishable to you at a glance. |
| N3 | Click **Open GeoParquet…** and select the file above (compat-corpus #1 — a declared, x-first EPSG:2056 file whose only integer column is `parcel_id`, not `id`; admitted-as-declared, session tier; §3 row 1). | The summary appears (240 rows). The **Identity** line (`DescribeSummary.tsx:28-29`) reads `session-ordinal:file_row_number — by-construction-within-generation` — the template `` `${identity.source} — ${identity.uniqueness}` `` (`describeSummaryText.ts:38`), filled with `identity.source` (`format!("session-ordinal:{FILE_ROW_NUMBER_COLUMN}")`, `engine/src/identity.rs:84`, where `FILE_ROW_NUMBER_COLUMN` is `"file_row_number"`, `:73`) and `identity.uniqueness` (`"by-construction-within-generation"`, `:133`). **A finding, not a shipped string to sight on screen:** the fuller session-tier sentence, `SESSION_IDENTITY_STATEMENT` (`engine/src/identity.rs:145-149`) — carried over the wire as `describe.identity.session_statement` (`frontends/shell/src/skp/types.ts:124`) and asserted at the fixture-test level (`frontends/shell/src/skp/__tests__/fixtures.test.ts:185`) — is **not rendered by any DOM component at this commit**: `identitySummaryLine` never reads `session_statement` (`describeSummaryText.ts:37-39`), and `DescribeSummary.tsx` renders only the compact Identity line above (`:28-29`). Record what the Identity line actually says on screen; the fuller sentence is shipped on the wire and test-covered, not sighted, and that gap is itself the honest record this row owes rather than a string to invent. |
...
machine for N1–N9: three quick corpus opens and reads (N1–N3), one reopen-and-read (N4), one publish
...
Fill in after running the script above: date, run by, build/commit, pass/fail per row N1–N9, N2's
distinguishability verdict, N5's guidance-wording verdict, N6's `{detail}` text as actually rendered,
N8's small-pan observation (and the session log check if taken), and N9's honesty verdict on the five
named KNOWN-LIMITATIONS rows.
