...
landing changed three files this Part cites, each re-read whole at the new commit:
`frontends/shell/src/admission/describeSummaryText.ts`, `frontends/shell/src/admission/DescribeSummary.tsx`,
and `frontends/shell/src/skp/__tests__/fixtures.test.ts` (N1's and N3's own rows, below). That same
re-read found two more of this Part's cites whose line numbers shifted since `158d450` without this
piece correcting them — `engine/src/dataset.rs:433-437` (N1, N2; the content is now at `:467-471`,
corrected in N1's row only) and `frontends/shell/e2e/source-changed.mjs:628-640` (N6; its S3 step
moved under a new POST-route branch) — filed here as a known gap, not fixed under this piece's
declared Scope. Every quote was copied from the cited file by direct read, never from memory or from
a preregistration's paraphrase — the reviewer byte-compares each.
