...
| L5 | **Block B. [CORRECTED 2026-09-06, after the first half-sitting observed no suffix — the original expected outcome below was wrong for this fixture, and the correction is code-grounded, not yet trace-confirmed.]** Reproduce L1's fill (Ctrl+R, re-arm, reopen, Zoom to layer) and watch the status line DURING the fill. | **Expect NO durable paused suffix on this fixture — that is the correct behavior, not a miss.** Over-budget latches mid-fill with tiles still queued (`candidateArmSession.ts` ingest path), but each admission then evicts partial in-viewport tiles (ADR-028 Amendment 1's declared exception), restoring headroom below the 0.9 margin — the drain resumes in a sawtooth until every tile terminates and the fill settles (which is also why L1's settled suffix appears at all: settling requires zero tracked tiles). The `stalled` state — *" Filling is paused until the next pan or zoom."* — exists here only in transient latch-to-eviction windows, likely sub-perceptual; its DURABLE form needs an over-budget view whose in-viewport tiles are complete/unevictable (the 5 GB shape, deferred behind entry 40). The original step text claimed the stall would show, with a "queue drains before the budget fills" escape hatch — **both halves of that were wrong** (the budget latches mid-fill; the queue drains via the eviction sawtooth, not before the latch). String 1's wording itself was already ruled 2026-09-05 and is BS3 unit-pinned, so nothing in the cut's close gates on sighting it live at this scale. **Optional confirm (your own instance, DevTools, before clicking Zoom to layer):** paste `(async()=>{const out=[];const t0=performance.now();const timer=setInterval(async()=>{const q=await window.__SPATIAL_E2E__.residencyQueuedTileCount();const el=document.querySelector(".residency-status");const text=el?el.textContent:null;const last=out[out.length-1];if(!last||last.q!==q||last.text!==text)out.push({t:Math.round(performance.now()-t0),q,text});},50);window.__L5_PROBE_STOP__=()=>{clearInterval(timer);console.table(out);return out.filter(s=>s.text&&s.text.includes("paused")).length+" paused sightings";};console.log("L5 probe armed — click Zoom to layer now; run __L5_PROBE_STOP__() after settle.");})();` — it samples the queue count and status text at 50 ms and shows every transition; queued>0 rows alongside over-budget text with zero/near-zero "paused sightings" CONFIRMS the sawtooth account; many-second paused sightings would REFUTE it and reopen this as a wiring question. |
...
assuming it was unrelated. L4's missed window, L5's paused suffix not appearing (per that step's
own corrected expectation — the eviction sawtooth keeps the stall transient at this scale), and
string 5 never appearing are all named expected outcomes, not deviations.
...
- **Blocks A (L1-L4), the human's verdicts verbatim:** *"L1 perfect, L2 perfect, L3 feels good,
  L4 honest."* All four pass; no deviations reported.
- **L5, reported before pausing the sitting:** *"in L5 it wasn't giving me any verbatim when
  zooming to layer"* — investigated same day machine-side and resolved as CORRECT behavior wrongly
  scripted: over-budget latches mid-fill but the ADR-028 Amendment-1 eviction exception keeps
  restoring headroom, so the drain resumes in a sawtooth and the stall stays transient at this
  fixture's scale (also exactly why L1's settled suffix can appear at all). L5's step text carried
  a wrong mechanism in both its expectation and its escape hatch — corrected in place, dated, with
  an optional in-DevTools confirm probe (code-grounded account, not yet trace-confirmed; the probe
  is the confirmation path). Durable-stall sighting moves to the 5 GB scale, behind entry 40.
- **L5-L9:** pending — the sitting resumes on the human's schedule; this entry completes then.
