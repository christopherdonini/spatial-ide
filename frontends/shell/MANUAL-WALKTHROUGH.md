...
| K7 | **Entry 66 (b) — geometric protection at every zoom. Queued for the next accumulated sitting bundle (`AI_DEVELOPMENT.md`, rule 11); this row never asks for same-day operator time.** From `polygons-100k.parquet` at a "Zoom to layer" fit, wheel out roughly eight notches, wait for the status to settle, then pan slowly in one direction. | Expected: tiles already drawn stay drawn while they remain on screen; the status still reports the view as partial. A tile that vanishes while still visible is the finding this row exists to catch. **Judge:** while you pan at that zoom, does the picture hold together — the same areas staying drawn as they move across the screen — or do parts of it drop out and come back? Describe what you see in your own words; the status wording itself is K2's job, not this row's. |
...
- **K7 (entry 66 (b), added 2026-09-10):** queued for the next accumulated sitting bundle
