...
scheduling for the custodian, not a claim about the app). All `path:line` cites below were read at
commit `158d4503f9c14c05cb19938ab4cd2502e5aad3b4` (`158d450`, origin/main when written — a pin, not
a claim origin/main reads this way now: origin/main has moved on since, and none of the files cited
below differs between `158d450` and it; the two commits between `004820b` (this branch's original base) and `158d450`
touch only `state/CUT-STATE.md`, `PLAN.yaml`, `CUSTODIAN-QUEUE.*`, `DECISIONS-PENDING.md`, `site/`,
and a new spike directory — none of the files cited below in either case). Every quote was copied
from the cited file by direct read, never from memory or from a preregistration's paraphrase — the
reviewer byte-compares each.
...
| N3 | Click **Open GeoParquet…** and select the file above (compat-corpus #1 — a declared, x-first EPSG:2056 file whose only integer column is `parcel_id`, not `id`; admitted-as-declared, session tier; §3 row 1). | The summary appears; the **Row count** line reads `not established (not-established)`, same as N1/N2 (`DescribeSummary.tsx:12-15`; `kernel/src/skp.rs:1019-1023`, `None` arm `:1023`). The **Identity** line (`DescribeSummary.tsx:28-29`) reads `session-ordinal:file_row_number — by-construction-within-generation` — the template `` `${identity.source} — ${identity.uniqueness}` `` (`describeSummaryText.ts:38`), filled with `identity.source` (`format!("session-ordinal:{FILE_ROW_NUMBER_COLUMN}")`, `engine/src/identity.rs:84`, where `FILE_ROW_NUMBER_COLUMN` is `"file_row_number"`, `:73`) and `identity.uniqueness` (`"by-construction-within-generation"`, `:133`). **P6's second named sight (this Part's own phase text, `state/NEXT-CUT.md:107`; the same row in the draft at `state/drafts/post-tag/DRAFT-2-BRIEF-A-admission-and-session-lifecycle.md:95`) is NOT discharged by this row.** The fuller session-tier sentence, `SESSION_IDENTITY_STATEMENT` (`engine/src/identity.rs:145-149`) — carried over the wire as `describe.identity.session_statement` (`frontends/shell/src/skp/types.ts:124`) and asserted only at the fixture-test level (`frontends/shell/src/skp/__tests__/fixtures.test.ts:185`) — is rendered by no component in this tree: `identitySummaryLine` never reads `session_statement` (`describeSummaryText.ts:37-39`), `DescribeSummary.tsx` renders only the compact Identity line above (`:28-29`), and no other component reads the field either. **There is nothing on screen for an operator to read as "the session-identity statement."** Recording that gap here does not discharge the sight — either a component renders `session_statement` before the sitting, or the human re-scopes what this sight asks for; that choice is a red-line call for the human, not this row's to make, and it is recorded in `DECISIONS-PENDING.md` by the custodian, not settled here. Until then, record only that the Identity line above is what the screen actually shows. |
