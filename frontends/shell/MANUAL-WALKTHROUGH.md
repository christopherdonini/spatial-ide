...
| M3 | The Notices view, and `NOTICE.txt` beside the executable | Click the collapsed **▸ Notices** disclosure (bottom of the window, beside **▸ Console**). It expands (**▾ Notices**) and renders the shipped notice text. Separately, in the install directory (found via the Start Menu shortcut's own "Open file location", or `%LOCALAPPDATA%\spatial-ide-shell\` — the per-user NSIS install root; this hint is a plausible location, not asserted exact — confirm from the shortcut if it differs), open `NOTICE.txt` in a text viewer. **Both** carry, verbatim: *"Geodetic Parameter Dataset, © IOGP (International Association of Oil & Gas Producers), used under the EPSG Terms of Use: https://epsg.org/terms-of-use.html"*; and, further down, under **"THE SPATIAL IDE APPLICATION, WHEN DISTRIBUTED AS AN INSTALLED PROGRAM"**, *"Its Corresponding Source, as AGPL-3.0 §6 (conveying non-source forms) requires, is available under route §6(d) (access from a network server) at: https://github.com/christopherdonini/spatial-ide"*. Also present, verbatim from the regenerated `notice()` header (RELEASE-0.1 Amendment 6's authorized sweep): *"this file enumerates ONLY the third-party works compiled into the bundle VIEWER -- it does NOT enumerate two further sets of third-party works the installed application also carries: (i) the packaged frontend's OWN npm dependencies, compiled into frontends/shell/dist and conveyed via tauri.conf.json's `frontendDist` -- react, react-dom, @deck.gl/core, @deck.gl/layers, and the shell's own apache-arrow, each a separate install from the copies compiled into this viewer; and (ii) the Rust crates statically linked into the kernel/data-engine/renderer binary the application also embeds (kernel, engine, protocol, and their own dependencies), listed instead in this repository's own DEPENDENCY-LICENSES.md. Generating BOTH of those into the installed copy of this file is OWED, not yet done, under RELEASE-0.1's item 9 (both notice generators, before the v0.1.0 tag) and ADR-030 (docs/adr/ADR-030-conveyed-artifact-notice-set.md, Proposed, candidate (a)) -- named here rather than silently assumed covered by the third-party section below."* **Byte-identical is the claim under test, and already a real one** — `frontends/shell/src/notices/noticeByteIdentity.test.ts` asserts `src/generated/NOTICE.txt` equals `renderer/bundle-viewer/dist/NOTICE.txt` byte for byte; this step is the felt/visual confirmation on the real, installed artifact. |
...
| M10 [pending the human's entry-53 word] | The ADR-025 refusal, on a source above the reader's ceilings | With `parcels-5gb.parquet` admitted FRESH in this Part (Part H's fixture, 3,300,000 rows — above `MAX_FEATURES` = 2,000,000; a fresh admission, not H6's — `ContentPin` lives in memory, per app instance, not persisted across sessions), click **Publish…**, choose **Whole dataset**, and pick a destination. **Before anything else: the "Preparing…" wait is the SAME pre-existing, uncancellable, progress-less whole-file SHA-256 `ensure_pinned` always pays on a not-yet-pinned dataset** (`publish.rs:753-761`'s own disclosed limitation, unrelated to ADR-025 — named here because this is exactly the 5 GB scale that limitation calls out as "a real gap"), THEN preflight runs and refuses — the native picker's own path is already resolved and converted by this point (it ran first, in `binding_publish_prepare`), so what is true is narrower than an earlier draft of this row claimed: **the approval (typed-phrase) dialog never opens**, not that nothing was acted on yet. The typed refusal renders in the Publish panel instead (verbatim, `kernel/src/publish/error.rs`'s `PublishError::ReaderCeilingExceeded` Display): *"refused: this publish is predicted to exceed the bundled viewer's declared ceiling MAX_FEATURES — limit 2000000, predicted 3300000 — before any bytes were written (ADR-025: refuse, typed, at preflight; reopens when a second reader exists). Instead, publish the current-viewport bbox instead of the whole file (the viewer's ceilings apply to what a bundle carries, not to what the source dataset holds)"* No destination is created — confirm the folder is absent or empty. **By design, this refusal produces NO audit record at all** (`kernel/src/permission/boundary.rs`'s own "what is not audited" list; `boundary::execute`'s first fallible call is `preflight(req)?`, before the audit log is even opened for writing) — M9's `--audit-show` run will not show this attempt, and that absence is correct, not a gap to chase. |
