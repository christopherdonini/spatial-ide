...
#[test]
fn the_first_batch_arrives_without_waiting_for_the_whole_result() {
    ...
    let (_, _, _, _) = drain(&mut s);
    let total = t0.elapsed();

    assert!(
        first * 3 < total,
        "first batch at {first:?} against a total of {total:?} — that is not streaming"
    );
}
