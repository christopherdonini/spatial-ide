...
**Amendment 10 — class 4, a mutation added after a gate finding, with its observed failure recorded by name. Written 2026-09-17 by the `engine/lod-tier-builder` test-isolation worker (PR #84), AFTER `cargo test --workspace` on `windows-latest` found `engine/tests/lod_tier_builder.rs` racy.** No file under `engine/src/` differs from before this amendment; only test code changed. **No measured figure is written into this document by this amendment.**

**(a) The finding.** CI runs 35170101692 and 35170138013 (both identical) failed `the_rejected_simplifier_is_the_one_that_emits_invalid_polygons` at `engine/tests/lod_tier_builder.rs:116:10` (this file's line numbers before this amendment) with `reader: EOF("Parquet file too small. Size is 0 but need 8")`; 13 other tests in the binary passed. `polygons_100k()`'s fixture check-and-generate ran unguarded on a fresh runner where the fixture was absent, and this crate's tests in one binary run in parallel threads: one thread's `is_file()` check could see the file the moment `File::create` made it, before `write_geoparquet` had written anything into it, and open it for reading — the "reader: EOF" panic. §3's own words already named this file "regenerated from the same seeded spec if it is not there"; what they did not say, and the finding shows, is that regeneration was neither serialized nor published atomically.

**(b) The fix, and where it lives.** `polygons_100k()` moved out of each of the three LOD test binaries and into one shared function, `engine/tests/common/mod.rs::polygons_100k` (`mod common;` in `engine/tests/lod_tier_builder.rs:56`, `engine/tests/lod_tier_cancellation.rs` and `engine/tests/lod_tier_measurements.rs`; `tests/common/mod.rs` is outside cargo's `tests/*.rs` auto-discovery, so it is never a fourth test binary). Two changes, both load-bearing and for different reasons (`engine/tests/common/mod.rs:44-51`): (i) a process-wide `Mutex` around the whole check-and-generate, so the first caller through an absent fixture generates and every other caller waits for that one generation rather than starting its own (`:53-55`); (ii) generation writes to a temporary path in the fixture's own directory and is `rename`d into place only once complete (`:58-65`), so nothing under the final name is ever a partial write for a reader that opens it directly. **Both discharged**, each proven by a named test below.

**(c) The proof, and why it counts generations rather than trusting the file's content.** `engine/tests/lod_tier_builder.rs`'s `two_concurrent_callers_of_an_absent_fixture_both_get_the_complete_file` deletes the fixture, calls `polygons_100k()` from two threads at once, and asserts each thread's own return is already backed by the declared, complete file (151,812,642 bytes, `LOD-PREREGISTRATION.md:146`'s table row) with identical SHA-256 between the two threads — proving (ii). It then asserts `common::generations_performed()` (a test-only instrument counting actual calls into the generator, `engine/tests/common/mod.rs:70-84`, whose only caller is this test) advanced by exactly one — proving (i). **`RECORDED MUTATION`** (removing only the `Mutex` guard, leaving the temporary-path write and the rename untouched): the size and SHA-256 assertions still passed on this machine — two threads independently writing the *same deterministic* bytes to the shared temporary name did not corrupt it here — but the generation count did not: **observed** `left: 2, right: 1` at `engine/tests/lod_tier_builder.rs:187:5`, recorded in full beside the test. This is why the test does not rely on corruption to prove the lock: on this machine's NTFS it would not have shown one.

**(d) A second, smaller finding, from writing the proof: this fixture's path is shared, not per-worktree, and the proof test cannot run alongside its siblings.** `POLYGONS_100K` is one absolute path (`C:\dev\spatial-ide\target\fixtures\slice-budgets\polygons-100k.parquet`), not resolved under `CARGO_TARGET_DIR` or `CARGO_MANIFEST_DIR` — every worktree's test run shares the same file on disk (§3 and `engine/tests/common/mod.rs:15-19` already said so; this amendment is the first place a test's *own* behaviour depended on it). A first version of `two_concurrent_callers_of_an_absent_fixture_both_get_the_complete_file` deleted the fixture unconditionally at its own start and ran un-ignored: alongside the rest of the suite it raced two siblings' own direct reads of the same path (`engine_opens_its_own_tier`, `the_rejected_simplifier_is_the_one_that_emits_invalid_polygons`) and failed both with `NotFound`/"No files found", **observed** on this machine in that configuration. `polygons_100k()`'s lock guarantees the file is complete the instant any call to it *returns*; it guarantees nothing about a *different* thread deleting the path afterward, which is what that first version did. The test is `#[ignore]`d for this reason (`engine/tests/lod_tier_builder.rs`, its own doc comment and ignore message) and run by itself, matching this file's `#[ignore]` and `--exact` convention for the 5 GB rows.

**(e) The suite-wide proof, without a test deleting anything mid-run.** `CARGO_TARGET_DIR` set to a worktree-local target, `polygons-100k.parquet` deleted once *before* `cargo test` starts, then `cargo test -p spatial-engine --test lod_tier_builder` (no `--ignored`): **14 passed, 0 failed, 2 ignored** — every test that calls `polygons_100k()` from the now-absent fixture (inside `ladder()`'s `OnceLock`, and independently in `the_rejected_simplifier_is_the_one_that_emits_invalid_polygons` and `wkb_writer_round_trips_the_first_tier_written`) raced the lock, not a sibling's read, and all of them observed a complete file. The regenerated fixture's SHA-256 (`9ecd79242ac7d99e09f1989c8c124fd53dcd697689546ec6013949f806ca6043`) is identical to the fixture's SHA-256 before this amendment's testing began, both taken by `certutil -hashfile … SHA256` on the same path — the generator is deterministic and this fix changes when it runs, never what it writes. `engine/tests/lod_tier_cancellation.rs` and `engine/tests/lod_tier_measurements.rs`'s non-ignored tests pass unchanged (`cancel_observed_within_the_declared_ceiling`; `the_row_order_check_fires_on_a_permutation`, which does not touch this fixture).

**(f) What this amendment does not do.** It amends no ADR and changes no ADR's status; it makes no performance claim; it changes no line under `engine/src/`; and it changes nothing in §§1–9 or in Amendments 1–9.
