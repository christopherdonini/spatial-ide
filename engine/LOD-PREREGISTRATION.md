...
- **I1 — the Arrow 58 vs 59 reconciliation.** The workspace pins one Arrow version, verbatim from the repo-root manifest `../Cargo.toml:41-45` (cited relative to this file's directory; not `engine/Cargo.toml`):
...
**Amendment 4 — class 3, cite fixes; mechanical, no claim changed. Written 2026-09-16.** Two cites in this document were unresolvable by `scripts/plan/verify-cites.mjs` (which resolves a bare basename against the citing file's own directory, `engine/`): §5 I1's `Cargo.toml:41-45` meant the repo-root workspace manifest, not `engine/Cargo.toml` — now written `../Cargo.toml:41-45`; Amendment 3's shift list wrote the spike README as a bare `README.md` — now the full path `spikes/lod-feasibility/README.md`. Found because the check failed on `main` from the commit that added Amendments 1–3 (2a939d3) — recorded in `state/CUT-STATE.md` as a self-inflicted red of the governance CI.
